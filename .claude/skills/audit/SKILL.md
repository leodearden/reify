...
**Exit 125 is now overloaded.** It previously meant only an infra/setup error — malformed argv, a rejected `--pre-done` flag combination, an unloadable tasks source, an unopenable runs-db; it now also covers an unusable index. A serve that cannot be reached has **never** exited 125 — it fail-softs, per the *Serve unreachable* arm above. The marker token in stderr is the discriminator **when there is one** — a 125 whose message names freshness but carries no token is the unreadable-HEAD case above, not a dead serve.

...
TS_CACHE="${TMPDIR:-/tmp}/prd-gate-ts-cache-$(id -u)-$(git rev-parse --show-toplevel | sha256sum | cut -c1-16)"
mkdir -m 700 -p "$TS_CACHE"
[ -d "$TS_CACHE" ] && [ ! -L "$TS_CACHE" ] && [ -O "$TS_CACHE" ] \
  || { echo "refusing: untrusted grammar cache dir $TS_CACHE" >&2; exit 1; }
XDG_CACHE_HOME="$TS_CACHE" tree-sitter parse --quiet /tmp/prd-gate-fixtures/<slug>-<n>.ri
```

`$TS_CACHE` is **derived, not random**, and that is the whole point. If you are running these commands as an agent, every tool call gets a **fresh shell** — a `TS_CACHE` you set in one call is gone by the next — so a `mktemp -d` idiom would silently mint a new dir per fixture, pay the full cold compile every single time, and leave a 776 KB `reify.so` behind in `/tmp` for each one: the exact opposite of what the **Performance note** below promises. Deriving the name from the repo root instead gives you the *same* dir on every call, in every shell, for the whole session, so only the first parse pays the compile. It stays keyed to this worktree, which is the isolation precondition 3 is about.

The last three lines are what make a derived name *safe* rather than merely convenient, and they are not optional. `/tmp` is world-writable and `tree-sitter` **executes** the `reify.so` it caches there, so a derived — hence fully predictable — name is precisely what a hostile local uid would need. The sticky bit on `/tmp` stops it deleting your entry; it does not stop it creating one first at a name nobody has claimed yet. Against that, `mkdir -p` is silently permissive: it succeeds on a directory another uid already owns, and on a symlink to one. So each line earns its place — `$(id -u)` in the name removes cross-uid collision outright; `mkdir -m 700` sets the mode *atomically* at creation, where a separate `chmod 700` would leave the dir group-writable under this host's 0002 umask for as long as it takes to plant a `.so`; and the `-L`/`-O` test **aborts instead of parsing** when the dir is not a real directory you own, which a trailing `chmod` cannot do — a `chmod` that fails EPERM on a foreign dir is a *separate command*, so the parse on the next line would still run and still dlopen the foreign `reify.so`. Mode protects a directory you created. It cannot protect one someone else created first. That is the same contract `scripts/prd-capability-check.py` enforces in code for the automated gate (`_assert_cache_dir_trustworthy`, under a docstring headed "WHY THIS IS NOT PARANOIA").
...
To inspect a failure, drop `--quiet` and look for `(ERROR ...)` nodes in the CST. `$TS_CACHE` is a *shell* variable, not an export, so in a fresh tool call it is unset and this snippet would silently parse against the host-global cache precondition 3 warns about — re-run the setup block above first. It is idempotent, and warm it costs ~0.01 s:
...
**Caveat before you pin it.** "Verbatim" is literal: on the override path `prd-capability-check.py` does `os.makedirs(override, exist_ok=True)` and returns, with neither the `mode=0o700` nor the `_assert_cache_dir_trustworthy()` call it applies to its own derived dir. Pinning therefore opts the automated gate out of the ownership validation it would otherwise run — so only ever point it at a dir minted by the guarded setup block above, never at a bare path you have not checked.

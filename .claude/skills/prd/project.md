...
- **Re-baselining an EXISTING `.ri` fixture that a Rust test reads** — verify.sh's `_RUST_COUPLED_RI_FIXTURES` set, today the `geometry_let_selector_consumer*`, `stdlib_ns_*` and `compiler_type_hygiene_*` names — still escalates `pre-commit` to the full workspace suite, **by design**: those fixtures are runtime inputs to compiled test targets, and a hook-gated commit on `main` has no later gate. That edit belongs on a task branch through the merge queue, not in the docs-landing commit. Adding NEW fixtures does not.

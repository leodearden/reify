...
**Author-mode rule (Stage 2 and Stage 10) and decompose-mode rule (the Step 1 gate re-walk):** reject/fix a `path:line` anchor with no as-of stamp in its header **that this session itself introduces or edits**, before filing the batch. Scoped deliberately: the corpus already carries hundreds of pre-existing undated anchors (see **Measured 2026-08-19** below), so an unscoped reject/fix would make decomposing any existing PRD an unbounded de-numbering job — and the cheap alternative, retro-stamping an as-of SHA the anchors were never re-located against, would manufacture the exact false provenance **Never re-anchor a dated snapshot** (above) forbids. A legacy undated anchor in a PRD the session does not otherwise touch is not a batch blocker; it is `#5872`'s domain (see **Enforcement posture** below), not a decompose-time fix-up.

...
Both branch routes stamp `DF_VERIFY_ROLE=merge`, which makes `scripts/verify.sh` **force** `--scope all` — unconditionally, overriding even an explicit `--scope` (contract C2 — the top-level `DF_VERIFY_ROLE` → `SCOPE="all"` block, whose comment opens "Defensive belt-and-braces (contract C2): the merge gate never narrows") — and **default** `--profile` to `both` (the role-driven `PROFILE="both"` assignment in the same role-defaults region just above it, gated on `PROFILE_EXPLICIT` so an explicit `--profile` still wins; verify.sh's own C2 comment calls it "the role-driven --profile default". Both are top-level blocks that run *before* `decide_scope` is called, not part of it); dark-factory's INV-1 (`verify.py:5499`, DF task 2883) additionally refuses the docs-only trivial pass at merge role and escalates it to the full global gate, because an adoptable merge verdict must never be stamped from zero evidence. A docs-only merge therefore costs a full cold debug+release workspace build — reify budgets `merge_verify_cold_command_timeout_secs: 10800` (3 h). A commit on `main` instead runs `hooks/pre-commit` → `verify.sh all --profile debug --scope staged --include-infra` with `DF_VERIFY_ROLE` unset (role `task`, so no C2 force), where `decide_scope` classifies every one of the artifacts as no-heavy-checks — the `.md`/`.yaml` via its `*)` catch-all, which delegates the inert class to `reify_is_inert_path` (`scripts/affected-crates-lib.sh`, the task-#7427 SPOT move), and a `tests/prd-gate/fixtures/*.ri` whose basename is in **neither `_RUST_COUPLED_RI_FIXTURES` nor `_GUI_COUPLED_RI_FIXTURES`** (tasks #5536, #6435) via its own arm sitting *ahead* of that catch-all: seconds — so the one-commit landing really is seconds *including* the fixtures.
...
- **Re-baselining (or renaming) an EXISTING `.ri` fixture that a Rust test reads** escalates `pre-commit` to the full workspace suite **only if that fixture is registered** — i.e. it lives under `tests/prd-gate/fixtures/` and its basename is in `_RUST_COUPLED_RI_FIXTURES` (the assignment of that name in `scripts/verify.sh`, consumed by the `case "$_RUST_COUPLED_RI_FIXTURES" in` arm inside `decide_scope`). A fixture absent from that list but present in `_GUI_COUPLED_RI_FIXTURES` (task #6435, the sibling arm nested in its `*)` fallback) is **not** inert either: it sets `gui=1` alone, running the vitest/typecheck lane — minutes, not the seconds of the fast path, and not the full workspace suite. Read both lists in `scripts/verify.sh` rather than trusting a copy: the `stdlib_ns_*` / `compiler_type_hygiene_*` families are split across `_RUST_COUPLED_RI_FIXTURES` membership rather than uniformly absent from it, and every one of them is in `_GUI_COUPLED_RI_FIXTURES` — so none of them takes the seconds path. Registration is kept honest behaviourally: `tests/infra/test_verify_scope.sh`'s PG-DRIFT re-derives the coupled set from `git grep` over tracked sources on every infra run and goes RED on an unregistered reference; its PG-DRIFT-GUI sibling does the same for the GUI list against the `reifyGrammarCorpus` `EXPECTED_CLEAN` pins. Such an edit belongs on a task branch through the merge queue, not in the docs-landing commit. Adding a NEW fixture does not escalate.
  **Known hole, do not rely on the fast path here:** a fixture under `docs/prds/**/fixtures/` gets **no** escalation at all — `decide_scope`'s `*)` catch-all hands the path to `reify_is_inert_path` (`scripts/affected-crates-lib.sh`), which matches `docs/*` (as well as `*.md`/`*.yaml`/`*.yml`) unconditionally, with no allowlist and no drift guard, and a bash `case` glob's `*` spans `/`, so everything under `docs/prds/**/fixtures/` is inert whatever its extension — yet five such fixtures are `include_str!` build inputs to compiled test targets that pin exact values. Editing one lands GREEN on `main` and reds an unrelated merge later. That is precisely why the directory is deprecated; #6431 empties it and #6077 closes the wider `docs/`↔compiled-test class.
- **A landed `delivered_check` correction is not one write.** `commit_planning` copies `delivered_checks` into producer task metadata exactly ONCE, at landing (Sequence step 2 above) — a later fix to a manifest pattern must be applied BY HAND to the producer task's `metadata.delivered_checks` too, or the dispatch gate keeps evaluating the stale descriptor. See "Capability Manifest — reify evidence forms" → "Correcting a landed check is THREE writes: .yaml, .md twin, producer task metadata."
...
`scripts/prd-decompose-verify.mjs` is the **entry point** — a thin orchestration shell over `scripts/prd-decompose-verify.py`, the **deterministic harness** it shells out to. All load-bearing adjudication (premise binding, the evidence gate, record classification, the captured-output report) lives in the Python harness and is unit-tested there; the `.mjs` only drives agents and aggregates.
...
The gate fails in **both** directions, so `blocks` alone is not the answer. Read these fields off the workflow's top-level return; none of them requires opening `journal.jsonl`:
...
2. **Exemplar-corpus update.** If the change introduces or alters an authoring idiom, a leaf adds/updates a worked example under `examples/best_practices/` (auto-compile-gated by `crates/reify-compiler/tests/harness_compilation_surface/examples_smoke.rs`; keep normative claims in code/constraints, not comments) plus its `INDEX.md` line. Where the new feature enables a cleaner idiom than existing corpus or stdlib code uses, updating those exemplars is in scope for this PRD. (Corpus seeded by task #5397.)
...
4. A whole-file `absent` grep over a Rust source also scans that file's inline `#[cfg(test)]` modules. The `grep` kind is single-line `git grep -E` with no region exclusion, and a git pathspec cannot exclude part of a file. So when the defect construct is also a natural test-fixture shape (e.g. a `field: Vec::new()` struct-literal init), a test fixture keeps the check FAILED forever. Assert the positive production construct instead (rule 3) and name its behavioural cover (rule 2).
...
Worked case — **esc-5469-12** (2026-09-25, rule 4, capability `registry-threads-real-dependent-cells`, task 5467, `docs/prds/v0_6/discrete-cost-minimisation.capability-manifest.yaml`):
- Before: `pattern: dependent_cells:\s*Vec::new\(\)`, `expect: absent`, `paths: [crates/reify-constraints/src/registry.rs]` matched only a ResolutionProblem fixture builder inside `#[cfg(test)] mod objective_consumption_tests`. The capability WAS delivered, yet the check blocked #5469/#6077/#6207/#6431 at critical L2 (esc-5469-12, esc-6077-4, esc-6207-1, esc-6431-4).
- After: `pattern: dependent_cells:\s*sub_dependent_cells`, `expect: present` — the per-component slice in `SolverRegistry::solve_inner`. rc=1 at the pre-delivery base `20abfe0103`, rc=0 on main.
- Accepted gap: proves the field init, not the fold contents; `crates/reify-constraints/tests/joint_drive_per_trial_recompute.rs` is the behavioural cover.
- Corrected with the three writes below (#7739): `.yaml` `7a31553267`, `.md` twin `5a8ad43416`, task 5467 `metadata.delivered_checks`.

**Correcting a landed check is THREE writes: .yaml, .md twin, producer task metadata.** The dispatch gate evaluates the DEPENDENCY TASK's `metadata.delivered_checks`, not the manifest file on disk — the evaluator is `Scheduler._compute_delivered_check_cache` in dark-factory `orchestrator/src/orchestrator/scheduler.py` (cite by symbol only; the file is ~7k lines and moves constantly). Editing the `.yaml` (or the `.md`) alone therefore does not change the running gate — the stamped task metadata must also be updated, via `update_task` on the producer task, and you must **resubmit the COMPLETE `delivered_checks` list**: the metadata merge is shallow last-write-wins, so a supplied list overwrites wholesale — a partial write silently drops the task's other checks, and since the descriptor digest hashes the whole list, the shrunken set re-evaluates as a clean cache MISS and reports DELIVERED, with no error and no diff on disk. Read the current list with `get_task` first. Keep the `.yaml` and `.md` manifest twins in sync with each other too: `commit_planning` stamps them together at landing (see "Landing PRD artifacts" above), but a later correction has no such mechanism and must update both by hand. A correction is therefore three writes: `.yaml` descriptor, `.md` twin row, producer task `metadata.delivered_checks` (the complete list).

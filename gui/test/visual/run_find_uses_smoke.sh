...
# Readiness-race fix: design decision 5 (task 4456 step-5).
...
if [ "${REIFY_SMOKE_SKIP_PREBUILD:-0}" != "1" ]; then
    echo "run_find_uses_smoke: pre-building sidecar + gui + reify-gui binary…"
    ...
    echo "run_find_uses_smoke: pre-build complete"
fi
...
GUI_LAUNCHER_PID=$!
echo "run_find_uses_smoke: launcher PID=$GUI_LAUNCHER_PID"
...
echo "run_find_uses_smoke: waiting up to ${WAIT_SEC}s for debug server on port ${PORT}…"
_start=$SECONDS
_server_ready=0
...
if [ "$_server_ready" -ne 1 ]; then
    echo "run_find_uses_smoke: debug server did not become ready within ${WAIT_SEC}s" >&2
    exit 1
fi
echo "run_find_uses_smoke: debug server ready ($(( SECONDS - _start ))s elapsed)"

# ---------------------------------------------------------------------------
# 7. Run the 4202 drive-only smoke driver.
...
# ---------------------------------------------------------------------------
if [ -n "$GUI_LAUNCHER_PID" ]; then
    echo "run_find_uses_smoke: sending SIGTERM to launcher (PID=$GUI_LAUNCHER_PID)"
    kill "$GUI_LAUNCHER_PID" 2>/dev/null || true
    wait "$GUI_LAUNCHER_PID" 2>/dev/null || true
    GUI_LAUNCHER_PID=""
fi

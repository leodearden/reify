...
#   6. Runs `node gui/test/visual/smoke_mesh_count_parity_e2e.mjs` (the parity driver).
...
# Adapted from run_surface_finish_viewport_e2e_smoke.sh (task-4892); only the fixture,
# driver and echo prefix differ.
#
# The launcher-death liveness contract in step 6 is pinned behaviorally by
...
echo "run_mesh_count_parity_e2e_smoke: using debug port $PORT"
...
if [ "${REIFY_SMOKE_SKIP_PREBUILD:-0}" != "1" ]; then
    echo "run_mesh_count_parity_e2e_smoke: pre-building sidecar + gui + reify-gui binary…"
    ...
    echo "run_mesh_count_parity_e2e_smoke: pre-build complete"
fi
...
FIXTURE="$REPO_ROOT/gui/test/fixtures/large_assembly.ri"
echo "run_mesh_count_parity_e2e_smoke: launching GUI with fixture: $FIXTURE"
REIFY_DEBUG=1 REIFY_DEBUG_PORT="$PORT" \
    bash "$LAUNCHER" "$FIXTURE" &
GUI_LAUNCHER_PID=$!
echo "run_mesh_count_parity_e2e_smoke: launcher PID=$GUI_LAUNCHER_PID"
...
echo "run_mesh_count_parity_e2e_smoke: waiting up to ${WAIT_SEC}s for debug server on port ${PORT}…"
_start=$SECONDS
_server_ready=0
while [ $(( SECONDS - _start )) -lt "$WAIT_SEC" ]; do
    ...
    if ! kill -0 "$GUI_LAUNCHER_PID" 2>/dev/null; then
        ...
        GUI_LAUNCHER_PID=""
        echo "run_mesh_count_parity_e2e_smoke: launcher exited early (rc=${_launcher_rc})" >&2
...
if [ "$_server_ready" -ne 1 ]; then
    echo "run_mesh_count_parity_e2e_smoke: debug server did not become ready within ${WAIT_SEC}s" >&2
    exit 1
fi
echo "run_mesh_count_parity_e2e_smoke: debug server ready ($(( SECONDS - _start ))s elapsed)"

# ---------------------------------------------------------------------------
# 7. Run the mesh-count parity smoke driver.
#    The driver's own waitForServer passes instantly (server already ready).
# ---------------------------------------------------------------------------
echo "run_mesh_count_parity_e2e_smoke: running smoke driver…"
DRIVER_RC=0
REIFY_DEBUG_PORT="$PORT" node "$REPO_ROOT/gui/test/visual/smoke_mesh_count_parity_e2e.mjs" || DRIVER_RC=$?

# ---------------------------------------------------------------------------
# 8. Reap the GUI launcher (its trap reaps both vite and reify-gui).
# ---------------------------------------------------------------------------
if [ -n "$GUI_LAUNCHER_PID" ]; then
    echo "run_mesh_count_parity_e2e_smoke: sending SIGTERM to launcher (PID=$GUI_LAUNCHER_PID)"
    kill "$GUI_LAUNCHER_PID" 2>/dev/null || true
    wait "$GUI_LAUNCHER_PID" 2>/dev/null || true
    GUI_LAUNCHER_PID=""
fi

...
async fn on_exit_emits_sidecar_crashed_event_via_emitter() {
    ...
    // Build engine + selection (same pattern as from_parts_with_mcp_intercepts_reify_tool_calls)
    let checker = SimpleConstraintChecker;
    let kernel = MockGeometryKernel::new();
    let session = EngineSession::new(Box::new(checker), Some(Box::new(kernel)));
    let engine = Arc::new(std::sync::Mutex::new(session));
    ...
    // Create an event sink to collect emitted events
    let events: Arc<std::sync::Mutex<Vec<(String, serde_json::Value)>>> =
        Arc::new(std::sync::Mutex::new(vec![]));
    let events_clone = Arc::clone(&events);

    // Create I/O streams — we will drop data_writer to trigger EOF
    let (stdin_writer, _stdin_reader) = tokio::io::duplex(1024);
    let (data_writer, data_reader) = tokio::io::duplex(1024);
    let reader = BufReader::new(data_reader);
    let state = Arc::new(tokio::sync::Mutex::new(SidecarState::Ready));
    ...
    let emitted = events.lock().unwrap();
    assert!(
        emitted.iter().any(|(name, _)| name == "claude-sidecar-crashed"),
        "Expected claude-sidecar-crashed event in sink, got: {:?}",
        emitted.iter().map(|(n, _)| n.as_str()).collect::<Vec<_>>()
    );

    // (b) Its payload must have a non-empty string 'reason' field
    let crashed_payload = emitted
        .iter()
        .find(|(name, _)| name == "claude-sidecar-crashed")
        .map(|(_, p)| p)
        .unwrap();
    assert!(
        crashed_payload["reason"].is_string() && !crashed_payload["reason"].as_str().unwrap().is_empty(),
        "Expected non-empty string 'reason' in claude-sidecar-crashed payload, got: {:?}",
        crashed_payload
    );

    drop(emitted);

    // (c) Handle state must be Crashed
    assert!(
        matches!(*handle.state().lock().await, SidecarState::Crashed(_)),
        "Expected SidecarState::Crashed after EOF"
    );
}

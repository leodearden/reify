...
/// Any spawn error (not just `NotFound` — e.g. `EACCES`/`EAGAIN`) degrades
/// to skip rather than panicking, since a sandboxed or resource-constrained
/// CI image can fail to spawn for reasons other than a missing binary.

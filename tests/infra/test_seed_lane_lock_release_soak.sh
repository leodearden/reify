...
# WHAT IT MEASURES
#
# seed's two detached background jobs are spelled `{ rm -rf ...; } 9<&- &`
# (scripts/seed-warm-lane.sh, the orphan-trash sweep and the reseed-trash rm).
# The `9<&-` redirection is performed by the FORKED CHILD, after the fork — so
# between fork() and the child executing close(9) the child holds a dup of the
# SAME open file description that carries seed's exclusive flock. flock locks
# live on the OFD, not on the descriptor, so the lane lock stays HELD for that
# whole window. If seed's parent exits before the child is scheduled, a probe
# issued immediately after seed exits observes the lock as still held — and
# acquire_lane's default is a NON-BLOCKING `flock -n` (exit 75 EX_TEMPFAIL), so
# the next consumer is spuriously refused. `9<&-` narrows that window; only an
# explicit LOCK_UN (`flock -u 9`) closes it, because LOCK_UN on any descriptor
# referring to the OFD drops the lock for every process holding a dup.
#
# This harness drives N real seed invocations under induced CPU contention and
# counts how many times the lane lock reads back HELD immediately after seed's
# own exit, reporting one structured line:
#
#     SEED_LANE_LOCK_SOAK held=<n> iters=<N> workers=<w>
#
# Measured on a 32-core host with 24 busy-spin workers, against a minimal
# faithful reproduction of seed's tail (200 trials each):
#
#     `9<&-` only (pre-fix seed shape) ............... 16/200 held-after-exit
#     `9<&-` + parent `exec 9>&-` .................... 14/200  (close is NOT enough)
#     `9<&-` + parent `flock -u 9` (the fix) .........  0/200
#
# The middle row is the experimental refutation of the close-only approach: the
# parent closing its OWN descriptor cannot drop a lock the child's dup'd OFD
# still carries.
#
# TWO LAYERS
#
...
#                                     At the measured ~8%/iteration pre-fix rate
#                                     50 iterations detect the race ~98% of the
#                                     time and 200 ~99.99%; use 200 for a
#                                     one-shot RED proof, the default for a
...
# 3s/iteration + 30s slack is comfortably above the observed per-iteration cost
# even on a loaded box; this is a resource budget, never an assertion.
...
while [ "$_i" -lt "$SOAK_ITERS" ]; do
    ...
    # An armed ITERS=200 run under load is minutes long and would otherwise
    # look stalled to dark-factory's stall detector.

...
# dark-factory tasks 3843/4080 re-armed the Tier-2 test on a layout-independent
# resolver, and a fresh -rs run confirms it now runs and fires on injected
# extension-vector drift.  Cycle 10 here still only covers the reify-corpus
# direction; the cross-source direction is covered from γ's side, not by this
# file — see the "Cross-repo seam: γ" header note in scripts/lock-charter-guard.sh
# for the current-state detail, deliberately not restated here.
#

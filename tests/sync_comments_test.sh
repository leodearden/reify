...
# so doc comments and SYNC markers (which may legitimately differ between the two
# copies) do not affect the body comparison.  The signature line itself is
# included — callers that only want the body should pipe through `tail -n +2`.
extract_fn() {
...
# Skip the fn signature line (visibility may differ: pub(crate) vs private).
# Capture first so we can assert non-empty before diffing — an empty result
# means the function was not found (e.g. renamed), and `diff <() <()` would
# silently succeed (false negative).
expr_body=$(extract_fn sanitize_value "$EXPR_FILE" | tail -n +2)
stdlib_body=$(extract_fn sanitize_value "$STDLIB_FILE" | tail -n +2)

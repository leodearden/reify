...
2. **Embedded paths are absolute + fully resolved, but contamination is concentrated and remappable.** A live task binary embeds ~9,340 `~/.rustup` + ~1,150 `~/.cargo/registry` hits (already path-stable across worktrees) vs only **22** worktree-path hits. `--remap-path-prefix=<wt>=<const>` drives the worktree-path hits to **0** (verified). So there are **three path-sensitivity vectors**: (a) embedded debuginfo/panic paths in *output* artifacts — originally "fixed by `--remap-path-prefix`"; **(SUPERSEDED by spike D3)** remap is **unnecessary**: cargo freshness is path-independent (383==383 Fresh units across a rename, identical unit hashes; spike #4641 §4/§6.1) — the remap machinery is dropped; (b) cargo's *internal* `target/.fingerprint/**/dep-*.d` bookkeeping — originally "fixed by a stable path, NOT confirmed fixable by remap alone"; **(SUPERSEDED by spike D3)** same finding: path-independent freshness means vector-(b) is benign — fixed-path-per-lane is kept only for **sccache-hit + landlock-scope** simplicity, **not** cargo correctness; (c) mtime freshness → §10.2.3 (still handled by mtime normalization — unchanged; see D5 in the PRD).
...
- **RESOLVED (D2) — loopback XFS image, not a carved LV.** Spike #4641 validated a **loopback XFS-reflink image** on the existing ext4 `data_lv` (`/var/lib/reify-xfs-spike.img`, `mkfs.xfs -m reflink=1,bigtime=1`, losetup + mount) — no VG surgery, no data-LV shrink, fully reversible (`umount` + `losetup -d` + `rm`). The "carve a new LV from the LVM VG" route is **deferred** — migrate only if loop overhead ever measurably bites (it did **not** in the spike).
- **VG free-extent open question is now moot.** The loopback approach operates on free space inside the existing ext4 LV (6.0 TB free on `data_lv`), not on unallocated VG PEs (`vgroup0`). No `vgs`/`pvs` audit needed; no data-LV shrink risk.

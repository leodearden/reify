...
- **Task 3378 (deferred) → 2924 (pending) chain** — the shells PRD's M-018 (auto-classification dispatch) routes through `solve_elastic_static`, which has no stdlib `fn` declaration. Inherits the FEA PRD's M-001 gap verbatim.

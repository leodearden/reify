...
- **RESOLVED — PRD §Scope.4 "re-route the four already-shipped filtered selectors through feature-tag resolution."** The `.ri` call path through `try_eval_topology_selector` now routes the 11 task-2699 names to real dispatch; `*_with_tags` variants are exercised end-to-end by `kernel_queries_integration.rs`. See M-007 (now WIRED).
...
- **State:** WIRED
- **Failure mode:** N/A
- **Evidence:** `crates/reify-eval/src/topology_selectors.rs:188-710` (4 base selectors + 4 `*_with_tags` variants); integration tests in `crates/reify-eval/tests/topology_filtered_selectors.rs` (229 lines, OCCT-gated); coverage in `crates/reify-eval/tests/feature_tag_selector_tests.rs` (573 lines, mock-kernel-driven, three of the four `*_with_tags` variants pinned). Eval-side `.ri` call path now routes through `try_eval_topology_selector` dispatch arms (M-003 landed, KGQ Phases 2–5), exercised end-to-end by `kernel_queries_integration.rs`.
- **Blocks:** none
- **Note:** Per PRD §Background these were originally shipped under #318. The re-exposure under feature-tag naming is now fully exercised: `*_with_tags` variants are reached from `.ri` source via the wired `try_eval_topology_selector` dispatch chain (`docs/prds/v0_3/kernel-geometry-queries.md`).

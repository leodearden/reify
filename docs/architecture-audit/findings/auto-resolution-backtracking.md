...
- **The DFS library is wired, but the v0.1 BFS path it falls back to is itself an unreachable library API.** The parser does not accept `auto:` in `type_arg_list` (`tree-sitter-reify/grammar.js:601-610` admits only `type_expr | number_literal`), and no compile-pipeline call-site of `resolve_auto_type_params` *or* `resolve_auto_type_params_with_backtracking` exists outside tests. The PRD's first stated pre-condition ("v0.1 alpha has shipped with per-parameter BFS") is **superficially true at the library level and false at the user-facing level** — every PRD claim from the user's perspective is currently dead code.

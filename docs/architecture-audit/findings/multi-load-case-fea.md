...
- The PRD's design-loop demo (`param thickness : Length = auto` and `minimize mass(...) subject to max(envelope_von_mises(results)) < material.yield_stress`) uses `auto` and `subject to` surface that is not in the grammar. `minimize` exists with a `where_clause`, but `subject to` and the `auto` literal are absent from `crates/reify-syntax/`. The example file `examples/m6/multi_load_bracket.ri` does not exist.
...
- **Failure mode:** F1 (entire example absent) + F4 (grammar/auto literal drift)
- **Evidence:** File `examples/m6/multi_load_bracket.ri` does not exist. Directory `examples/m6/` does not exist. Task 3018 status `deferred`. The PRD-cited grammar `param thickness : Length = auto` requires an `auto` literal — zero hits for `= auto\b|kAuto|Auto::` in `crates/reify-syntax/`. The `subject to` clause does not exist in the grammar — `minimize` accepts a `where_clause` per `crates/reify-syntax/src/ts_parser.rs:1624-1636`. PRD also references `BodyForce`/`Gravity` load types (5g acceleration case in task 3018 detail) which have no stdlib presence.
- **Blocks:** the design-loop story of the entire PRD (Goal section claim "should be a one-liner")
- **Note:** Two distinct gaps bundled: (a) the example file itself awaits 3005 + 3007 + 2929; (b) the *language surface* the PRD writes (`= auto`, `subject to`, `BodyForce`) is fictional and not tracked in the PRD or its decomposition. This is the single biggest scope surprise of this audit.
...
- **`= auto` literal / `subject to` clause / `BodyForce` / `Gravity` (M-015 sub-gaps)** — language-surface fictions referenced by this PRD that are not tracked in any decomposition task here. Phase 3 may want to surface these as separate gaps with their own provenance.

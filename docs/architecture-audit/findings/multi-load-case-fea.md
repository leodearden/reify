...
- The PRD's design-loop demo (`param thickness : Length = auto` and `minimize mass(...) subject to max(envelope_von_mises(results)) < material.yield_stress`) mixes grammar-supported and fictional surfaces: `param thickness : Length = auto` is grammar-supported at the param-default position (via `auto_keyword`); `subject to` is fictional. **CORRECTED 2026-08-25:** this finding went on to say "`minimize` accepts a `where_clause` per `crates/reify-syntax/src/ts_parser.rs:1624-1636`" — true of the PARSER, but not a capability. The compiler's Minimize/Maximize arms never read `MinimizeDecl.where_clause`; the guard is silently discarded and the objective runs unopposed. Ruled reject (owner task **6575**). The honest form is a bare `minimize` plus a separate `constraint` member. The example file `examples/m6/multi_load_bracket.ri` does not exist. *(2026-05-27 update: broader `auto` binding-site coverage beyond param-default — sub-overrides, named-args, let, connect-param — is being addressed by `docs/prds/auto-binding-site-positions.md`, α task 3802 landed.)*
...
- **Evidence:** File `examples/m6/multi_load_bracket.ri` does not exist. Directory `examples/m6/` does not exist. Task 3018 status `deferred`. The `subject to` clause does not exist in the grammar. **CORRECTED 2026-08-25:** the original evidence line here read "`minimize` accepts a `where_clause` per `crates/reify-syntax/src/ts_parser.rs:1624-1636`" — a parser fact that was read as a capability. The compiler drops that guard silently (task **6575**, ruled reject), so `minimize ... where ...` is NOT the remedy for `subject to`; a separate `constraint` member is. PRD also references `BodyForce`/`Gravity` load types (5g acceleration case in task 3018 detail) which have no stdlib presence. *(2026-05-27 update: `param thickness : Length = auto` is grammar-supported at the param-default position via `auto_keyword`; this sub-mechanism is no longer a grammar fiction. The fictional sub-mechanisms here are `subject to` and `BodyForce`/`Gravity`. Broader `auto` binding-site coverage is in `docs/prds/auto-binding-site-positions.md`, α task 3802 landed.)*

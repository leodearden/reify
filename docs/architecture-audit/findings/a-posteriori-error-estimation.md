...
- **Note:** Two encoding hazards inherited from existing stdlib (already documented at lines 17-35): `Field<X,Y>` in `param` position not yet resolved (TODO(field-in-param), task #3117), and `Number = auto` not in grammar. PRD's `target_accuracy : Number = 0.05` is encodable; `error_indicator: Field<Element, ScalarPressure>` is NOT today.

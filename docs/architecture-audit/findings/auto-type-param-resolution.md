...
- **Evidence:** `tree-sitter-reify/grammar.js:606-610` (`type_arg_list`) accepts only `choice($.type_expr, $.number_literal)`. `auto_keyword` (lines 430-433) covers the `auto` / `auto(free)` lexeme but is only used as a *value default* (line 421), not as a type-arg. Module doc-comment at `auto_type_param.rs:100-106` explicitly calls this out: "the parser does not yet accept `auto: TraitName` syntax inside `type_arg_list`". Fixture `examples/bearing_auto_seal.ri:20-25` and the determinism / topology-trigger test headers all document this gap.

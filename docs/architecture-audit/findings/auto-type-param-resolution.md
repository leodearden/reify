...
- **The end-user surface is still fiction at the semantic level, even though the parser has caught up.** Library-level Phase A/B/C/orchestrator code is well-tested and shipped, and the PRD's *user-visible* surface — the source-level `Bearing<auto: Seal>` syntax — now **parses** since commit `a46e7d3888` added `auto_type_arg` at `grammar.js:710-729`. But `resolve_auto_type_params` is still not invoked from any production code path in `crates/reify-compiler/src/lib.rs::compile_*`. `CompiledModule.auto_type_substitution` is initialised to `AutoTypeSubstitution::default()` in both `compile_builder/ctx.rs:188` and `compile_builder/defs_phase.rs:296` and is never written by any non-test caller. The producer side of the substitution pipeline is dark; the consumer (`Snapshot::from_compiled_module` at `crates/reify-eval/src/snapshot.rs:54`) just sees an empty vec. This is the same gap M-002/M-014 in the sibling v0.2 audit (`auto-resolution-backtracking.md`) — and the v0.1 PRD is its *origin*, not the v0.2 follow-up.
...
- **Failure mode:** F1 → reclassified to F4 (parser surface lands but no compile-pipeline call site invokes the resolver)
- **Evidence:** `tree-sitter-reify/grammar.js:710-714` defines `type_arg_list` as `choice($.type_expr, $.number_literal, $.auto_type_arg)`; the `auto_type_arg` rule at `grammar.js:725-729` admits `auto: <bound>` and `auto(free): <bound>` via the `auto_keyword` modifier (defined at ~`grammar.js:459`). Corpus tests at `tree-sitter-reify/test/corpus/auto_type_arg.txt` cover both forms; landed in commit `a46e7d3888`. The remaining gap is semantic: no caller in the compile pipeline invokes `resolve_auto_type_params` against parsed `auto_type_arg` nodes, so source-level `Bearing<auto: Seal>` parses cleanly but produces no substitution at compile time. The doc-comment at `auto_type_param.rs:100-106` (which still asserts the parser doesn't accept `auto: TraitName` and cites the obsolete `grammar.js:601-605`) and the test headers in `examples/bearing_auto_seal.ri:20-25` / determinism / topology-trigger fixtures are themselves stale; follow-up task 3896 filed to refresh those citations.

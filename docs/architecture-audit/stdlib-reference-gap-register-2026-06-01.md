...
> **STATUS 2026-07 — 12 of 16 rows RESOLVED by geometry-transforms-frames-projection
> PRD (α–ι, #4164–#4172); 3 rows STALE; 1 row DEFERRED.** Owned by
> `docs/prds/geometry-transforms-frames-projection.md`. RESOLVED-BY-PRD:
> **α #4164** apply_transform (`reify-compiler/src/geometry_transform.rs:144`),
> **β #4165** project (`reify-stdlib/src/geometry.rs:957`), **γ #4166** rotate(Orientation<3>)
> (`geometry_transform.rs:55-67` + `reify-eval/src/geometry_ops.rs:2041-2057`),
> **δ #4167** scale(Vector3<Real>) (`geometry_transform.rs:90-100` + `geometry_ops.rs:2245-2306`),
> **ε #4168** arbitrary_pattern honouring full Transform<3> (`geometry_ops.rs:2613-2629` +
> `reify-kernel-occt/src/lib.rs:3241-3261` — also resolves task **323**'s ⚠️PHANTOM
> partial, see Executive summary), **ζ #4169** orient_look_at (`reify-stdlib/src/orientation.rs:170`)
> + the EulerConvention enum-value path (`orientation.rs:56`/`274`), **θ #4171**
> Geometry/Transformable supertraits + is_closed/is_connected/is_bounded
> (`reify-compiler/src/units.rs:104-107` + `geometry_ops.rs:3550-3552`/`3598-3600`).
> **η #4170** (mirror(Plane)/circular_pattern(Axis)) lands in the same PRD but has no
> corresponding row here. **ι #4172** (this task) reconciles LSP completions,
> `docs/reify-stdlib-reference.md`, and this register. RESOLVED (premise corrected) —
> **not** fixed by this PRD, just mis-surveyed: rows **L61** (plane/axis constructors),
> **L67** (`Plane`), **L68** (`Axis`) — the six constructors already existed at survey
> time (`reify-stdlib/src/geometry.rs:818-825`); the "missing/unconstructible" claim
> was false (matching stale label corrected in `docs/reify-stdlib-reference.md` by ι
> #4172). DEFERRED: **L72** (`Convex`/`is_convex`) — no runtime convexity predicate
> ships in this PRD (`docs/reify-stdlib-reference.md` §3.10 deferral note).

| sev | symbol / gap | doc | type | tracked | evidence-summary |
|-----|--------------|-----|------|---------|------------------|
| high | project() (both overloads) documented but unregistered | §3.1 geom_ctor L239-240 (docs/reify-stdlib-reference.md:239-240) | missing_decl | untracked #3535(done,0.39); #3902(done,0.39); #2583(done,0.39) | RESOLVED-BY-PRD — LANDED on main by β #4165 (`reify-stdlib/src/geometry.rs:957`: `"project" => { ... }`, decoding both the Point and Vector overloads). |
| high | apply_transform(geometry, transform: Transform<3>) not registered as a stdlib fn | §3.7 transform L406 | missing_decl | tracked_partial #3963(pending,0.54); #3901(done,0.51) | RESOLVED-BY-PRD — LANDED on main by α #4164 (`reify-compiler/src/geometry_transform.rs:144`: `"apply_transform" => { ... }` builds `TransformKind::ApplyTransform`). |
| high | plane_xy/plane_xz/plane_yz/axis_x/axis_y/axis_z constructors not implemented | §3.10 fns L527-532 | missing_decl | untracked #3116(deferred,0.46); #3230(done,0.39) | RESOLVED-STALE (premise corrected) — the six constructors already existed at survey time (`reify-stdlib/src/geometry.rs:818-825`: `plane_xy`/`plane_xz`/`plane_yz` → `make_plane`, `axis_x`/`axis_y`/`axis_z` → `make_axis`); the "entirely missing" claim was false. Not part of the geometry-transforms-frames-projection PRD's remediation. `docs/reify-stdlib-reference.md`'s matching stale "not yet implemented" label corrected by ι #4172. |
| medium | orient_look_at documented but never implemented | §3.1 geom_ctor L231 (docs/reify-stdlib-reference.md:231) | missing_decl | untracked #2956(done,0.47); #250(deferred,0.42); #2583(done,0.36) | RESOLVED-BY-PRD — LANDED on main by ζ #4169 (`reify-stdlib/src/orientation.rs:170`: `"orient_look_at" => { ... }`, Gram-Schmidt basis build delegating to `orient_basis`). |
| medium | EulerConvention enum absent; only lowercase strings accepted, uppercase variants rejected | §3.1 geom_ctor L229,242,260 (docs/reify-stdlib-reference.md:242) | missing_decl | untracked #3942(pending,0.40); #4108(done,0.39); #3936(done,0.38) | RESOLVED-BY-PRD — LANDED on main by ζ #4169: the qualified-enum-value path (`reify-stdlib/src/orientation.rs:56` in `orient_euler`, `:274` in `orient_to_euler`: `Value::Enum { type_name, variant, .. } if type_name == "EulerConvention"`). A **bare** unqualified variant (`XYZ` alone, without the `EulerConvention.` qualifier) remains unsupported — documented as a caveat by ι #4172. |
| medium | rotate(geometry, orientation: Orientation<3>) overload not implemented | §3.7 transform L402 | missing_decl | untracked #3230(done,0.41); #1639(done,0.48) | RESOLVED-BY-PRD — LANDED on main by γ #4166 (`reify-compiler/src/geometry_transform.rs:55-67` dispatches `TransformKind::Rotate`; `reify-eval/src/geometry_ops.rs:2041-2057` `transform_rotate` decodes the orientation via `decode_orientation_to_axis_angle`). |
| medium | scale(geometry, factors: Vector3<Real>) per-axis overload not implemented | §3.7 transform L405 | missing_decl | tracked_partial #3959(done,0.49); #3960(pending,0.42); #3963(pending,0.40) | RESOLVED-BY-PRD — LANDED on main by δ #4167 (`reify-compiler/src/geometry_transform.rs:90-100` routes `Vector3` args to `TransformKind::ScaleNonUniform`; `reify-eval/src/geometry_ops.rs:2245-2306` `transform_scale_non_uniform`). |
| medium | ⚠️PHANTOM arbitrary_pattern accepts translation triples, not full Transform<3> | §3.8 pattern L418 | partial | tracked_done #323(done,0.49) | RESOLVED-BY-PRD — LANDED on main by ε #4168: `arbitrary_pattern` now decodes a full `Transform<3>` per instance (`reify-eval/src/geometry_ops.rs:2613-2629` `decompose_transform_to_arrays`; `reify-kernel-occt/src/lib.rs:3241-3261`, stride-7 `[qw,qx,qy,qz,tx,ty,tz]` per-instance rigid transform — an identity quaternion reproduces the pre-4168 translation-only behavior for back-compat). Resolves task 323's ⚠️PHANTOM-partial flag (see Executive summary); task 323's phantom_note refreshed by ι #4172. |
| medium | Plane structure not declared in .ri and not constructible from source | §3.10 structures L510-513 | declared_only_eval_undef | untracked #3116(deferred,0.51) | RESOLVED-STALE (premise corrected) — `Plane` is constructible from source via `plane_xy`/`plane_xz`/`plane_yz` (`reify-stdlib/src/geometry.rs:818-820`), which already existed at survey time; the "unconstructible" claim was false. See L61. |
| medium | Axis structure not declared in .ri and not constructible from source | §3.10 structures L515-518 | declared_only_eval_undef | untracked #3149(done,0.40); #3230(done,0.39) | RESOLVED-STALE (premise corrected) — `Axis` is constructible from source via `axis_x`/`axis_y`/`axis_z` (`reify-stdlib/src/geometry.rs:823-825`), which already existed at survey time; the "unconstructible" claim was false. See L61. |
| low | trait Geometry never defined | §3.10 traits L500 | missing_decl | untracked #2297(done,0.58); #2312(done,0.61) | RESOLVED-BY-PRD — LANDED on main by θ #4171: the `Geometry` supertrait is now declared (see `docs/reify-stdlib-reference.md` §3.10). |
| low | trait Transformable never defined | §3.10 traits L501 | missing_decl | untracked #2297(done,0.50); #2312(done,0.49) | RESOLVED-BY-PRD — LANDED on main by θ #4171: the `Transformable` supertrait is now declared (see `docs/reify-stdlib-reference.md` §3.10). |
| low | trait Closed is a marker with no conformance query | §3.10 traits L502 | declared_only_eval_undef | tracked_partial #2297(done,0.57); #2320(done,0.50); #2318(done,0.50) | RESOLVED-BY-PRD — LANDED on main by θ #4171: `is_closed` conformance query (`reify-compiler/src/units.rs:104-107` `GEOMETRY_QUERY_HELPER_NAMES`, "Task 4171 (θ)"; `reify-eval/src/geometry_ops.rs:3550-3552` marker short-circuit + `:3598-3600` kernel-query dispatch). |
| low | trait Convex is a marker with no conformance query | §3.10 traits L505 | declared_only_eval_undef | tracked_partial #2318(done,0.54); #2315(done,0.54); #2312(done,0.52) | DEFERRED — `is_convex` is intentionally absent from the geometry-transforms-frames-projection PRD: convexity requires a separate convex-hull-comparison predicate with its own accuracy bound (`docs/reify-stdlib-reference.md` §3.10 deferral note, confirmed by ι #4172). `Convex` remains a name-only marker trait. |
| low | trait Connected is a marker with no conformance query | §3.10 traits L506 | declared_only_eval_undef | tracked_partial #2550(done,0.50); #2312(done,0.47); #2318(done,0.45) | RESOLVED-BY-PRD — LANDED on main by θ #4171: `is_connected` conformance query (`reify-compiler/src/units.rs:104-107` `GEOMETRY_QUERY_HELPER_NAMES`; `reify-eval/src/geometry_ops.rs:3550-3552` marker short-circuit + `:3598-3600` kernel-query dispatch). |
| low | trait Bounded is a marker with no conformance query | §3.10 traits L507 | declared_only_eval_undef | tracked_partial #2312(done,0.52); #2315(done,0.51); #2318(done,0.49) | RESOLVED-BY-PRD — LANDED on main by θ #4171: `is_bounded` conformance query (`reify-compiler/src/units.rs:104-107` `GEOMETRY_QUERY_HELPER_NAMES`; `reify-eval/src/geometry_ops.rs:3550-3552` marker short-circuit + `:3598-3600` kernel-query dispatch). |
...
| sev | symbol / gap | doc | type | tracked | evidence-summary |
|-----|--------------|-----|------|---------|------------------|
| high | Process-category trait params documented but absent from .ri (empty bodies) | §8 std.process process.ri L956-985 | missing_decl | tracked_done #4273(done) | RESOLVED — LANDED on main by β #4273: all 7 category traits (Subtracting/Adding/Forming/Joining/Parting/SurfaceTreating/HeatTreating) populated with capability params (tool_access:Solid, min_feature_size, build_volume:Solid, etc.). The DFMRule surface (rule_name/severity/applies_to + subject:Solid) and the §8 DFMRule/category prose were reconciled by the process-dfm-geometry-metrology PRD; δ #4275 added the §8 DFM constraint-def surface (Manufacturable/FeatureManufacturable/Bend/Draw/Draft/FitsBuildVolume). |
| medium | std.process is trait-surface-only — no DFM/process evaluation engine | §8 std.process process.ri L944-996 | declared_only_eval_undef | tracked_done #4272(done); #4274(done); #4275(done) | RESOLVED — α #4272 (fits_build_volume builtin + dfm::diagnose severity bridge, crates/reify-stdlib/src/dfm.rs) + γ #4274 (Manufacturable/FeatureManufacturable/BendManufacturable/DrawManufacturable/DraftManufacturable/FitsBuildVolume constraint defs in process.ri) ship the declarative DFM evaluation engine. δ #4275 adds the CI example (examples/process/std_process_dfm.ri) + e2e test asserting the scalar DFM Satisfied/Violated set and compile-cleanliness. The geometry-backed OK→VIOLATED build-volume flip + I/W/E_DFM_BUILD_VOLUME diagnostics need the UnifiedDag post-geometry constraint re-check (#4358, default-off) and are deferred to the build-DAG cutover (#4362; canonical flip proof owned by #4360). |
| low | All documented '= undef' defaults dropped from io/process .ri | §8-9 io.ri/process.ri L948-1077 | partial | tracked_done #3918(done); #4275(done) [§8 doc] | RESOLVED (premise corrected) for the §8/process.ri half — `undef` is a first-class literal since #3918, so the "grammar lacks undef → defaults dropped" premise is false. The §8 doc (reconciled by the metrology PRD; constraint-def surface added by δ #4275) shows the real `= undef` defaults accurately (Process.duration/cost, SurfaceTreating.coating_thickness). The §9/io.ri half is not in scope for this task and remains under P15. |
...
> **STATUS 2026-06-17 — all 7 rows RESOLVED by std.fields β/γ/δ/ε/ζ/η tasks.**
> fn_field eval arm #4220 (β); from_samples + InterpolationMethod enum #4221 (γ);
> restrict + ContainmentQuery seam #4222 (δ); constant_field / clamp_field /
> remap_field / threshold .ri fns #4223 (ε); compose .ri fn #4224 (ζ);
> full-surface integration gate + doc/gap-register reconcile #4225 (η).

| sev | symbol / gap | doc | type | tracked | evidence-summary |
|-----|--------------|-----|------|---------|------------------|
| high | constant_field is not implemented or declared | §11 std.fields.interpolation L1121 | missing_decl | untracked #4025(done,0.41); #3780(done,0.40) | RESOLVED — LANDED on main by ε #4223 (`crates/reify-compiler/stdlib/fields.ri`: `pub fn constant_field<D,C>(value: C) -> C { value }`; eval via reify-stdlib). Integration gate #4225. |
| high | fn_field is not implemented or declared | §11 std.fields.interpolation L1122 | missing_decl | untracked #3071(done,0.41); #1002(done,0.40); #2336(done,0.37) | RESOLVED — LANDED on main by β #4220 (eval arm `eval_fn_field` in `reify-expr/src/lib.rs`; intercepting builtin wraps lambda as `Field::Analytical`). Integration gate #4225. |
| high | from_samples is not implemented or declared | §11 std.fields.interpolation L1123 | missing_decl | untracked #2338(done,0.57); #2341(done,0.49); #2666(done,0.44) | RESOLVED — LANDED on main by γ #4221 (eval arm `eval_from_samples` in `reify-expr/src/lib.rs`; `InterpolationMethod` enum in `fields.ri`; `DiagnosticCode::FieldSamplesNotGrid` + `InterpMethodUnsupported`). Integration gate #4225. |
| medium | restrict is not implemented | §11 std.fields.spatial L1131 | missing_decl | untracked #2414(done,0.32) | RESOLVED — LANDED on main by δ #4222 (eval arm `eval_restrict_field` + `ContainmentQuery` seam in `reify-expr/src/lib.rs` + `reify-eval`; B5 restrict integration gate via `fields_restrict_e2e.rs`). Integration gate #4225. |
| medium | clamp_field is not implemented or declared | §11 std.fields.spatial L1132 | missing_decl | untracked #2414(done,0.48); #3006(done,0.41) | RESOLVED — LANDED on main by ε #4223 (`crates/reify-compiler/stdlib/fields.ri`: `pub fn clamp_field<D,Q>(...)` + eval via reify-stdlib). Integration gate #4225. |
| medium | remap_field is not implemented or declared | §11 std.fields.spatial L1133 | missing_decl | untracked #2414(done,0.41); #3006(done,0.38) | RESOLVED — LANDED on main by ε #4223 (`crates/reify-compiler/stdlib/fields.ri`: `pub fn remap_field<D,Q>(...)` + eval via reify-stdlib). Integration gate #4225. |
| medium | threshold is not implemented or declared | §11 std.fields.spatial L1134 | missing_decl | untracked #2314(done,0.45) | RESOLVED — LANDED on main by ε #4223 (`crates/reify-compiler/stdlib/fields.ri`: `pub fn threshold<D,Q>(...)` returning `Field<D,Bool>` + eval via reify-stdlib). Integration gate #4225. |
...
> - **Owned by sibling implement-PRD doc tasks (pending; no new task):** §4 Flexible→**#4227** (structural-traits β); §6 Material→MaterialSpec / Elastic-Strong-Hard-Ductile free-standing / Insulating `determined()`→**#4243** (materials ε); §8 DFMRule params→**#4275** (process-DFM δ); §11 InterpolationMethod (internal Rust enum→Reify enum) + `compose` callable — **RECONCILED by #4225** (fields η; `InterpolationMethod { Linear, NearestNeighbor, Cubic, RBF, Kriging }` in `fields.ri` via γ #4221; callable `compose<A,B,C>(f:Field<B,C>,g:Field<A,B>)` in `fields.ri` via ζ #4224; doc §11 updated); §13 E_KINEMATIC_CLOSED_CHAIN dead + joint/snapshot type-tags→Map→**#4313** (mechanism θ).
> - **§5 std.ports (6 rows) — reconciled by task #4314.** P11 ports-breadth landed (batch #4254/#4255/#4256/#4257/#4259/#4260); doc leaf #4314 aligned §5 member-for-member (Frame3, SignalKind/PWM retained, scalar FluidPort+medium+VolumetricFlowRate+FittingStandard, Bore/Shaft/StructurePort bare markers, heat_flow through-variable+HeatFlux/ThermalResistance aliases+ThermalContactPort). All 6 rows closed.
...
| §8-9 process/io | DFMRule documented params (subject, process) absent; .ri has different params | **RECONCILED** (done) — §8 doc now shows the shipped DFMRule surface: `rule_name : String`, `severity : DFMSeverity`, `applies_to : Process`, `subject : Solid`, plus `enum DFMSeverity { Info, Warning, Error }`. The DFMRule/DFMSeverity surface + the auto-measurement (subject) prose were reconciled by the process-dfm-geometry-metrology PRD; δ #4275 added the §8 DFM constraint-def surface (Manufacturable/FeatureManufacturable/Bend/Draw/Draft/FitsBuildVolume). No doc-vs-code mismatch remains for the §8 surface. |
| §10-11 analysis/fields | InterpolationMethod is an internal Rust enum, not a Reify-language enum | The only match (task 2338, done) deliberately built InterpolationMethod as an INTERNAL Rust sampler module — it never aimed to expose a Reify-language enum, so the gap (no language-level enum) is unaddressed despite the done status; effectively untracked as a language-surface feature. **RECONCILED — task #4225** (fields η): `pub enum InterpolationMethod { Linear, NearestNeighbor, Cubic, RBF, Kriging }` shipped in `crates/reify-compiler/stdlib/fields.ri` by γ #4221; doc §11 updated by #4225 to match. |
| §10-11 analysis/fields | compose exists only as composed{} syntax, not as the documented callable fn | Task 2343 (done) wired the composed field source kind, but only the composed{} block syntax — no task adds a callable compose(f,g) function, so the documented callable form remains a gap despite the done match. **RECONCILED — task #4225** (fields η): callable `pub fn compose<A,B,C>(f: Field<B,C>, g: Field<A,B>) -> Field<A,C>` shipped in `fields.ri` by ζ #4224; doc §11 arg order corrected by #4225 (`compose(f,g)(p)=f(g(p))`). |

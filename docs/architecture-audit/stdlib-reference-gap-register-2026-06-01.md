...
| sev | symbol / gap | doc | type | tracked | evidence-summary |
|-----|--------------|-----|------|---------|------------------|
| high | Process-category trait params documented but absent from .ri (empty bodies) | §8 std.process process.ri L956-985 | missing_decl | tracked_done #4273(done) | RESOLVED — LANDED on main by β #4273: all 7 category traits (Subtracting/Adding/Forming/Joining/Parting/SurfaceTreating/HeatTreating) populated with capability params (tool_access:Solid, min_feature_size, build_volume:Solid, etc.). DFMRule params (subject/process → rule_name/severity/applies_to) landed by the initial process-stdlib task (pre-β). §8 doc reconciled by δ #4275. |
| medium | std.process is trait-surface-only — no DFM/process evaluation engine | §8 std.process process.ri L944-996 | declared_only_eval_undef | tracked_done #4272(done); #4274(done) | RESOLVED — α #4272 (fits_build_volume builtin + dfm::diagnose severity bridge, crates/reify-stdlib/src/dfm.rs) + γ #4274 (Manufacturable/FeatureManufacturable/BendManufacturable/DrawManufacturable/DraftManufacturable/FitsBuildVolume constraint defs in process.ri) ship the declarative DFM evaluation engine. Runtime OK→VIOLATED flip + I/W/E_DFM severity diagnostics exercised by δ #4275 e2e test. |
| low | All documented '= undef' defaults dropped from io/process .ri | §8-9 io.ri/process.ri L948-1077 | partial | tracked_done #4275(done) [§8 half]; tracked_open [§9 half remains under P15] | CLOSED for §8/process.ri half — δ #4275 §8 doc reconcile drops '= undef' from Process.duration/cost and SurfaceTreating.coating_thickness; annotates all category params as required (no defaults). The §9/io.ri '= undef' defaults are not in scope for this task and remain under P15. |
...
| §8-9 process/io | DFMRule documented params (subject, process) absent; .ri has different params | **RECONCILED by δ #4275** (done) — §8 doc now shows the shipped params: `rule_name : String`, `severity : DFMSeverity`, `applies_to : Process`, plus `enum DFMSeverity { Info, Warning, Error }` and the full DFM constraint-def surface (Manufacturable/FeatureManufacturable/Bend/Draw/Draft/FitsBuildVolume). No doc-vs-code mismatch remains for the §8 surface. |

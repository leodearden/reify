...
- **Grammar-level fictions** (multi-load-case-fea `= auto` / `subject to`, varying-thickness-shells `@shell(thickness = linear_taper(...))`, imported-field-source-hdf5-csv `schema = { x: Length(mm), ... }`, specialization-scope `sub name : Type { body }`, match-block-decls decl-level `match`, deep-dot-chain method-call AST). PRDs assume grammar shapes that don't exist. Worth a Phase-3 category distinct from runtime FICTION.

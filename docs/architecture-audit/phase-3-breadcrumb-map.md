...
| structural-analysis-shells | auto-resolution-backtracking | M-026 (`param thickness : Length = auto`) — residual type-arg-position `auto:` work (semantic / compile-pipeline wiring) owned there; value-position `= auto` at param-default is grammar-supported and now covered by `docs/prds/auto-binding-site-positions.md` | ownership split: value-position → auto-binding-site-positions; type-arg-position → auto-resolution-backtracking. Both grammars have landed (param-default via task 3802; type-arg via commit a46e7d3888); the remaining B1 chain is integration, not parser work. |

...
| `DF_VERIFY_ROLE` ∈ {task, merge} reliably stamped by the orchestrator on the merge path | ✓ (`verify.sh` already branches on it 4 ways; `dark-factory-orchestrator.yaml:30,34-35`) |
...
- **C3 — Role→FIFO selection (single source).** Exactly one place sets `CARGO_MAKEFLAGS`: `verify.sh apply_env()`. `DF_VERIFY_ROLE=merge` → merge FIFO; `task` → task FIFO. Per-role `-p` guard: if the role's FIFO is absent, leave `CARGO_MAKEFLAGS` unset → cargo's private pool (graceful degradation, exactly as today). `dark-factory-orchestrator.yaml verify_env` **must not** also inject `CARGO_MAKEFLAGS` (resolves the never-unset asymmetry — see §7).
...
- **D — `CARGO_MAKEFLAGS` ownership moves wholly into `verify.sh`.** Removed from `dark-factory-orchestrator.yaml verify_env`. Both files live in the reify repo, so this is a reify-only change (no dark-factory coordination).
...
- **Raising `max_concurrent_tasks`** itself — this PRD *unblocks* it (surface #3) but the cap change is a separate one-line `dark-factory-orchestrator.yaml` decision Leo makes after the acceptance gate (η) is green.
...
| `CARGO_MAKEFLAGS` injection (verify_env vs apply_env) | **this PRD (reify)** | Conflict resolved by removing it from `dark-factory-orchestrator.yaml verify_env`; `verify.sh apply_env` becomes the single source (C3/§5.D). Both files are reify-owned. |
...
- **δ — verify.sh role→FIFO selection + ownership move.** `apply_env` selects merge/task FIFO by `DF_VERIFY_ROLE` with per-role `-p` fallback; delete `CARGO_MAKEFLAGS` from `dark-factory-orchestrator.yaml verify_env`. *Signal:* **`verify.sh --print-plan` env line shows role-appropriate FIFO path (merge→merge FIFO, task→task FIFO); `dark-factory-orchestrator.yaml` no longer injects `CARGO_MAKEFLAGS`** (oracle test).
...
- **η — End-to-end mixed-load acceptance gate (CRITICAL).** Real merge verify concurrent with N task verifies, run under the **standing** verify.sh budgets (debug 60m / release 75m — battle-tested; not ε's synthetic constants). *Signal (criteria (a)/(d) re-specified 2026-06-11, Leo-ratified — see the acceptance report's oracle-provenance section):* **(a) the partition strands no cores — dual-pool busy-core fraction within 0.03 of the single-pool baseline, same regime (the original absolute ≥0.85 floor measured workload shape, not balancer behaviour); (b) merge wall-clock improved vs single-pool baseline; (c) no task verify spuriously exits 124; (d) contention ratchet observed — merge pool FIONREAD strictly above its seeded baseline partition under contention (the original 'merge reaches full token allocation / FIONREAD → nproc' conflated allocation with availability: FIONREAD counts free tokens, and a hungry merge holds tokens exactly when fully allocated, so free==nproc is structurally fleeting).** The user-observable proof + the C-as-integration-gate. The committed acceptance report additionally records the merge wall-clock and the slowest task verify wall-clock with sccache cache state per run — the authoritative floor ζ consumes. (dep α, β, γ, δ — ζ moved downstream of η, 2026-06-11)

...
**C2 profile-axis carve-out (task 5279 / PRD `docs/prds/merge-gate-riders.md` task ε, rider 3; SWEEP-GATED, default-OFF).** C2 forbids narrowing the merge gate's **crate-set** axis, and that prohibition is untouched. A separate, orthogonal carve-out applies to the **build-profile** axis only: when `REIFY_RELEASE_DELTA_SKIP=1` **and** `DF_VERIFY_ROLE=merge` **and** the merge delta's reverse-closed affected crate set (§3 C3) ∩ the declared release-sensitive set (`scripts/release-sensitive-crates.txt`) is **empty** ("delta-clean"), `verify.sh` omits the ~17-min release **nextest** pass and emits the frozen plan marker `RELEASE-PASS: skipped (delta-clean)` in its place (predicate: `scripts/release-scope-lib.sh:release_delta_requires_pass`). The DEBUG full-workspace pass and the unconditional `reify-cli`/`reify-audit` release **prebuilds** are untouched, and the C1/C2 crate-set (branch-diff) narrowing is untouched — only the release **test** re-execution for a delta-clean merge is deferred, from merge-time to the **main-tip background sweep** (`DF_VERIFY_ROLE=background`, `--profile both`, landed with task 5210), which re-runs the full release-sensitive set in release on cadence. **This deferral is conditional, not absolute:** a release-only breakage introduced by a delta-clean merge is caught by the sweep at **main-tip cadence (hours), not at the gate**, and only while that sweep stays green-on-cadence and alerts on failure — if the sweep is unhealthy the safety argument lapses. Every widening path stays at RUN by the fail-open/fail-wide ladder (§5.1): an underivable delta, the `ALL` sentinel (C4/C5), and an unreadable declared set all resolve to REQUIRED; `DF_VERIFY_ROLE=background` never skips (the sweep is the backstop). The knob **defaults OFF** — the emitted plan is byte-identical, so `tests/infra/test_occt_flock_gate.sh` Tests 17/17b are unaffected — and activation is itself **sweep-gated**: leaf ζ (task 5280) flips `REIFY_RELEASE_DELTA_SKIP=1` in `dark-factory-orchestrator.yaml` only once the background sweep is demonstrably healthy (merge-gate-riders §5.3). This task ships production-inert.
...
**Inert paths are ONE list (task 7427).** The documentation/configuration class — `docs/**`, `*.md`, `*.yaml`, `*.yml` — is defined once, by `scripts/affected-crates-lib.sh:_is_inert`, and consulted by **both** sides of the scope decision: `_is_noncrate` (crate attribution, above) and `scripts/verify.sh:decide_scope` (heavy-check selection). The two had drifted — `decide_scope` matched all four patterns while `_is_noncrate` matched `docs/**` alone. The disagreement was invisible on a pure-docs diff, where `RUN_RUST=0` skips the closure entirely, but destroyed narrowing on a **mixed** one: measured, `affected_crates crates/reify-doc/src/lib.rs README.md` yielded `ALL` while the crate alone yielded a four-crate set, so one top-level `README.md` turned a four-crate verify into a full-workspace one. `decide_scope` consults `_is_inert` from **inside** its `*)` catch-all, never ahead of the `case`, so the arms that must outrank the inert rule still do: `gui/**` (a `gui/*.md` is GUI work), `tests/prd-gate/fixtures/*.ri`, and the `docs/gui-event-channels.md` carve-out.

**Corpus mapping: `examples/**/*.ri` → `_RI_CORPUS_CRATES` (task 7427).** The `examples/` tree is a test corpus: compiled Rust targets walk it and open `.ri` leaves by path, and `gui/src/__tests__/reifyGrammarCorpus.test.ts` sets `CORPUS_ROOTS = ['examples', …]` and walks it recursively. With no mapping rule, all 264 tracked `.ri` files were unmappable, so C5 fired and an `.ri`-only edit was the most expensive diff shape in the repo. They now map to the declared reader set `_RI_CORPUS_CRATES` (`reify-cli reify-compiler reify-eval reify-eval-fea-tests`), seeded through the ordinary C3 reverse closure. Membership is **derived, not hand-maintained**: `tests/infra/test_affected_crates_lib.sh`'s RI-CORPUS-DRIFT derives the set from the repo's own Rust sources (every crate with a non-comment line naming an `examples/<path>.ri` literal) and asserts DERIVED ⊆ DECLARED. The subset direction is deliberate — an extra *declared* crate only widens, which is the direction of error C5 already blesses, whereas an *undeclared* reader would have its tests narrowed away by an edit to the very fixture they read. Non-`.ri`, non-inert content under `examples/` (a `.gcode` datum, a `.gitkeep`) has no declared reader and still takes the C5 fail-wide arm.
...
**C7 — GUI lane split: typecheck always, vitest on the closure (task 7427).** `RUN_GUI` (`= rust | gui`) governs the node lane as a whole and is unchanged. Within it the two halves are now gated separately:

- **`npm ci` + `npm run typecheck` run whenever `RUN_GUI=1`.** Unconditional by design: the frontend consumes generated Rust→TS bindings, so *any* Rust change can break typechecking.
- **`npm test` (vitest) runs iff `RUN_GUI_VITEST=1`** — that is, `RUN_GUI=1` **and** either a frontend-read path changed (`GUI_PATH_SIGNAL`: `gui/**`, `tree-sitter-reify/**`, the workspace-global manifests, `docs/gui-event-channels.md`, `examples/**/*.ri`, and the prd-gate `.ri` fixtures pinned in the grammar drift ledger) **or** `closure_reaches_reify_gui` holds.

`closure_reaches_reify_gui` is a **fail-open ladder**, in the same register as C5: it is true for `SCOPE=all` (C2 — the merge gate never narrows, so the GUI lane there is untouched), true for every closure-*unavailable* shape (the `ALL` sentinel, an empty `CHANGED_FILES_RAW`, a malformed `REIFY_AFFECTED_CRATES_OVERRIDE`), and otherwise true iff `reify-gui ∈` the affected-crate closure. The only shape that therefore **skips** vitest is a `RUN_RUST=1` branch/staged diff whose closure is a real crate list excluding `reify-gui`. Because C2 keeps the merge gate unconditional, a Rust-only change outside `reify-gui`'s cone that nonetheless breaks the frontend is still caught — **delay ≤ 1 gate, never a coverage hole**. The predicate is a single implementation shared with the gui-feature nextest pass; role-specific skips (that pass's `DF_VERIFY_ROLE != offline` guard) stay at their call sites.

*Measured reach (task 7427; last 60 first-parent merges on `main`, 2026-09-09 → 2026-09-12, 3.00 days, replayed through the real predicate).* vitest would be **skipped on 3 of 60** merges (5.0%, ≈1.0/day at the measured 20 merges/day); 47 run it and 10 never reached the node lane at all (`RUN_GUI=0`). The rate is low because the ladder fails open so often: 15 merges yielded the `ALL` sentinel, 10 an empty closure, and 31 a closure containing `reify-gui` — leaving only 4 real narrow closures that exclude it, one of which still ran vitest on a frontend-read path. **The saving is CPU, not wall clock, by default:** the node lane is backgrounded ahead of the foreground Rust gates and joined later, so a skip converts to wall clock only when that lane is the long pole. Recorded so a future scope-tuning task need not re-derive it — the win here is hygiene (a crate change no longer implies a frontend suite), not throughput.

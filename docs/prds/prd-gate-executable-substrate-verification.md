...
- `value` → run `reify eval <fixture>` (the same argv as `ir`; the kind names the observation model, not the command) and match a **numeric predicate against the printed value**: `match.stdout_value` names a regex with a capture group plus at least one of `min`/`max`/`finite`. `present` ⇔ exit 0 **and** the capture is a finite number inside the bounds; `absent` ⇔ exit 0 and a capture was read but is not; exit ≠ 0, **or** the pattern located no capture at all, ⇒ **INDETERMINATE** (nothing was read, so "absent" and "broken fixture or mis-aimed pattern" are indistinguishable — ABSENT is reserved for a value actually read). **This is the branch `ir` cannot reach (task #6876):** `ir` answers ABSENT on exit 0 *before consulting `match`*, so an "eval exits 0 AND prints a finite in-range value" premise bound as `ir`/absent verifies only that eval did not crash. The mandatory constraint is what keeps the new kind from re-opening the same hole one level down.

...
- W2c (DF, conditional): if 2821 ships classifier-only, deterministic-red circuit-breaker at the retry seam (`CategoryPolicy.is_infra_transient`): identical failing-set on retry (reuse `parse_per_test_results` + `^FAILED \S+\.sh` matcher) ⇒ deterministic; same set across different tasks/SHAs ⇒ main-health probe + loud escalation, not infra-hold.

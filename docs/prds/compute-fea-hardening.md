...
2. **INV-FEA-1 (LSP posture).** The LSP's trampoline-free posture becomes documented + locked (mirroring `cmd_check`'s existing posture *(2026-08-27: that `cmd_check` posture has since been overturned — see the dated CORRECTION box in Sketch §1; the LSP subtraction itself is retained)*) instead of silently dropping FEA-constraint truth; a hint diagnostic tells the user why.
...
- **(a)** Document the posture (mirror `cmd_check`'s comment *(2026-08-27: `cmd_check`'s trampoline-free posture is retired — see the CORRECTION box in Sketch §1; the LSP posture stands on its own)*) + a locking test on a newly-authored FEA-bearing fixture (LSP tests use inline `&str` sources, not `.ri` files on disk — no existing FEA fixture in the LSP suite today).
...
9. **INV-FEA-3's gate stays scoped to the FEA numeric solve path — KEEP-SCOPED** (task 5159; supersedes that task's own 2026-07-09 census). This was a *sequencing* call, not a rejection in principle: the widening was deliberately queued **behind** the hardening it would otherwise force, and the trigger for revisiting it is stated below.

   **AMENDED 2026-08-20 by task #6376 — the trigger HALF-fired.** `SCOPE_PATHSPECS` now covers `crates/reify-stdlib/*.rs`: that crate's four class-A sites were hardened and its three class-B sites annotated, so the "not honestly executable" objection below no longer holds for it. Everything else in this decision stands as written — `reify-constraints` remains class A and remains **excluded**, so KEEP-SCOPED is still the live policy for the rest of the out-of-scope set. The strikethrough-free way to read what follows: the census, the reasoning and the rule are the historical record of WHY the deferral was right; the per-class notes mark what task #6376 discharged. *(That "remains excluded" clause was true on 2026-08-20 and is superseded by the amendment immediately below.)*

   **AMENDED 2026-08-27 by task #6377 — the trigger is now FULLY FIRED.** `SCOPE_PATHSPECS` also covers `crates/reify-constraints/*.rs`. That crate's class-A hazard was closed at its root: `eval_objective_set` (`solver.rs`) now fails closed on a non-finite **accumulator** (`if !acc.is_finite() { return Err(ObjectiveAbstention::NonFinite) }`), which is resolution (i) of decision 4's taxonomy — the function already abstained on a non-numeric term and every caller already had a correct abstention path (drop the candidate / `FeasibilityOnly` / `UNDEF_OBJECTIVE_PENALTY`). The abstention became a structured `ObjectiveAbstention` in the amendment pass, replacing the bare `Option`, so the causes reach a user apart rather than as one disjunctive reason string; a later amendment split the non-finite case in two (`NonFiniteTerm` / `NonFiniteFold`), because a term whose own expression evaluates to `inf` is rejected *before* anything is folded and telling that user their "weighted fold" overflowed points them at weights that are not at fault. Because that one function is the choke point for **both gate-visible ordering sites** — `solve_ranked_impl`'s sort and `impl Ord for ScoredModel` — the single guard discharged both with no change to either sort (the crate's *other* objective fold, `registry.rs::eval_rank_cost`, feeds an ε-band anchor rather than any ordering site, so it is outside INV-FEA-3's scope and is untouched here), and the two `unwrap_or(Ordering::Equal)` sites now carry honest `// nan-safe:allow` escapes that may truthfully claim **finite** (the guard rejects ±Inf too), not merely "never NaN". Only `reify-eval`'s `engine_build.rs` remains class A, so KEEP-SCOPED is now the live policy for reify-eval and reify-ir alone. Compile-time `term.weight` validation — resolution (ii) — was deliberately NOT taken: it cannot close the finite-weight overflow path (see the corrected evidence bullet below) and would create the false impression that it had.

   **The behaviour change that "no caller signature changes" hides — recorded because it is one (added 2026-09-01, task #6377 amendment pass).** Resolution (i) leaves every caller's *type* alone but changes what **four** of them *return*, deliberately and fail-closed in each case. The mechanism — why the accumulator can go non-finite though every term value is finite, and why the only honest answers are "abstain" and "lie" — is stated once, at the guard in `eval_objective_set` (`solver.rs`), and is not restated here. **This record is the SINGLE inventory of who reclassified, to what, and which test holds it**; the guard comment deliberately carries no per-caller test-name list, so a rename rots one copy rather than two (amended 2026-09-16 after review found four such free-prose cross-references in production code with nothing enforcing them).

   **What the guard replaced was MEASURED, not assumed** (guard disabled, `non_finite_objective_fold_at_feasible_initial_returns_no_progress`'s fixture re-run): not a uniform prior `Solved`, but whichever garbage the poisoned cost surface produced — a `NaN` weight yielded a `NoProgress` carrying argmin's internal `Potential bug: "NelderMead: Reached unreachable point."` text, and a `+Inf` weight yielded `Infeasible { ConstraintNonUnique }`, sending a user to edit constraints that were never at fault.

   Executable pins, one per reclassified caller:

   | reclassified caller | what it now returns | pin |
   |---|---|---|
   | `solve_core` (both objective-validation sites) | `NoProgress`, naming the cause | `non_finite_objective_fold_at_feasible_initial_returns_no_progress` |
   | `CpSatSolver::solve_ranked_with_budget` | drops the model rather than heaping a `NaN` | `a_non_finite_objective_fold_falls_back_instead_of_corrupting_the_heap` (whole-ranking fallback), `one_model_whose_fold_overflows_is_dropped_while_its_siblings_still_rank` (the PARTIAL case: one model dropped from a non-empty ranking, leaving a hole in the enumeration-index sequence that the closing `debug_assert_eq!` must still hold across) |
   | `ConstraintCostFunction::cost` | folds to `UNDEF_OBJECTIVE_PENALTY`, which for a `-Inf` fold inverts that point of the surface from global attractor to most repellent | `cost_function_penalizes_a_non_finite_objective_fold`, with `cost_function_leaves_a_finite_objective_cost_byte_identical` as the I1 positive control |
   | `score_solution` → `rank_single` / the multistart loop / `verify_uniqueness` | `None`, so the OPTIMALITY VERDICT changes: `BestFound` + `Some(inf)` becomes `FeasibilityOnly` + `None` (and `classify_uniqueness` takes `_ => NonUnique` where it could have reported `IncumbentSuboptimal`) | `a_non_finite_fold_reranks_a_solved_candidate_as_feasibility_only` |

   The cpsat pin was added after review observed that the five `eval_objective_set` unit cases reached the *more severe* cpsat site only by transitive argument, so a future second scoring path could re-open the debug-build panic with nothing failing; its partial-case sibling and the `score_solution` row were added by the 2026-09-16 amendment, which found the fourth caller unenumerated and its user-visible reclassification unasserted. `classify_uniqueness`'s arm needs a two-basin fixture and is recorded here rather than pinned. The `NoProgress` causes — an undefined TERM, a non-finite TERM, and a non-finite FOLD of finite terms — are reported *apart*, via a structured `ObjectiveAbstention` rather than one disjunctive sentence, so a reader and a test can tell which fired; the `solve_core` pin asserts the WHOLE reason string against `ObjectiveAbstention::NonFiniteFold.reason()`, which is what makes that discrimination three-way rather than merely "not `Undefined`".

   **Corrected census (measured on branch base 2026-08-20).** Task 5159's description asserted "~11 additional PRODUCTION sites … all currently guarded". Both halves are wrong. It was **stale** — it missed `crates/reify-constraints/src/solver.rs` entirely, and every line number it cited had drifted (`engine_build.rs` 10696/10760 → 12621/12685; `persistent_cache.rs` 1224 → 1228). And its central claim is **false**: 7 of the 14 gate-visible out-of-scope sites are *not* provably NaN-safe. Measured by copying the gate to a scratch file, inserting each candidate crate's pathspec, and running it — which is what separates a real match from one the `#[cfg(test)]` skipper already exempts. Gate-visible counts: reify-stdlib 7, reify-eval 4, reify-ir 2, reify-constraints 1. **CORRECTED 2026-08-27 (task #6377): the reify-constraints count is 2, not 1.** This census missed `crates/reify-constraints/src/cpsat.rs` (`impl Ord for ScoredModel`, production — that file's first `#[cfg(test)]` is well below it) entirely — the same staleness failure mode this decision already documents for 2026-07-09, recurring six weeks later in the census that was written to correct it. Found by the very method this paragraph prescribes: inserting `'crates/reify-constraints/*.rs'` into a scratch copy of the gate and running it reports **two** sites, `solver.rs` and `cpsat.rs`. The missed site was also the **more severe** of the two (see the evidence bullet below).

   *Caveat, and the reason this record exists: line numbers drift — the 2026-07-09 set went stale in six weeks unnoticed. **The file-level record below is what is authoritative**; treat every `:NNN` as a 2026-08-20 hint, not an identifier.*

   **A. NOT provably NaN-safe (7 at census, 8 in truth; 2 remain)** — these are why widening in 2026-08 was not honestly executable. Task #6376 hardened the four reify-stdlib rows (`analysis.rs:194`, `:222`, `matrix.rs:223`, `:239`), which therefore leave class A. Task #6377 then hardened the reify-constraints rows — `solver.rs:2763` **and the uncensused `cpsat.rs` site**, which is why the true class-A count was 8, not 7 — and they leave it too; **one** fail-closed guard discharged both, because both consume `eval_objective_set`. The two rows below are what is left, and both are reify-eval's:

   | site | what it orders | really the idiom? | provably NaN-safe? | guard citation |
   |---|---|---|---|---|
   | `reify-eval/src/engine_build.rs:12621` | dot-product projection sort (nearest-3 shell/tet nodes) | yes | **no** | `iface.normal` is finite-guarded at `:12591-92`, but `nodes[..]` is not |
   | `reify-eval/src/engine_build.rs:12685` | nearest-node distance sort | yes | **no** | unguarded at every level; `dist3_sq` can even *manufacture* NaN from infinities (`inf - inf`) |

   Evidence for the four load-bearing rows (each kept after resolution as the record of why it was a hazard):

   - **`analysis.rs:222`** — *RESOLVED by task #6376; kept as the record of why it was a hazard.* It was confirmed reachable in production, not hypothetical. `compute_eigenvalues_3x3` has exactly two returns (`:195`, `:223`), **both `Some(..)`** — it can never return `None`. `crates/reify-expr/tests/field_reductions_tests.rs:3442` already documents that `compute_eigenvalues_3x3([NaN; 9])` returns `Some([NaN, NaN, NaN])`. An all-NaN window (the out-of-solid sentinel the FEA elaborator emits) takes this branch, because `NaN <= 1e-30` is false; `.clamp(-1.0, 1.0)` at `:212` propagates NaN rather than trapping it.
   - **`matrix.rs:239`** — *RESOLVED by task #6376; kept as the record of why it was a hazard. This site was missed by task 5159's plan-time table and was classified during implementation.* The branch guard at `:235`, `complex_eigs.iter().all(|c| c.im.abs() <= 1e-9 * (c.re.hypot(c.im) + 1.0))`, does reject NaN in the ordinary case: any NaN in `im`, or a NaN `re` with finite `im`, makes the comparison false and returns `Value::Undef`. It has one **measured** hole: `f64::NAN.hypot(f64::INFINITY)` is `inf` (verified by direct execution), so for `re = NaN, im = ±∞` the predicate reads `inf <= inf` → **true**, and a NaN real part reaches the sort. Whether nalgebra's `complex_eigenvalues()` can actually emit that `(NaN, ±∞)` pair was **not** demonstrated — so this is "not provably safe with a narrow, reachability-unproven hole", strictly weaker than `:223`. Noted also: the `eigenvalues` builtin applies **no input finiteness guard at all** (`rank2_components` checks shape and dimension only; `matrix.rs`'s single `is_finite` at `:261` belongs to the *other*, `complex_eigenvalues`, builtin).
   - **`solver.rs:2763`** — *RESOLVED by task #6377; kept as the record of why it was a hazard.* The diagnosis below was right. The comment directly above it (`:2756-2760`) claims `eval_objective_set` "already filters out" NaN so `unwrap_or(Equal)` is "never actually exercised". **That comment is false.** The `.filter(|v| v.is_finite())?` at `:1413` is **per-term, on `v`**; the accumulator returned at `:1419` (`acc += term.weight * v`, `:1415-1416`) is never re-checked, and `term.weight` is an unvalidated public field (`crates/reify-ir/src/constraint.rs:84`, `:240`). A NaN or infinite weight yields a NaN `acc` that reaches this sort.

     The fix is resolution (i), fail-closed at the fold (`if !acc.is_finite() { return Err(..) }`), **not** resolution (ii), compile-time `term.weight` validation. Weight validation alone would have left a path open while looking closed: with **every weight finite and positive** — exactly what such a check would enforce — `term.weight * v` can still overflow to ±Inf, and a `+Inf` `Minimize` term plus a `-Inf` `Maximize` term folds to `inf - inf` = **NaN**. Only a guard on the ACCUMULATOR covers that. The same guard discharged the uncensused `cpsat.rs` site at the same time, because `ScoredModel.score` is only ever an `eval_objective_set` result — which is why one function, not two sorts, was the right place to fix this.

   - **`cpsat.rs` (`impl Ord for ScoredModel`)** — *MISSED by the 2026-08-20 census; classified and RESOLVED by task #6377.* Strictly worse than `solver.rs:2763`'s silent mis-rank, and the reason the miss mattered. A NaN `score` (1) makes `impl Ord`/`impl Eq` violate their own contracts inside a `BinaryHeap`, so eviction picks the wrong victim; (2) sends the running best/ties tally to its `_` arm on **every** subsequent model, because NaN compares false both ways, degrading `best` from "the minimum seen" to "the last score seen"; and (3) then trips that function's own `debug_assert_eq!` (`Some(NaN) == Some(NaN)` is `false`) — a **reachable debug-build panic**, not merely a wrong ranking.

   **B. Provably NaN-safe, honestly annotatable today (5; 3 now ANNOTATED).** Task #6376 carried the widening into reify-stdlib, so `matrix.rs:283`, `:284` and `stackup.rs:589` now carry real `// nan-safe:allow` escapes rather than merely being eligible for them. The `stackup.rs` rationale is deliberately phrased "never NaN", not "always finite" — a pathological overflow to ±Inf remains possible and remains totally ordered by `partial_cmp`, which is all that site needs; claiming "finite" would be subtly false, and a rationale a reader cannot trust is exactly what turns this escape into a rubber stamp. The two reify-eval entries stay eligible-but-unannotated (that crate is still out of scope). `matrix.rs:283` and `:284` (explicit `is_finite` early-return at `:261`); `stackup.rs:589` (`validate_dimensioned_scalar`, `helpers.rs:238`); `persistent_cache.rs:1228` (`eviction_score` is structurally finite); `warm_pool.rs:449` (clamped at `:371-375`, the sole `PoolEntry` construction site). These are the sites a widening *could* annotate truthfully.
   ...
   **Why KEEP-SCOPED follows.** Task 5159 offered "extend `SCOPE_PATHSPECS` … and annotate each guarded site with `// nan-safe:allow — <reason>`". Class A makes that not honestly executable: 7 of the sites are **not** guarded, so widening today forces either (a) 7 **false** `nan-safe:allow` rationales — which destroys the escape's only value, that a reader can trust the stated reason, converting a safety gate into a rubber stamp — or (b) 7 fail-closed fixes, which are E1–E5-class hardening spanning 3 crates. The task's own framing ("a scope/coverage decision, not new machinery") is therefore the argument for **deferring** the widening, not for performing it. Policy for the eventual fixes is decision 4's fail-closed-vs-`total_cmp` taxonomy; it is not restated here.
   ...
   **The widening trigger — FULLY FIRED (2026-08-27).** As stated: when the follow-up hardening lands, widen to `crates/reify-stdlib/*.rs` and `crates/reify-constraints/*.rs` and annotate the then-provably-safe class-B sites. Task #6376 fired the **reify-stdlib half** (2026-08-20); task #6377 fired the **`crates/reify-constraints/*.rs` half** (2026-08-27), discharging `tkt_0RSNHFG67K1G73BK4XJJK7CY9N`. Both named pathspecs are now in `SCOPE_PATHSPECS` and this trigger is discharged. It does **not** extend to reify-eval or reify-ir: those were never part of this trigger's wording, `tkt_0RSNHFTV5AHHX092ZBA6EF9CEE` still owns reify-eval's two class-A sites, and KEEP-SCOPED remains the live policy for both crates.
   ...
   |---|---|---|
   | `tkt_0RSNHF643224BNS68VK857YH6N` — **discharged by task #6376** | reify-stdlib | `analysis.rs:194`, `analysis.rs:222`, `matrix.rs:223` (3) |
   | `tkt_0RSNK3M5QXP0Q6JZ6490E9DP34` — **discharged by task #6376** | reify-stdlib — `matrix.rs:239` `hypot(NaN, inf)` hole | `matrix.rs:239` (1) |
   | `tkt_0RSNHFG67K1G73BK4XJJK7CY9N` — **discharged by task #6377** | reify-constraints `solve_ranked` | `solver.rs:2763` (1) — plus the uncensused `cpsat.rs` site this row did not know about; both closed by the one `eval_objective_set` guard |
   ...
   **Executable baseline.** This decision is machine-checked, not only prose: `tests/infra/test_nan_safe_ordering_guard_wired.sh` block **(hJ)** pins the accepted over-flag and its escape, and block **(hK)** pins the exclusions — seven at census, **five** after task #6376 removed the two reify-stdlib entries and **four** after task #6377 removed the reify-constraints entry (each became false by design once its crate came into scope) — with an anti-vacuity control asserting the byte-identical hazard *is* flagged at a covered path. Task #6376 added the complementary block **(hL)**: the positive pin that reify-stdlib IS covered and that the escape still works there, so a crate that leaves the excluded set does not land in "pinned by nothing". Task #6377 added block **(hM)**, the same positive pin for reify-constraints — pinning **both** `solver.rs` and `cpsat.rs`, precisely so that pinning only the censused site could not re-create the blind spot that let `cpsat.rs` go unrecorded. `SCOPE_PATHSPECS`, this decision, and the (hK) pins are meant to fail together; a change that flips any (hK) assertion is the intended tripwire, and the reviewer should be sent to this census and the three tickets above before approving it. **Measured widening cost.** At census the gate scanned 121 files in ~1.5–1.8 s warm (3.8 s cold), and adding reify-stdlib + reify-ir + reify-constraints was *projected* to add **88** scan files (59 + 18 + 11) for a total of 209. Task #6376 fired the reify-stdlib half and **measured** the result rather than leaving the projection to go stale: **180 scan files** (121 + 59 — the stdlib projection was exactly right), gate green in ~4–6 s wall on a heavily loaded host, so treat that as an upper bound rather than a like-for-like successor to the ~1.5–1.8 s warm figure. Task #6377 fired the reify-constraints half and likewise **measured** it: **192 scan files**, up from a 181-file baseline immediately before the widening (the 180 measured on 2026-08-20 plus one file of ordinary tree growth in the already-covered crates). That is **+11** for reify-constraints — the projection was exactly right a second time. Only the reify-ir projection (+18) is left unmeasured, and it is still a projection, not a commitment to widen.
...
| `docs/prds/reify-eval-fea-decomposition.md`, task **#4935** ("B — create reify-eval-fea") | contested (file relocation vs. in-place edit) | Task 4935 is **pending now** (not deferred — the M-gate flip-condition already cleared 2026-07-03; the PRD doc's own status line is stale) and its declared file set moves `crates/reify-eval/src/{compute_targets/mod.rs, compute_targets/elastic_static.rs, modal_ops.rs, engine_compute.rs}` wholesale to a new `reify-eval-fea` crate. This collides structurally (not just contends-for-the-lock) with A1, D1-D9, E4, E5 — a *move*, unlike an ordinary same-file edit, can leave those tasks' `metadata.files` anchors pointing at a path that no longer exists. (Ordinary same-file edit contention from ~11 *other* unrelated pending/in-progress tasks touching these same files — 4039, 4351, 4744, 4828, 4880, 4909, 4944, 4951, 4996, 5002, 5030 — is normal narrow-lock-orchestrator business and is *not* treated specially here.) | this PRD's decompose wires `add_dependency(4935, depends_on=<each of A1,D1-D9,E4,E5>)` so the relocation carries the hardened code across instead of racing it | **wired** at decompose time |
...
- NaN-ordering sites outside the survey's five named crates/modules found incidentally during the grep sweep (`reify-eval::persistent_cache.rs:1224`, `engine_build.rs:10710,10774`, `warm_pool.rs:449` — cache-eviction/version-ordering/event-ordering, not physical/geometric numeric domain) — noted for a possible future audit sweep, not filed here (scope discipline: the brief named specific crates/modules; expanding to all of `reify-eval` is a materially larger, un-ratified scope). **Revisited by Resolved design decision 9 (task 5159)**, which re-measured this set on 2026-08-20: the domain classification above holds for `persistent_cache.rs` (cache-eviction) and `warm_pool.rs` (cost ordering), but is **wrong for `engine_build.rs`** — those two sites are a dot-product projection sort and a nearest-node distance sort, both squarely physical/geometric. Decision 9 carries the corrected census and the current line numbers; the ones in this bullet have drifted.
...
4. **Whether task #4935's relocation should instead proceed independently of this PRD** (i.e., whether the `add_dependency(4935, depends_on=...)` edge this PRD wires is the right call, vs. Leo preferring 4935 to run on its own schedule and accepting the re-verification cost if it lands mid-flight). Discovered during decompose, not author-mode — flagged here rather than stalling. Suggested resolution (implemented): gate 4935 on this PRD's file-touching tasks landing first, since a mechanical crate-move should carry finished hardening across rather than race it; the edge is a pure ordering constraint (no scope change to 4935) and is trivially reversible via `remove_dependency` if Leo disagrees. Revisit if 4935 sits blocked-on-this-PRD long enough to become its own friction point.

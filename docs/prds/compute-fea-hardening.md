...
9. **INV-FEA-3's gate stays scoped to the FEA numeric solve path — KEEP-SCOPED** (task 5159; supersedes that task's own 2026-07-09 census). `scripts/check-nan-safe-ordering.sh`'s `SCOPE_PATHSPECS` is **not** widened now. This is a *sequencing* call, not a rejection in principle: the widening is deliberately queued **behind** the hardening it would otherwise force, and the trigger for revisiting it is stated below.
   ...
   **A. NOT provably NaN-safe (7)** — these are why widening today is not honestly executable:

   | site | what it orders | really the idiom? | provably NaN-safe? | guard citation |
   |---|---|---|---|---|
   | `reify-stdlib/src/analysis.rs:194` | 3×3 principal stresses, diagonal-matrix branch | yes | **no** | only two `debug_assert!`s, and the symmetry one at `:157-177` is *explicitly written to tolerate NaN* (`a.is_nan() && b.is_nan()`); the `p1 <= 1e-30` branch guard at `:191` constrains only the off-diagonal |
   | `reify-stdlib/src/analysis.rs:222` | 3×3 principal stresses, general branch | yes | **no — confirmed reachable** | see evidence below |
   | `reify-stdlib/src/matrix.rs:223` | `eigenvalues` builtin, symmetric branch | yes | **no** | the symmetry test at `:219` compares only strict off-diagonal pairs (`(i+1..n)`), so the **diagonal is never inspected** and NaN diagonals reach the sort; `sanitize_value` runs *after*, at `:224` |
   | `reify-stdlib/src/matrix.rs:239` | `eigenvalues` builtin, general (non-symmetric) branch | yes | **no — narrow hole** | see evidence below |

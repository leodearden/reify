...
9. **INV-FEA-3's gate stays scoped to the FEA numeric solve path — KEEP-SCOPED** (task 5159; supersedes that task's own 2026-07-09 census). This was a *sequencing* call, not a rejection in principle: the widening was deliberately queued **behind** the hardening it would otherwise force, and the trigger for revisiting it is stated below.
   ...
   **A. NOT provably NaN-safe (7 at census; 3 remain)** — these are why widening in 2026-08 was not honestly executable. Task #6376 hardened the four reify-stdlib rows (`analysis.rs:194`, `:222`, `matrix.rs:223`, `:239`), which therefore leave class A; the three rows below are what is left:

   | site | what it orders | really the idiom? | provably NaN-safe? | guard citation |
   |---|---|---|---|---|
   | `reify-constraints/src/solver.rs:2763` | multi-start candidate ranking by objective score | yes | **no** | see evidence below |
   ...
   Evidence for the three load-bearing rows:
   ...
   - **`solver.rs:2763`** — the comment directly above it (`:2756-2760`) claims `eval_objective_set` "already filters out" NaN so `unwrap_or(Equal)` is "never actually exercised". **That comment is false.** The `.filter(|v| v.is_finite())?` at `:1413` is **per-term, on `v`**; the accumulator returned at `:1419` (`acc += term.weight * v`, `:1415-1416`) is never re-checked, and `term.weight` is an unvalidated public field (`crates/reify-ir/src/constraint.rs:84`, `:240`). A NaN or infinite weight yields a NaN `acc` that reaches this sort.
   ...
   **The widening trigger — HALF FIRED (2026-08-20).** As stated: when the follow-up hardening lands, widen to `crates/reify-stdlib/*.rs` and `crates/reify-constraints/*.rs` and annotate the then-provably-safe class-B sites. Task #6376 fired the **reify-stdlib half only** — that was its whole scope. The `crates/reify-constraints/*.rs` half is **still pending** on `tkt_0RSNHFG67K1G73BK4XJJK7CY9N`; do not read this trigger as discharged.

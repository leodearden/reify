...
9. **INV-FEA-3's gate stays scoped to the FEA numeric solve path — KEEP-SCOPED** (task 5159; supersedes that task's own 2026-07-09 census). This was a *sequencing* call, not a rejection in principle: the widening was deliberately queued **behind** the hardening it would otherwise force, and the trigger for revisiting it is stated below.
   ...
   **AMENDED 2026-08-27 by task #6377 — the trigger is now FULLY FIRED.** `SCOPE_PATHSPECS` also covers `crates/reify-constraints/*.rs`. That crate's class-A hazard was closed at its root: `eval_objective_set` (`solver.rs`) now fails closed on a non-finite **accumulator** (`if !acc.is_finite() { return None }`), which is resolution (i) of decision 4's taxonomy — the function already returns `Option<f64>` and every caller already has a correct `None` path (drop the candidate / `FeasibilityOnly` / `UNDEF_OBJECTIVE_PENALTY`). Because that one function is the choke point for **both gate-visible ordering sites** — `solve_ranked_impl`'s sort and `impl Ord for ScoredModel` — the single guard discharged both with no change to either sort (the crate's *other* objective fold, `registry.rs::eval_rank_cost`, feeds an ε-band anchor rather than any ordering site, so it is outside INV-FEA-3's scope and is untouched here), and the two `unwrap_or(Ordering::Equal)` sites now carry honest `// nan-safe:allow` escapes that may truthfully claim **finite** (the guard rejects ±Inf too), not merely "never NaN". Only `reify-eval`'s `engine_build.rs` remains class A, so KEEP-SCOPED is now the live policy for reify-eval and reify-ir alone. Compile-time `term.weight` validation — resolution (ii) — was deliberately NOT taken: it cannot close the finite-weight overflow path (see the corrected evidence bullet below) and would create the false impression that it had.

   **The reclassification that "no caller signature changes" hides — recorded because it is a behaviour change, not a refactor (added 2026-09-01, task #6377 amendment pass).** Resolution (i) leaves every caller's *type* alone but changes what three of them *return*, and all three are deliberate fail-closed choices rather than side effects. (1) `solve_core`'s fallback-point and post-solve checks — both `eval_objective_set(..).is_none()` — now report `NoProgress`, and their reason string was widened from "objective expression evaluated to undefined at … point" to add "(or its weighted fold was non-finite)", because telling a user their objective was *undefined* when every term was well defined and only the weighted fold overflowed is a wrong diagnosis. **What that replaced was MEASURED, not assumed** (guard disabled, fixture re-run): not a uniform prior `Solved`, but whichever garbage the poisoned cost surface produced — a `NaN` weight yielded a `NoProgress` carrying argmin's internal `Potential bug: "NelderMead: Reached unreachable point."` text, and a `+Inf` weight yielded `Infeasible { ConstraintNonUnique }`, sending a user to edit constraints that were never at fault. (2) `ConstraintCostFunction::cost` folds `None` to `UNDEF_OBJECTIVE_PENALTY` (`f64::MAX / 2`); for a `-Inf` fold that *inverts* the cost surface at that point, from a global attractor Nelder-Mead would dive into to the most repellent point available — intended, since a `-Inf` attractor is an overflow artefact and never a real optimum. (3) cpsat's ranked path drops the model rather than pushing a NaN into `BinaryHeap<ScoredModel>`. Executable pins: `non_finite_objective_fold_at_feasible_initial_returns_no_progress` (solver.rs) and `a_non_finite_objective_fold_falls_back_instead_of_corrupting_the_heap` (cpsat.rs) — the latter added in the same amendment pass, after review observed that the five `eval_objective_set` unit cases reached the *more severe* cpsat site only by transitive argument, so a future second scoring path could re-open the debug-build panic with nothing failing. The canonical prose statement of all three lives at the guard itself (`solver.rs`, `eval_objective_set`); this bullet is the decision record, not a second copy of the argument.

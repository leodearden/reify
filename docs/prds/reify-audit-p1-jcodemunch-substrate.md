...
2. **An HTTP MCP client** (`crates/reify-audit/src/jcodemunch_client.rs`) — a near-clone of `fused_memory_client.rs`: sync `ureq`, streamable-HTTP protocol `2024-11-05`, with the `initialize` → `notifications/initialized` → tool-call handshake (the MCP `call` method). It invokes the jcodemunch MCP tool methods and adapts their wire shapes to the crate's `ChangedSymbol`/`SymbolReference`/new result structs. `RealJCodemunchOps` implements the `JCodemunchOps` trait over this client.

> **Path-guard note (re-decomposition hazard):** fused-memory's submit-task path-guard is a naïve substring matcher — any task text containing the literal segment `tools` + `/` is rejected (it is attributed to another project; reify has no such directory). When decomposing this PRD, scope every file reference to `crates/reify-audit/...` and never paste that substring into a task title/description/details/files list. This is why the protocol method above is written as "the MCP `call` method" rather than its raw JSON-RPC name.
...
4109 landed a `NoopJCodemunchOps` fall-back in `bin/reify-audit.rs` (lines 547–567): when `needs_jcodemunch` is true and `RealJCodemunchOps::new` errors (server down), the binary falls back gracefully — P2/P5 survive, P1 and all jcodemunch detectors degrade to zero findings, exit-125 is reserved for genuine arg/IO misconfiguration. `needs_jcodemunch` (lines 433–443) already returns true for the no-`--pattern` default sweep (P1 is in the default set), so the default sweep already opens and fail-softs a jcodemunch connection today. Folding PDEAD/PUNTESTED/PLAYER would add NO new connection or failure mode — only three additional repo-wide round-trips. This input removes the safety objection and satisfies the graceful-degradation precondition, but it is necessary, not sufficient.

...
6. **Pool containment (ζ activation ruling, esc-5280-6).** The knob is consulted **exactly once** — by the top-level merge verify at plan-build time (`verify.sh:1160-1178`), whose decision is frozen into the PLAN before any plan line executes. When ζ activates the knob via `verify_env` it becomes ambient across the whole merge-gate verify.sh process tree, including the `run_all.sh` infra pool launched as a plan line (`verify.sh:2079`). Pool members that re-assert `DF_VERIFY_ROLE=merge` inline to drive their own hermetic `verify.sh --print-plan` fixtures (e.g. `test_verify_scope.sh` MG-B5, `test_verify_role_prio.sh` C1, and the occt/release/semaphore plan-shape meta-tests) would otherwise inherit the ambient knob and see the release pass wrongly suppressed on a delta-clean fixture — turning the merge gate RED on *every* delta-clean merge. The ruling (chokepoint strip over per-test hardening) is realized by `tests/infra/run_all.sh` exporting `REIFY_RELEASE_DELTA_SKIP=0` for all members at the same seam that already normalizes `DF_VERIFY_ROLE=task` — extending K4's "never ambient in the pool" invariant to this knob. This is the direct counterpart of the identical-shape `DF_VERIFY_ROLE` leak that seam was written for; per-test env-pinning was rejected because it would edit `test_verify_scope.sh` (owned by in-flight task 5166, §8) and leave an open-ended "every future merge-role capture must remember to pin" obligation.
...
| K5′ | Pool containment (§5.1.6, esc-5280-6): the activated `verify_env` knob is stripped for all `run_all.sh` pool members (`export REIFY_RELEASE_DELTA_SKIP=0` beside the `DF_VERIFY_ROLE=task` normalization), so a member that re-asserts role=merge inline never inherits it — K4's "never ambient in the pool" extended to this knob | `test_verify_release_delta_skip.sh` CONTAINMENT block (task ζ) |

...
**(A) Held-slot test-run counting semaphore, composed with the PSI gate.** A new sourceable lib (`scripts/lib_test_semaphore.sh`) provides acquire/hold/release of an N-slot semaphore using the proven mechanics of `cargo-test-occt-gated.sh`: shuffle-order acquire over `${LOCK}.slot-1..N`, slot held on an FD for the run's whole duration, child exec'd with the FD closed (`9<&-`) so no descendant (sccache, rustc) leaks the slot, deadline-checked-before-sleep, exit 75 on `LOCK_WAIT`. `verify.sh` runs `psi_gate()` first (pressure headroom), then `compile_gate()` (block-entry load gate, tasks 4853/4862), then acquires the slot, then runs one combined build+execution nextest pass per profile with the slot held, releasing after the last pass. Gated region = the **entire build+execution block per profile** — never `psi-gate` itself, never compile/check/clippy (those are outside the gated region on the lint/typecheck path). Merge-exempt: when `DF_VERIFY_ROLE=merge`, skip acquisition entirely (mirrors the PSI bypass).

_Update (task 4839 / esc-4837-6 — compile-outside-slot, 2026-06-25):_ The initial implementation wrapped `cargo nextest run` (compile + execution) inside the held slot. Under concurrent merge verifies the task jobserver pool is merge-favored (8 task tokens), so the compile crawled and the slot clung for tens of minutes — starving other task verifies. Fix: emit `cargo nextest run --no-run` (compile-only, host-bounded by the jobserver) AFTER psi-gate but BEFORE `@@SEMAPHORE_ACQUIRE@@`. The slot then wrapped test **execution** only — short, pressure-bounded, fast-rotating._

_**Reversal (task 4862, 2026-06-27):** The compile-outside-slot split is removed — see D6 reversal note. The build+execution block runs again as one unified slot-held pass per profile._
...
- **D6 — Gate the test-execution phase only.** Compiles overlap freely (jobserver-bounded host-wide); the semaphore wraps only the nextest/cargo-test test-**execution** passes. This is why the held slot can't be a fire-and-return plan line like `psi-gate` — it spans multiple pass executions, so `verify.sh` holds the FD in its own process and `--print-plan` marks the gated region with a comment, not an executable line. _Update (task 4839 / esc-4837-6):_ "test-execution phase" now literally excludes the test-binary compile: `add_test_passes()` emitted `cargo nextest run --no-run` compile passes BEFORE `@@SEMAPHORE_ACQUIRE@@` (after psi-gate, outside the slot) and only the execution passes (`cargo nextest run` without `--no-run`) ran inside the held region. Rationale: when the compile crawled under the merge-favored task jobserver pool the slot clung for tens of minutes — the compile-outside-slot split eliminates this starvation class while preserving the D6 invariant that execution is the only phase the slot bounds.

  _**Reversal (task 4862, 2026-06-27):** The D6 compile-outside-slot decision is reversed — the held slot now wraps the ENTIRE build+execution block again (one unbroken slot-held block per profile). WHY: (1) **MEMORY is now the binding constraint**, not CPU starvation. Host memfull avg10 ~28%, ~161 GiB swap on a 125 GiB host. The held slot's whole-block serialization was the only implicit bound on concurrent RSS-heavy link waves (OCCT/OpenVDB/gmsh/manifold link, ~1-3 GiB RSS/link). 4839's pipelining only pays under memory headroom, which is gone — it is net-negative, worst during merges when multiple link waves can overlap. (2) **Run-to-completion serialization is optimal under memory pressure.** For box-saturating work, serializing whole build+test blocks beats interleaved pipelining for both latency (staggered completions) and throughput (same core-seconds minus paging). (3) **The starvation 4839 band-aided is now fixed properly** by the clock-stop seam (task 4838): the in-process continuous wait (WAIT=unlimited) turns a slot-wait-during-build into a graceful hold, not exit-75. So the original D6 starvation risk is eliminated at the root, making the compile-outside-slot workaround unnecessary. (4) The compile-gate (task 4853) is **kept** but repositioned as a **block-entry load gate** — it sits after psi-gate and BEFORE `@@SEMAPHORE_ACQUIRE@@`, gating the whole build+test block's admission. It carries the memory-PSI dimension (cpu-admit.sh `_ca_mem_full_threshold`, default 10%) as an implicit property — no add_test_passes() plumbing change needed._
...
- **Integration face (ε):** an e2e test drives real `scripts/verify.sh` the way the orchestrator does — two concurrent `verify.sh test` (`role=task`) hold-serialize; a `role=merge` run is exempt; exit 75 propagates out of `verify.sh`; the emitted plan shows occt cap=24, compile/check passes *outside* the gated region, and (task 4862 reversal of 4839) all `cargo nextest run` passes appear INSIDE the `@@SEMAPHORE_ACQUIRE@@`/`@@SEMAPHORE_RELEASE@@` region — no `--no-run` compile pass outside the slot.

...
| new codes mintable | `E_CTOR_UNKNOWN_FIELD` / `E_CTOR_ARITY` minted in ε's own diff following the `SelectorKindMismatch`/`ArgTypeMismatch` minting discipline — two fieldless unit variants appended to `DiagnosticCode`, grep: `pub enum DiagnosticCode` `crates/reify-core/src/diagnostics.rs:156` (185 unit variants today, `#[non_exhaustive]`). No accessor to update: grep `impl DiagnosticCode` is **zero hits workspace-wide**, and the serde `rename_all = "PascalCase"` derive (`:154`) alone supplies the wire form. **No completeness gate fires when a variant is added** — `DiagnosticCode` derives no `strum` trait (zero `strum`/`EnumIter`/`EnumCount` hits in that file), so `DiagnosticCode::iter()`/`::COUNT`/`::VARIANTS` do not exist (zero hits repo-wide); `#[non_exhaustive]` forbids an out-of-crate exhaustive match, so there is no compile-time pressure either; and nothing anywhere scans declared-vs-used diagnostic codes (`DiagnosticCode` is zero hits across `scripts/`, `tests/infra/`, `hooks/`). ε must therefore **hand-write** the opt-in per-variant `--lib` test into the inline `mod tests` of the same file, in its own diff — grep: `mod tests` `:4131` (128 `#[test]` fns today; exemplar `eval_cycle_and_unresolved_codes_exist_and_attach` `:4142`; note `diagnostic_code_multi_kernel_variants_with_code_round_trip` `:5244` loops a HARDCODED array, not an enumeration — it is not a sweep). Correction 2026-08-12 (task 6224, re-verified against `12396211741b`): this cell previously cited a named strum-completeness guard that does not exist anywhere in the tree, plus an "asymmetry scan" that likewise names nothing (the phantom identifier is deliberately not reproduced here, so a `git grep` for it stays at zero hits — see task 6224's diff for the removed text); the repo's real completeness precedents split into two classes (anchors re-verified first-hand at `713c0678f5`), and `DiagnosticCode` follows NEITHER — **(i) strum-derived sweeps**: `TypeDiscriminants`, derived at `crates/reify-core/src/ty.rs:207-208` and consumed via `TypeDiscriminants::iter()` by `every_type_variant_is_claimed_by_exactly_one_member_access_handler_class` `crates/reify-compiler/src/expr.rs:9882`; and `GeometryOp` via `GeometryOpDiscriminants`, declared `#[strum_discriminants(name(GeometryOpDiscriminants), derive(strum::EnumIter, strum::EnumCount, Hash))]` at `crates/reify-ir/src/geometry.rs:568` and consumed as `GeometryOpDiscriminants::iter()` at `crates/reify-eval/src/engine_build/tests.rs:6765` (inside `parent_handles_for_op_returns_expected_handles_per_variant_family`, `:6225`–`:6774`) — those two `strum_discriminants(name(…))` sites are the only ones in `crates/`; **(ii) hand-rolled `const ALL_*` + `VARIANT_COUNT` checks**: `compile_geometry_op_registry_completeness` `crates/reify-eval/src/geometry_ops/tests.rs:20435`, which is emphatically NOT strum-based (zero `strum`/`EnumIter`/`EnumCount` hits in that whole file) — it cross-checks hand-written arrays such as `const ALL_PRIMITIVE: [PrimitiveKind; 8]` (`:20442`) against `PrimitiveKind::VARIANT_COUNT`, itself the hand-written `pub const VARIANT_COUNT: usize = Self::ALL.len();` `crates/reify-compiler/src/types.rs:1388` (in `impl PrimitiveKind`, opens `:1361`), never strum's `EnumCount::COUNT`. Class (ii) is the CLOSER analogue for ε: like those kinds, `DiagnosticCode` gets no derive-driven sweep, so ε must author the check by hand rather than expect one to go red. (An earlier revision of this breadcrumb mislabelled `:20435` as a `GeometryOp` strum precedent on all three counts — wrong enum family, wrong mechanism, wrong class.) Verdict stays PASS: the capability (mintable) is true, and cheaper than the old cell claimed — only its evidence was fabricated. | PASS |

...
Cargo emits one test binary per `tests/*.rs`. Consolidate many former standalone files into a small number of **harness compile units**, each a `tests/harness_<subsystem>.rs` that `mod`-declares the former files (now under `tests/harness_<subsystem>/<file>.rs`) under each former file's **original stem**, giving the merged test a stable, predictable `<file>::<test_name>` selector *going forward* — this does not preserve the pre-consolidation id (full statement: §Contract C1 / `tests/infra/test_harness_kloc_cap.sh` C1 header). nextest still schedules every `#[test]` fn independently across binaries (substrate verified, survey §G3) — per-test selection and scheduling are unchanged; only the link count drops.
...
- **Grouping principle: by subsystem module prefix** (gives each consolidated test a predictable `<file>::<test_name>` selector going forward; §Contract C1).
...
- nextest schedules per-`#[test]` across binaries; `test(/regex/)` filtersets match `<module>::<test>`, `binary(X)` matches the compile-unit name (confirmed `.config/nextest.toml:91-122`). Consolidation changes both halves of a former id — the binary id becomes `<crate>::harness_<subsystem>` and the test name gains a `<file>::` module prefix it did not have before (the moved files declare `#[test]` fns at file top level, no enclosing `mod`) — so a `test()`/`binary()` selector naming a former id would break; none does today, in-repo (§Contract C1). `binary()` filtersets are exactly why the 7 named binaries stay standalone.
...
- Naming: `crates/<crate>/tests/harness_<subsystem>.rs`; former `tests/<file>.rs` becomes `crates/<crate>/tests/harness_<subsystem>/<file>.rs`, declared with a **mandatory** `#[path = "harness_<subsystem>/<file>.rs"] mod <file>;` — a harness root is an integration-test *crate* root, so a bare `mod <file>;` there resolves against `tests/`, never the harness subdirectory; `#[path]` is the norm for every moved file, not a fallback (rationale, and the one exception for a retained `tests/` sibling such as the shared `common` helper: `tests/infra/test_harness_kloc_cap.sh` C1 header). This is **mechanically enforced**, not documented-only — that guard's §6 flags any bare `mod <ident>;` in a harness root whose crate-root-relative resolution does not land on a real retained sibling (which is the same condition that decides whether the declaration compiles, so the exception cannot drift into an allow-list). The submodule path **must equal** the former file stem — this fixes the post-consolidation selector as `<file>::<test_name>`; it does **not** preserve the pre-consolidation id (both the binary id and, for the top-level-`#[test]` files being consolidated, the test name itself change — full statement and measured example: `tests/infra/test_harness_kloc_cap.sh` C1 header).
...
**C2 — Anti-re-accretion drift guard** (`tests/infra/test_harness_kloc_cap.sh`, new; lands **before** any consolidation leaf; run-all-classification.manifest row in the same diff). Enumerates each consolidatable crate's `tests/*.rs`, asserts (a) each harness compile unit ≤ the kLOC cap, (b) every `tests/*.rs` is either a sanctioned harness or one of the 7 named overrides (no new gratuitous standalone binary re-accretes), (c) failure names the offending file. **The measured unit** under (a) is the root file, its own `harness_<subsystem>/` module directory, **and** the transitive closure of the `#[path]`/bare-`mod` includes that escape that directory (the shared `tests/common/` helpers) — rustc compiles a separate copy of such a file into every including binary, so those lines are that unit's real per-unit compile cost, which is exactly what this cap governs (resolution rules and the verdict's field grammar: that guard's C1/C2 header, not restated here). Emits a structured pass/fail line (not a log-scrape). It also carries the two C1-contract checks that would otherwise be prose-only: (d) the `#[path]` mandate above, over every `crates/*/tests/harness_*.rs` root on disk (§6); (e) BT-2's no-stale-selector invariant (§7). Both are live-tree scans, so the census behind each is re-derived per run rather than hand-counted.
...
| BT-2 | Test-id churn is closed and harmless | consolidate crate C's non-excluded `tests/*.rs` into harnesses | **Mechanically gated** — `tests/infra/test_harness_kloc_cap.sh` §7 derives the consolidated-stem set from disk and fails if any `binary(<stem>)` or `--test <stem>` atom in `.config/nextest.toml`, `scripts/`, or `tests/infra/` names a pre-consolidation compile unit; the only surviving `binary(X)` selectors are the 7 override binaries (BT-3), which is why they stay standalone rather than being consolidated. *Stated limit:* the bare `test(=<name>)` form is not statically decidable (test names exist only in a compiled `nextest list`) — BT-1's `#[test]`-fn count is what covers that axis, so BT-2 deliberately no longer restates it | consumer (nextest overrides) |
...
- **kLOC cap ≈ 10–20** per harness; drift-guarded (C2) so harnesses don't re-accrete into 1,103 again. Measured as **raw line count** (not sloc — simplest and conservative; settled in the B1 leaf).
- **Shared `tests/common/` includes ARE attributed to every unit that includes them** (task #5620) — per-unit, not amortised, because rustc compiles a separate copy into each including binary. The alternative, a permanent carve-out, was rejected: it leaves the C2 ratchet guarding a quantity it does not define, and it is directionally exploitable (move code into `tests/common/`, `#[path]`-include it, cap evaded).
- **An over-cap harness is resolved by SPLIT, never by raising the cap** (task #5620). Raising `CAP_LINES` above 20000 would contradict the ratified 10–20 kLOC band above and loosen the ratchet to fit its first offender. Concretely: attribution put `harness_topology_selector` at 21470 lines, and it was resolved by splitting `selective_demand` (which carried the 2128-line `common/differential.rs` include) into its own harness — 16786 + 4709, both under the cap.

...
| BT-2 | Test-id churn is closed and harmless | consolidate crate C's non-excluded `tests/*.rs` into harnesses | no `#[test]` fn is added or removed (I3, cross-checked with BT-1's count); no repo-resident selector (`.config/nextest.toml`, `scripts/heavy-test-filter-lib.sh`, or elsewhere) names a pre-consolidation id — the only `binary(X)` selectors in-repo are the 7 override binaries (BT-3), which is why they stay standalone rather than being consolidated | consumer (nextest overrides) |

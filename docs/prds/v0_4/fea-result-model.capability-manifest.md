...
| B9 end-to-end (per-case results off the shared realization) | `producer:4152` — assertion preserved verbatim as the `#[ignore]`d `multi_case_non_prismatic_body_caches_one_realization_for_both_cases` in `tests/solve_elastic_static_body_e2e.rs` (the `cases`-map / Sampled-field half only; the re-mesh-avoidance half is split out green, row above). Blocked on **#5951**: the `RepresentationWithin` bound that makes the counter observable also silently empties the solve's `cases` map, so B9's two halves need mutually exclusive fixtures today. | ⏳ blocked on #5951 |

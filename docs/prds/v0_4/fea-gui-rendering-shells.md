...
- **Backend event channel naming + convention seam-owned by `docs/prds/v0_3/gui-event-channel-inventory.md`** — this PRD adds payload extensions (`vonMises_top`/`vonMises_mid`/`vonMises_bottom`, `element_kind`, `region_tags`, `vector_channels`) to the existing `mesh-update` channel; per inventory §2.4 the payload extensions are owned here as ordinary kernel/IPC-types work, but the channel name + payload convention live in the inventory PRD. See also `docs/gui-event-channels.md`.

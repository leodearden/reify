...
2. **OOB surface-vertex policy.** ~~Sample at a vertex marginally outside the grid (tessellation vs grid-bound rounding): clamp to nearest in-bounds grid cell, or emit NaN? *Suggested:* small `tol` expansion on `locate_element_p1`, NaN beyond. Decide in δ.~~ **RESOLVED at #6154 (2026-08-10):** **emit `NaN`; clamping is rejected; the suggested `tol` expansion is rejected as the answer here** — the measurement says the mechanism is not round-off.
   ...
   *Hazard recorded for #5951 (do not fix here).* Two assertions in `crates/reify-eval/tests/solve_elastic_static_body_e2e.rs` — the `"displacement data must be non-empty and all-finite"` and `"stress data must be non-empty and all-finite"` pair — assert all-finite `displacement`/`stress` on the CYLINDER fixture. They are inert today, inside `multi_case_non_prismatic_body_caches_one_realization_for_both_cases`, which is `#[ignore = "blocked on #5951 …"]`. (Named, not line-numbered: the earlier `:1076`/`:1090` citations had already drifted by ~40 lines.) They contradict the normative row above: a cylinder's AABB corners are genuinely out-of-solid, and the control test measures 75% of that grid as correctly `NaN`. When #5951 unblocks that test, those two assertions will fail, and the correct resolution is to replace them with a bucket-split expectation — **not** to weaken the sentinel. Left untouched by #6154 deliberately: that test belongs to #5951's unblock path.

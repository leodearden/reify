...
declared `files`/lock scope MUST list the full set — they are not optional. The
...
**Task split (R3b-1 #4802 / R3b-2 #4818, /unblock 4802 2026-06-24).** The ~177
constructor edits are one *irreducible merge*: the moment the required field is
added, the whole `reify-eval` crate fails to compile (E0063) until every constructor
is fixed, and the `--scope all` merge gate compiles everything — so the field-add +
all constructors + the thread to `CheckResult.structured_detail` land as a single
~41-file `reify-eval` merge (**R3b-1**, signal B12). The GUI surface (`FeaDiagnosticInfo`
mirror + `GuiState.fea_diagnostics` read of `CheckResult.structured_detail`) is
*additively separable* — a written-but-unread `pub CheckResult` field compiles — so
it is peeled off as **R3b-2** (~3 files, signal B13). R3b-1 owns the channel up to
`CheckResult.structured_detail`; R3b-2 owns `CheckResult` → `GuiState`. Splitting does
not shrink the R3b-1 atom; it only lets B12 land independently of the GUI work.
...
| B11 | typed diagnostics → overlay | unconstrained body | overlay renders rigid-body arrows from `Vec<DofDirection>`; problem-element outlines from `Vec<ElementId>` | consumer (R3b-2 → ι) |
| B12 | structured detail rides outcome | unconstrained-body solve (Completed-warning) + near-degenerate solve (Failed) | `ComputeOutcome.structured_detail` carries `Fea(Unconstrained{rigid_body_modes:[…6]})` on the warned Completed; `Fea(ProblemElements{ids:[…]})` on the Failed (Rust engine test) | producer (R3b-1 #4802) |
| B13 | detail reaches GuiState IPC | GUI loads unconstrained-body fixture, solves | `GuiState.fea_diagnostics` is non-empty with the `Unconstrained` payload (debug-MCP `store_state`); failed solve clears `scalar_channels` but keeps `fea_diagnostics` | producer (R3b-2 #4818) |
...
5. **2930 stays a bracket with the full field-reduction design loop**, gated on producer-completion. No honest-scalar interim (Leo-ratified in 2930's parked note). The free-fn `face(body,"top")` half per GR-040 is sound — a real topology-selector (`units.rs:262,379`). **The objective grammar this decision originally ratified is not.**
   ...
   **CORRECTED 2026-08-25 (Leo-ruled).** This decision originally specified the objective as `minimize mass(body) where max(von_mises(fea.stress)) < material.yield_stress`, on the stated basis *"both parse, G3 verified"*. That basis was a **G3 PARSE check only** (`grammar-fixture:fea-result-model-2.ri`, 0 ERROR nodes — an ephemeral `/tmp` probe, never committed). Parsing was verified; **semantics never were.** All three clauses of that line were broken as measured at `main` `1040b07fe3`. Clause 2 has since been closed by #6577; its measurement is retained below as the historical symptom, not current state. Clauses 1 and 3 still hold — their *behavioural* measurements stay dated to `1040b07fe3`, while their line anchors were re-verified against this tree and repointed on 2026-09-11:

   1. **The `where` guard is silently discarded.** `crates/reify-syntax/src/ts_parser.rs:2738-2750` (`lower_minimize`, via `lower_where_clause` at `:2571`; the `lower_maximize` twin at `:2752`) lowers it into `MinimizeDecl.where_clause`, but the compiler's Minimize/Maximize arms (`entity.rs:3760`, `:3877`; `traits.rs:585`, `:590`) compile only `.expr` into an `ObjectiveTerm`, which has no guard slot (`reify-ir/src/constraint.rs:80-87`). The predicate registers **zero** constraints and emits **no diagnostic**. Measured: `constraint t < 20mm` + `minimize t where t > 5.0mm` resolves `t` to **1 µm** — the auto-param lower bound — where the `constraint` spelling of the same predicate resolves it to 12.5 mm. Every other member kind routes its guard through `compile_per_decl_guard` (`crates/reify-compiler/src/guards.rs:647`); these two are the hole. **Ruling (Leo, 2026-08-25): reject.** The compiler emits an Error; objectives take no guard in either form (`docs/reify-language-spec.md` §6.3 and §15 amended to match).

      *Ruling status, measured on this tree 2026-09-11:* the spec half landed, the compiler half has **not**. `min_decl.where_clause` / `max_decl.where_clause` still have zero readers anywhere under `crates/`, no rejecting diagnostic exists, and `tree-sitter-reify/test/corpus/minimize_maximize.txt` still carries passing `where`-form cases — so the behaviour above is **current state**, not history. #6575 is `done` (auto-closed `found_on_main` at `dc22d897a3` — a merge that landed one docs file, `crates/reify-mcp/src/tools/chunks/constraints.md`, while its own record's compiler-half "STILL TO DO" stayed unbuilt), so it is **not** a live owner and must not be cited as one. The live owner is **#6647** ("Implement the #6575-ruled compiler Error for minimize/maximize `where` guards"), `pending`/high, which carries #6575's unbuilt enforcement half forward.

   2. **`max(von_mises(fea.stress))` did not type-check** — `dimension mismatch in comparison: Real vs Scalar[kg*m^-1*s^-2]`. **CLOSED by #6577** (merged `929528831b`).

      *Symptom, as measured at `main` `1040b07fe3`:* `analysis_fn_result_type` lowered `von_mises`/`max_shear` as `scalar_or_real(tensor_quantity(args, 0))`, and `tensor_quantity` matched only `Type::Tensor` and `Type::Matrix`, so a `Type::Field` argument — the shape `ElasticResult.stress` carries — fell through to DIMENSIONLESS, and comparing that `Real` against a `Pressure` literal mismatched.

      *Fix, in-tree on main today:* `analysis_fn_result_type` (`crates/reify-compiler/src/analysis_signatures.rs:112`) gained an explicit `Type::Field` arm (`:119-139`, rationale at `:113-118`), gated by the `field_tensor_arg` shape check (`:204`) and consulted **before** the concrete-tensor ladder at `:141` — so `von_mises(.stress)` types as `Field<Point3<Length>, Scalar<Pressure>>` and `max` reduces that codomain to `Scalar<Pressure>`. Per-name arity and result forms: the Field-form table at `:78-89`; rationale at the module doc `:15-24`. With the eval half closed separately by #7129, `examples/fea_bracket_minimize_mass.ri:171-181` records in-tree that `max(von_mises(result.stress)) < yield_limit` "now compiles clean and evaluates for real".

      *Travels with #6001, not lost to it:* the Field arm is ported as `field_tensor_arg`/`reduce_tensor_arg` in `crates/reify-builtins/src/resolvers.rs`, which `tensor_scalar_reduction` consults before falling back to `tensor_quantity` (that module's `:141` doc — "Why the Field form is a `Field` and not a reduced scalar (task #6577)" — carries the ported rationale). Verified on `task/6001` @`3100d29849`, **in flight, unlanded as of 2026-09-11** — re-point when it lands.
...
- **R3b-1 — emit structured FEA diagnostics → `ComputeOutcome` → `CheckResult` (producer half of R3b; task #4802).** *Modules:* `reify-eval/src/engine_compute.rs` (new `StructuredComputeDetail` wrapper enum + `structured_detail` field on `ComputeOutcome::{Completed,Failed}`), `reify-eval/src/compute_targets/elastic_static.rs` (call `failure.structured_detail()` at the `:416` Completed-warning + `:708` Failed sites), plus the thread `run_compute_dispatch`→`EvalResult`→`CheckResult` (new `CheckResult.structured_detail` field) so the GUI half (R3b-2) can read it. **Scope: ~41 `reify-eval` files / ~177 mechanical constructor edits** (see §4.6 "Scope reality") — every `ComputeOutcome::{Completed,Failed}` literal across 16 src + ~21 test files gains `structured_detail: vec![]`; constructors are entirely within `reify-eval` (single-crate atom, no cross-crate ordering). NOT a 4-file change. *Signal (intermediate):* B12 (Rust engine test — `ComputeOutcome.structured_detail` / `CheckResult.structured_detail` carries `Unconstrained` on the warned Completed + `ProblemElements` on the Failed). *Prereqs:* 4090 (R3 classifier, done), γ/2962-line GUI dispatch wiring (4086, done). **High priority** — push the wide single-crate merge through reify-eval-core contention. Contract: §4.6. Task #4802.
- **R3b-2 — surface `CheckResult.structured_detail` → `GuiState.fea_diagnostics` (GUI consumer half of R3b; task #4818).** *Modules:* `gui/src-tauri/src/types.rs` (serde IPC mirror `FeaDiagnosticInfo` + `GuiState.fea_diagnostics` field, `#[serde(default)]`), `gui/src-tauri/src/engine.rs` (propagate the channel into `fea_diagnostics` on both the success-with-warning and failed-solve build paths; failed-solve clears scalar channels, keeps `fea_diagnostics` per §6.8). ~3 files; additively separable from R3b-1 (a written-but-unread `pub CheckResult` field compiles, so the GUI read lands as its own merge). *Signal (intermediate → unlocks ι):* B13 (debug-MCP `store_state` — `GuiState.fea_diagnostics` non-empty with the `Unconstrained` payload after an unconstrained-body solve). *Prereqs:* R3b-1 (#4802, provides `CheckResult.structured_detail`). `UnresolvedSelector` arm channel-plumbed, data-deferred to P2/4092. Contract: §4.6. Task #4818 (split from #4802 at /unblock 4802, 2026-06-24).
- **ι — 2966 diagnostic overlay.** *Signal (leaf):* B11 (rigid-body arrows + problem-element outlines render). *Prereqs:* R3b-2 (#4818), 2961. Re-dep 2966: `[2924,2929,2961,4090]` → `[R3b-2 #4818, 2961]` (R3b split into producer #4802 + GUI #4818 at /unblock 4802, 2026-06-24; 2966 consumes the GUI surface, transitively gated on #4802).
...
2. **OOB surface-vertex policy.** ~~Sample at a vertex marginally outside the grid (tessellation vs grid-bound rounding): clamp to nearest in-bounds grid cell, or emit NaN? *Suggested:* small `tol` expansion on `locate_element_p1`, NaN beyond. Decide in δ.~~ **RESOLVED at #6154 (2026-08-10):** **emit `NaN`; clamping is rejected; the suggested `tol` expansion is rejected as the answer here** — the measurement says the mechanism is not round-off.

   *Measured evidence* — `box(1000mm, 100mm, 100mm)` through the realized OCCT→gmsh→§7a path, in two tiers so a later reader can tell what a test still guards from what was a one-off run:

   - ***Pre-#6200 (2026-08-10), historical*** — the reading `solve_elastic_static_body_e2e::assert_box_grid_miss_measurement` took while the upstream coverage defect was still live: 1055 of 2989 grid nodes out-of-solid (35.3%), reconciling as **interior=36, face=749, edge=266, corner=4** under `classify_grid_misses`' index-extremity bucketing. It is **not** what a run reports today — see the resolution paragraph below, and read the per-run dump, never this line. (The instrument itself is still live and still re-measures on every run; only these numbers are historical.)

     That split is a **snapshot, not a constant** — read the per-run dump, never this line. Measured 2026-08-20 on one lane, same test binary, five runs: four reproduced 1055 / interior=36 / face=749 / edge=266 / corner=4 exactly, and one returned **1060 / interior=33 / face=757 / edge=266 / corner=4**. Test-thread count is ruled out: the full suite gave the 1055 split both at `--test-threads=1` and at the default parallelism. *Hypothesis:* gmsh/HXT's parallel tetrahedralization is not bit-reproducible under varying host load. The drift does not touch any conclusion below — the fill-fraction inequality, the margin distribution and the `missed_interior > 0` verdict all survive a wobble of ≤8 nodes in any one bucket (total +5, interior −3, face +8, edge and corner unchanged) — but it is exactly why **no test pins the count**, and why #6200's acceptance was recommended as a fill-fraction tolerance rather than an exact node count. That recommendation was adopted: #6200 shipped `crates/reify-kernel-gmsh/tests/volume_fill_fraction.rs`, which asserts `TetFillReport::aabb_fill_fraction` against a stated relative tolerance rather than any node count.
   - *One-off instrumented measurement*, from a temporary `nearest_miss_margin` probe in §7a that was reverted before landing (recorded verbatim in esc-6154-1's evidence; re-derivable at any time via `reify_solver_elastic::resample::nearest_miss_margin`, which stays `pub` in that `pub mod` for exactly this reason (it is deliberately NOT on the crate root: no build depends on it)): over all 1055 misses the nearest-miss barycentric margins run min **−5.02e-1**, p25 −1.38e-1, median **−8.42e-2**, p75 −3.58e-2, max **−1.63e-9** — so against the production `tol = 1e-9`, **exactly 1 of the 1055** lies inside `|margin| < 1e-8`. The 36 interior misses alone span −5.76e-3 … −2.80e-1. No test pins these numbers; the conclusion they support is independently proved by the volume inequality below.

   *Mechanism actually found — NOT a `tol` shortfall.* The PRD's own *Suggested* answer above was a `tol` expansion, and adopting it here would have been dishonest: at a median margin of −8.42e-2 in absolute barycentric units, no defensible tolerance reaches these points, and 1054 of 1055 are nowhere near the round-off band. The realized tet mesh instead **fails to cover the AABB it spans**: its 159 tets sum to 8.4291e-3 m³ against a 1.0000e-2 m³ AABB (84.3% fill, 0 interior nodes). Because `union(tets) ≤ Σ|tet vol|`, and because for a box the convex hull of the mesh's own boundary nodes *is* that AABB, the inequality is a **proof** of missing coverage, not a hypothesis. `volume_mesh_to_solver_mesh` is exonerated: it rejects a whole mesh on a bad index rather than dropping an element, and its only compaction touches tet-unreferenced vertices. The defect was upstream, in the gmsh tetrahedralization path (`crates/reify-kernel-gmsh`), and was filed as **#6200**; #6154 was re-scoped to observation-only rather than chase it across a crate boundary.

   *Since fixed — the figures above are the pre-fix record.* **#6200 has landed** (merged `6ed34b2fe8`, on `main`) and the realized box now measures **complete fill**: `n_missed = 0` of 2989 grid nodes, interior/face/edge/corner all 0, re-measured on this branch. The `missed_interior == 0` prediction — carried un-weakened through the wait as #6200's executable acceptance gate — is consequently a **live, un-`#[ignore]`d regression guard**, asserted inside `assert_box_grid_miss_measurement` itself. It briefly lived in a separate `realized_box_mesh_tiles_its_own_aabb` test that paid for a SECOND full OCCT+gmsh realization of the same fixture (~12 s) for nominal independence; since both paths went through the same `build_realized` on the same source and asserted on the same field, that independence bought nothing, and the claim was folded back onto the capstone's realization. Everything above stays because it is the historical record of *why* this question was resolved the way it was, and because the reasoning generalises: no `tol` can substitute for coverage. It is no longer what a run reports.

   *Policy.* The §4.1 row stands: finite inside the solid, `NaN` outside, on both `displacement` and `stress`. Both halves are measured: `assert_box_grid_miss_measurement` and `assert_cylinder_grid_miss_measurement` classify the stride-3 `displacement` and the stride-9 `stress` field of the same realization and pin that they mark the **same** grid points out-of-solid — §7a locates each grid point once (`resample_multi_nodal_to_grid`) and writes both fields from that one containment test, so a divergence is a real defect, and a `stress`-only weakening cannot slip past a displacement-only guard. Clamping to the nearest in-bounds grid cell is rejected outright — it would fabricate a value at a point the solver never solved, and would silently mask exactly the coverage defect above. A `tol` expansion remains available for *genuine* round-off (a surface vertex a few f32 ulp outside its own tet), and if one is ever needed it should be a named constant sized from a measured margin, not a bare literal; but it is **not** the answer to this instance and must not be introduced under cover of it. The cylinder control `assert_cylinder_grid_miss_measurement` (from the `non_prismatic_body_solve_runs_on_realized_volume_mesh` capstone) pins the sentinel where it must fire — every one of the 84 of 112 nodes (75%) the re-derived closed form places outside must carry it — so any future weakening of the sentinel fails loudly. The complementary half, that the realized cylinder mesh covers everything the closed form places *inside* (miss set exactly 84, split interior=0/face=40/edge=36/corner=8 — re-measured on this branch and still exact), is pinned in `realized_cylinder_mesh_covers_its_own_aabb`, which stays `#[ignore]`d on **drift and expense** rather than gating the queue: it pins an exact node count over a gmsh/HXT tetrahedralization that is not bit-reproducible, so one drifted node would red the merge queue, and it costs a full OCCT+gmsh realization of its own. Run it explicitly (`--ignored`) to re-measure.

   *Hazard from #6154 — **RESOLVED by #5951** (2026-08-26).* #6154 recorded a hazard here: two assertions in `crates/reify-eval/tests/solve_elastic_static_body_e2e.rs` — the `"displacement data must be non-empty and all-finite"` and `"stress data must be non-empty and all-finite"` pair — asserted all-finite `displacement`/`stress` on the CYLINDER fixture, inert only because they sat inside `multi_case_non_prismatic_body_caches_one_realization_for_both_cases`, which was `#[ignore = "blocked on #5951 …"]`. All three clauses of that record are now discharged. (Named, not line-numbered: the earlier `:1076`/`:1090` citations had already drifted by ~40 lines.) That test is **un-`#[ignore]`d and green** — #5951's root cause proved to be a premature per-template geometry redispatch latching the compute node out, **not** the tolerance contract its own filing named (that attribution is retracted in-tree). The all-finite pair is gone, and the resolution taken was **exactly the prescribed one**: a bucket-split expectation via `assert_cylinder_grid_miss_measurement`, the same helper the *Policy* paragraph above already cites for the single-case capstone. The sentinel was **not** weakened, the out-of-solid values were **not** clamped, and `tol` was **not** widened. Re-measured on that fixture — which, unlike the capstone's, additionally carries a 100 µm `RepresentationWithin` bound — both load cases and both fields report `axes=[4, 4, 7] n_grid=112 n_missed=84 (75.0%) | interior=0 face=40 edge=36 corner=8 | nonfinite_anomalies=0`, i.e. the closed form exactly, so the bound does not move the realized AABB. One liveness clause survives alongside the helper, and it is not an all-finite bar: `data.iter().any(is_finite)`, per field. The helper asserts only that predicted-outside nodes DO miss and leaves the excess direction logged (that being #6200's non-bit-reproducible territory), so an ALL-sentinel field would pass it — and an all-sentinel field is precisely the silent-empty solve #5951 fixed. The box-side sibling `multi_case_body_solve_survives_a_preceding_template` states the same contract through `assert_box_grid_miss_measurement`, whose `missed_interior == 0` already excludes an all-sentinel field, so it needs no such clause.

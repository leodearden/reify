...
- **`docs/prds/v0_3/engine-integration-norm.md`** (GR-017, landed 2026-05-12) — owns the engine-seam catalog. Under §4 mechanism-to-seam matrix, shell extraction plugs into **§3.4 ComputeNode dispatch** (the geometry-expensive realization-output heuristic in CN-contract §6 applies; threshold ≥~50 ms). This PRD intentionally does **not** add a §3.2 realization-kind dispatcher arm: the mid-surface mesh is consumed only as an upstream ComputeNode dependency of the FEA trampoline — never realized as a stand-alone user-visible Mesh entity — so §3.4-only is the minimum-viable seam coverage. (GR-017 §7.2 brief framed it as "§3.2 + possible §3.4 wrap"; the simplification stands because no user-level `realize_mid_surface(body)` path exists or is planned in v0.4.) This PRD's tasks γ/δ wire the producer-side `reify-shell-extract` symbols that G-tool currently flags as Type-A orphans (`scripts/audit-orphan-producers.sh` baseline 2026-05-12); once landed, no `// G-allow:` markers are required. The PRD's §5 (G1 checklist) is honoured: seam = §3.4; consumer call site = the FEA trampoline (`engine_compute.rs::solve_elastic_static_compute_fn`) at the upstream-dependency edge; user-observable signal = §9 task ι end-to-end leaf.
...
Per Leo's decision: shell extraction registers as a separate ComputeNode target. Under engine-integration-norm §3.4 + §4 row 4: this is the only engine seam this PRD plugs into (no §3.2 realization-kind dispatcher; the mid-surface mesh is never realized as a stand-alone user-visible Mesh).
...
2. **OpenVDB B-rep→Voxel resolution seam.** Parent shells PRD M-025 previously noted that `BRep→Voxel` sampling was "deferred to a separate follow-up" in `reify-kernel-openvdb::register`; that bullet no longer exists. Landed state (authorities, cited rather than restated to avoid duplicating their contract prose — SPOT: register.rs's "# BRep→Voxel (task 6560)" module-doc section; the parent shells PRD's 2026-09-01 Updates bullet): `BRep→Voxel` is served by the pre-existing two-stage `BRep --occt--> Mesh --openvdb--> Voxel` chain over the `(Convert { from: Mesh }, Voxel)` descriptor entry (`register.rs:169`), at a resolution the caller now chooses via `reify_ir::VoxelResolution` + `GeometryKernel::ingest_mesh_at_resolution` + `MeshToVoxelOptions::for_resolution`, reachable from a realized B-rep through `Engine::realize_solid_sdf_at`. Three caveats carry forward, each already filed: (a) the resolution is honoured only by that post-build DIRECT recipe, not by the dispatcher's conversion executor, which still voxelizes at the honest floor because its intermediate realization-cache entry is `NO_OPTIONS`-keyed (**#6989**); (b) the `SampledField` lowering remains DENSE under a 256Mi-voxel budget (**#6988**, near-duplicate **#6565**); (c) no `(Convert { from: BRep }, Voxel)` descriptor entry was added, deliberately — OpenVDB cannot read a B-rep handle and the descriptor is a feasibility table. What is still open, and is now this question's actual subject: `Engine::realize_solid_sdf_at` is `pub(crate)` (`crates/reify-eval/src/realize_solid_sdf.rs:121`), and its only non-test caller, `realize_solid_sdf`, delegates the fixed `VoxelResolution::HonestFloor` (`:50`) — every `MinFeature` / `TargetVoxelSize` call site in that file is a unit test — so task 6560 built the producer seam but wired no consumer. `pub(crate)` is not the actual barrier for this PRD's task δ: `shell_extract_compute.rs` lives in the same crate (`reify-eval`) as `realize_solid_sdf.rs`, so a same-crate caller could reach `realize_solid_sdf_at` directly today. The real gap is that no call site does — `shell_extract_compute.rs:518,550` maps `ElasticOptions.shell_voxel_size` to `MedialOptions.distance_tolerance`, labelled "(proxy)" in its own doc comment, which never reaches the voxelizer. That is not a second gap: **#6989** (pending), already cited above as caveat (a), covers it too in its own "related, same pass" note — once #6989 lands, `shell_extract_compute.rs`'s proxy mapping should drive `VoxelResolution::TargetVoxelSize` through `Engine::realize_solid_sdf_at` instead of the current medial-mask proxy. **Suggested resolution:** no new cross-PRD edge — until #6989 lands, task δ ships against a synthetic `SampledField` path exactly as before, now justified by the unwired-consumer gap rather than the retired producer deferral.

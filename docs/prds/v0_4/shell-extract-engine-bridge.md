...
2. **OpenVDB B-rep→Voxel resolution seam.** Task 6560 retired the producer deferral: `BRep→Voxel` sampling is no longer "deferred to a separate follow-up" in `reify-kernel-openvdb::register`. The landed chain, its resolution API, and two of three carried caveats are recorded in the parent shells PRD's 2026-09-01 Updates bullet (`docs/prds/v0_4/structural-analysis-shells.md`); the third caveat (no `(Convert { from: BRep }, Voxel)` descriptor entry, deliberately) is register.rs's "# BRep→Voxel (task 6560)" module-doc section alone. Cited here rather than restated (SPOT). The remaining gap is on the consumer side: `realize_solid_sdf` (`crates/reify-eval/src/realize_solid_sdf.rs:50`) delegates the fixed `VoxelResolution::HonestFloor` to `realize_solid_sdf_at` (`:121`), and no call site requests a thickness-scale resolution — including this PRD's own `parse_elastic_options_from_value` (`crates/reify-eval/src/shell_extract_compute.rs:518,550`), which maps `ElasticOptions.shell_voxel_size` onto `MedialOptions.distance_tolerance` as a labelled "(proxy)" that never reaches the voxelizer. `pub(crate)` is not the barrier — `shell_extract_compute.rs` is in the same crate as `realize_solid_sdf.rs` — the gap is simply that nothing calls through. **#6989** (pending) already owns this wiring; it is the same task as the linked Updates bullet's caveat **(1)**, not a second gap. **Suggested resolution:** no new cross-PRD edge — until #6989 lands, task δ ships against a synthetic `SampledField` path exactly as before, now justified by the unwired-consumer gap rather than the retired producer deferral.

...
- **2026-09-18 (task 6566 — shell voxel size: `thickness/3` retired, and the window has a CEILING):** This PRD's `thickness/3` figure is **withdrawn** in favour of a measured window of **`t/4` … `t/6`**, i.e. 4 to 6 voxels across the thinnest feature. The four body sites that carried the old number (Pre-conditions, Cost picture, Resolved design decisions, item 17) now read `t/4`. Measured basis, all reproduced first-hand on the task branch: **(1) `t/3` is REFUSED by the producer, not merely discouraged.** `MeshToVoxelOptions::for_resolution(plate_100x100x1, TargetVoxelSize(t/3))` → `Err(RequestTooCoarse { requested_voxel_size: 0.3333…, min_extent: 1.0 })`. On any thin plate the smallest bbox extent IS the thickness, so `t/3` can never clear `MIN_FEATURE_VOXELS_ACROSS = 4`; the figure is unreachable end-to-end by construction. **(2) The medial extractor is NOT the reason.** `min_wall_thickness` measures a 3-voxel wall bit-exactly (error exactly `0.0`) at all eight sub-voxel alignments — task 7527's `min_wall_thickness_is_alignment_invariant` sweeps `SWEEP_THICKNESS_VOXELS = [3, 4, 5]`. The PRD's open question "does the walk yield a usable estimate at 1.5 voxels of half-thickness?" is therefore answered YES; it is the producer, not the walk, that rules `t/3` out. **(3) NEW — there is a CEILING, recorded nowhere before now.** `compute_medial_mask` discards any voxel with `|φ| > narrow_band_half_width_voxels × spacing` (default `3.0`), and that filter runs BEFORE the 7527 walk-direction step, so a wall's mid-plane voxel (at `|φ| = half-thickness`) is dropped before the ridge fallback can see it. A wall is measurable only while voxels-per-thickness ≤ `2 × narrow_band_half_width_voxels` = **6**. Measured (analytic slab, h = 1, eight alignments): `Measured` with error exactly `0.0` at all eight for vpt ≤ 6; `NoMeasurement` at all eight for vpt ≥ 8; alignment-dependent in between (vpt 6.5 measures at 5 of 8, vpt 7 at 1 of 8) and deliberately not relied upon. **Refining past `t/6` destroys the measurement rather than improving it** — the opposite of what "finer is safer" would suggest. **(4) The gap this does NOT close.** The producer ACCEPTS over-refinement: `TargetVoxelSize(t/8)` returns `Ok(0.125)`, and the extractor then returns `NoMeasurement` with no diagnostic naming over-refinement as the cause. Nothing connects `MIN_FEATURE_VOXELS_ACROSS` (reify-kernel-openvdb) to `narrow_band_half_width_voxels` (reify-shell-extract), and a user setting `ElasticOptions.shell_voxel_size` finer than `t/6` hits exactly this. Closing it needs a new `VoxelResolutionError` variant plus a policy call about whether the voxelizer should know its consumer's band at all (today it deliberately does not — adding the dependency would invert the layering), so it is filed as separate follow-up work (curator ticket `tkt_0RTS8A7FBBCY8Y304P8957DG5H`; task id pending curation). Enforcing artifacts landed with this update: `crates/reify-shell-extract/tests/medial_resolution_window.rs` (the extractor's upper edge, derived from the constant rather than hard-coded), `crates/reify-kernel-openvdb/tests/mesh_to_voxel_resolution_tests.rs::the_prd_thickness_over_three_voxel_size_is_refused_on_a_plate` (the producer's refusal of `t/3`), and `crates/reify-eval/tests/harness_kernel_realization/shell_voxel_resolution_window.rs` (the cross-crate bracket asserting the window is non-empty at all).
- **2026-09-01 (task 6560 — BRep→Voxel gate disposition):** The resolution clause of the "Pre-conditions for activating" OpenVDB bullet is now **satisfied**. `reify_ir::VoxelResolution` + `GeometryKernel::ingest_mesh_at_resolution` + `MeshToVoxelOptions::for_resolution` let a caller that knows its thinnest feature request `MinFeature(t)` and get `h = t/4` — finer than this PRD's ≈ thickness/3, and the coarsest value clearing OpenVDB's measured interior-signing floor (half-thickness ≥ 2×voxel_size); at `t/3` the interior can fail to sign negative at all, which would empty-mask the medial walk. That grid is reachable from a realized B-rep via `Engine::realize_solid_sdf_at`. The B-rep→Voxel path itself is unchanged and pre-existing: the two-stage `BRep --occt--> Mesh --openvdb--> Voxel` chain over the descriptor's `(Convert { from: Mesh }, Voxel)` entry. Two caveats carried forward, both filed: **(1)** the resolution is honoured by the post-build DIRECT recipe, NOT by the dispatcher's conversion executor, which still voxelizes at the honest floor because its intermediate realization-cache entry is `NO_OPTIONS`-keyed — making that stage options-carrying is the ESC-3433-117 aliasing hazard and needs its own cache-key work (task **#6989**). **(2)** the lowering to `SampledField` remains DENSE, bounded by a 256Mi-voxel budget checked in Rust before any FFI allocation; the ≈10⁷ sparse-active-voxel projection below (a 1 m part at 0.1 mm voxels) is four orders over that ceiling and is now REFUSED BY NAME rather than allocated for and mislabelled (tasks **#6988**, near-duplicate **#6565**). The v0.4 shell-mesh-coarse working point is unaffected: the PRD's own 100 mm plate with a 1 mm feature implies ≈ 2.0M dense voxels, ≈ 130× under budget.
...
- v0.2 multi-kernel work has shipped the OpenVDB integration: `ReprKind::Voxel` realizable from B-rep via the dispatcher's conversion chain at resolutions sufficient for shell-mesh-coarse use (≈ thickness/4 voxel size for the thinnest expected feature; the admissible band is t/4 … t/6 — see the 2026-09-18 update).
...
Cost picture: OpenVDB sparse narrow-band only allocates voxels near the surface (~10⁷ active voxels for a 1m part with 0.1mm voxels and 0.1m² of thin-sheet surface — orders of magnitude smaller than dense or uniform-octree). Resolution defaults to thickness/4 for the thinnest expected feature; user-overridable via `ElasticOptions.shell_voxel_size`. That override is bounded on BOTH sides: coarser than t/4 is refused by name, and finer than t/6 is accepted but silently unmeasurable — the medial extractor's narrow band drops the mid-plane voxel and every measurement degrades to `NoMeasurement` with no diagnostic. See the 2026-09-18 update.
...
- **Voxel resolution auto-default.** `thickness/4` is the lean for the thinnest expected feature (measured; `thickness/3` was the original lean and is unreachable — see the 2026-09-18 update), but "thinnest expected feature" requires either user hint or a coarse-voxelization-and-iterate pre-pass. Decide during implementation.
...
17. `ElasticOptions` extension: `shell_threshold` (default 0.2 thickness/extent ratio), `shell_voxel_size` (default thickness/4 of thinnest feature, admissible band t/4 … t/6, with iterative auto-default fallback), `shell_branch_prune_ratio`, `shell_force` (off/auto/on, settable globally and per-body via annotation).

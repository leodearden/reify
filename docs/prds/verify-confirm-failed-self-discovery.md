...
| Attempt-0 tree-OID sidecar | **exists, but merge-role-gated only** | `add_test_passes`'s sidecar-stamp guard (`verify.sh`): stamped **only** when `DF_VERIFY_ROLE = "merge"`. Never fires for `DF_VERIFY_ROLE=offline` — only the *pattern* is reusable, not the file. |

...
| **Persist tightened set to `metadata.files` on the success branch** (the δ residual) | `producer:task-δ` (DF) — substrate: success path `workflow.py` `self.modules = plan_modules` is in-memory only; the requeue branch `scheduler.py` `update_task(... {'files': needed})` is the **pattern to mirror** onto the success path | **PASS** (producer = δ; substrate confirmed) |

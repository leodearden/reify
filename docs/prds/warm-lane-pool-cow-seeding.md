...
13. **Delta-touch-base substantiation** *(added 2026-07-27 — task 5632)* — under `--fresh-checkout`, when all three delta-touch-base resolution tiers (`--base-commit` / `<base_target_dir>.basecommit` / `.warm-base-meta BASE_COMMIT`) come back empty, the seed MUST REFUSE rather than return a lane whose freshness claim it cannot substantiate. The seed's contract with its caller is: empty stdout ⇒ fall back cold; non-empty stdout ⇒ this lane is safe to build warm. A no-base seed cannot make the second claim, because the delta-touch is the ONLY mechanism that identifies which sources differ from the base commit — with no base, that set is uncomputable.

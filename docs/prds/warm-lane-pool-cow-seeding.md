...
- **D9 — Pool size derives from `max_concurrent_tasks` at startup** *(Leo, 2026-06-17; corrected 2026-07-12 — task 5177, `warm-lane-pool-sizing-lifecycle.md` §2)*. N = the orchestrator's effective task-lane pool size, derived **once at startup** as `max_concurrent_tasks + spare_warm_lanes` (`orchestrator.yaml`, currently 48 + 8 = **56**), **not** a hardcoded constant — if either knob is retuned, the pool tracks it. Total lanes = N (task, = `max_concurrent_tasks` + `spare_warm_lanes`) + K (merge-spec, = `_MERGE_AHEAD_BOUND`).

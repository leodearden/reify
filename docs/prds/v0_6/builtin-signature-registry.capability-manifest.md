...
|---|---|---|---|
| α | parse fns registered+typed today (seed family) | wired — `crates/reify-builtins/src/registry.rs:14` `registry!` rows `ParseLength`/`ParseLengthR`; compiler seam `crates/reify-compiler/src/builtin_registry.rs:58` `registry_result_type`; exhaustive eval arms `crates/reify-stdlib/src/registry_dispatch.rs:79`. **Superseded 2026-08-30 by α itself** — the pre-α evidence was `crates/reify-compiler/src/parse_signatures.rs:22` `PARSE_FN_NAMES` + eval arm `crates/reify-stdlib/src/parse.rs:17` (verified 2026-08-04 against main `5fb3959fbb`), but α deleted `parse_signatures.rs` outright and removed `parse.rs`'s `eval_parse` string matcher, so those anchors are historical, not live. Regression fixture `tests/prd-gate/fixtures/parse_length_match_resolves.ri` stays non-probative for registration (see D3 note) | PASS |
| α | analysis fns registered (seed family) | wired — `crates/reify-builtins/src/registry.rs:14` `registry!` rows `VonMises`/`MaxShear`/`PrincipalStresses`/`SafetyFactor`/`StressInvariants` (5 names); exhaustive eval arms `crates/reify-stdlib/src/registry_dispatch.rs:79`. **Superseded 2026-08-30 by α itself** — the pre-α evidence was `crates/reify-compiler/src/analysis_signatures.rs:38` `ANALYSIS_FN_NAMES` (verified 2026-08-04 against main `5fb3959fbb`), a file α deleted. | PASS |

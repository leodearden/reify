...
2. **`in_frame(v, f)` = express `v` (given in frame `f`'s coordinates) in the current scope frame:** returns `f · v`. Typical use: `in_frame(bush.bore_axis, bush.world_frame)` — a sub's local datum in assembly coordinates. The inverse direction composes with existing frame ops (the `frame_to_frame` arm of `eval_geometry`, `crates/reify-stdlib/src/geometry.rs:512-575`); no second builtin in v1.
...
| kernel tasks #5342 (helix), #5343 (pipe orientation), #5385 (generate-of-geometry) | consumes | λ ← #5385 (hard); μ ← #5343 — `pipe()` used directly; #5342 not needed (belt paths are planar) | kernel tasks own their fixes | #5342 in-progress, #5343 done, #5385 pending |
...
**DAG:** α→{γ,η,ι,μ} · β→{ζ,μ,ν,ξ} · δ→μ · ε standalone · ζ→{ι,ν,ξ} · η→{θ,ι,μ} · ι→{μ,ν,ξ} · κ→λ · λ→{μ,ν,ξ} · {ν,ξ}→ο. Out-of-batch: λ←#5385 (hard) · trap (b) referenced to #4388 (no edge — no leaf here depends on the ledger).

...
| `pipe()` non-+Z paths | DONE #5343 — `pipe()` accepts any finite start-tangent; the `sweep` posed-profile fallback is no longer needed | `crates/reify-ir/src/geometry.rs:834–853` (`GeometryOp::Pipe` doc) |
...
| kernel tasks #5342 (helix), #5343 (pipe orientation), #5385 (generate-of-geometry) | consumes | λ ← #5385 (hard); μ ← #5343 — landed, `pipe()` used directly (no fallback needed); #5342 not needed (belt paths are planar) | kernel tasks own their fixes | #5342 in-progress, #5343 done, #5385 pending |
...
**7.1.5 Instance values carry their frame (the member-read frame rule — ratified by Leo 2026-07-25, seam review).** A structure-instance *value* intrinsically carries a frame, and member geometry reads compose through it: `x.body` yields the body posed by `x`'s carried frame. A sub instance carries its placement (explicit `at` ∘ relate-solved); an instance constructed in-scope (`let g = Kid(...)`) carries the identity frame. Thus "sub-member reads are scope-posed, let-instance member reads are local" holds — but as **corollaries of one uniform value-level rule, not as a path-based special case**. The rule is alias-stable by construction: `let h = self.a; h.body` ≡ `self.a.body` (the alias carries `a`'s frame); a path-based rule would silently change the frame of every downstream value under that innocent refactor — an INV-SF-7-adjacent silent-wrong-value hazard, decisive now that uniform-member-access makes aliasing idiomatic. Implementation notes: `Value::StructureInstance` gains a frame component; **content-hash/caching keys MUST include the carried frame** (else geometry dedup returns cross-posed false hits) — β (#5436) inherits this requirement; `.world_frame` *is* the carried frame composed to the reading scope (one mechanism, not two); a frame travels with an instance passed into a `fn` (functions can observe placement — intended: the frame is data). Consumers: indexed-sub γ (#5483 — its `distance(ps[0].body, ps[1].body)` scope-posed signal is this rule applied), UMA resolver (#5424 — D4's "local-frame member reads" is superseded for instance-valued receivers by this section), `member_sub_geom_baseline.ri` (existing cross-sub boolean pins must be consciously re-pinned under this rule, never silently migrated).
...
- **F2 (resolved)** — `pipe` +Z restriction (#5343, landed): `pipe()` now sweeps the flat belt path into a solid directly; the posed-profile `sweep` workaround is no longer needed.
...
- **δ — zero-auto relate verification (trap a).** Modules: reify-eval/relate_solve.rs (the :836 filter), diagnostics. *Signal (B1/B2):* fixture emits `W_RELATE_NO_AUTO`; violated ⇒ error, exit ≠ 0. *Prereqs:* none. **[DROPPED at decompose — superseded by DIC α #5415; ratified 2026-07-25.]**
...
**DAG:** α→{γ,η,ι,μ} · β→{ζ,μ,ν,ξ} · δ→μ · ε standalone · ζ→{ι,ν,ξ} · η→{θ,ι,μ} · ι→{μ,ν,ξ} · κ→λ · λ→{μ,ν,ξ} · {ν,ξ}→ο. Out-of-batch: λ←#5385 (hard) · trap (b) referenced to #4388 (no edge — no leaf here depends on the ledger). (#5343 has landed — μ's prior soft, no-edge dependency on it and its fallback note in μ's text are resolved; no DAG edge was ever added.)
...
1. **`W_RELATE_NO_AUTO` severity** — warning vs info when the assertion holds (B2). Suggested: warning. Decide during δ. *[Moot — δ dropped, warning not implemented; ratified 2026-07-25.]*
...
5. ~~**μ tendon-solid route** — `sweep` posed-profile vs waiting on #5343 `pipe`; decide at μ dispatch by checking #5343 status.~~ *[Resolved 2026-07-29: #5343 landed; μ uses `pipe()` directly, no dispatch-time check needed.]*

...
4. **`maximize` against a `<= 40mm` bound returns `24mm`**; `minimize` against `>= 8mm` returns `8.8mm`. Objectives look soft-penalised rather than bound-seeking. Needs triage before it is called a bug. *(Triaged 2026-08-28 by #6756 — `docs/notes/objective-seed-parking-triage-2026-08-27.md`. **It is a bug.** Both numbers are exactly the seed, returned by the silent seed-fallback at `crates/reify-constraints/src/solver.rs:1997-2031`: the penalty method's optimum sits `1/(2·PENALTY_WEIGHT)` = 5e-7 outside the active bound, is rejected against `FEASIBILITY_THRESHOLD = 1e-12`, and the untouched seed is returned as `Solved` with no diagnostic — the clamp that would snap the undershoot back is gated on `floor_applied`, i.e. Money-only. Not soft-penalisation: the answer tracks the seed bit-for-bit. **Wording correction —** the `24mm` came from a **two-sided** `8mm..40mm` shape (the derived-box midpoint seed); a genuinely one-sided `<= 40mm` returns **`36mm`** (= `40mm − 0.1 × 40mm`, the one-sided seed nudge), so this is one defect, not two. No new fix task filed: loudness → **#6654** arm 3; the `floor_applied` clamp gate → **#5711**, which `solver.rs:1801-1809`/`:2605-2612` bind jointly to the `verify_uniqueness` contract; the 5e-7 root cause → **#6678** (κ, which absorbed the cancelled #6688). **Severity —** `minimize` and `maximize` over the same two-sided problem return *bit-identical* answers, so this is a correctness defect, not a tolerance one. **Boundary with item 3 —** the seed is returned only when the objective points *toward* a derived bound; pointing *away*, the optimiser runs to the `default_bounds_for(Length)` corner instead — measured 10 m for `maximize` under `x >= 8mm` (item 3's case) and 1e-6 m for `minimize` under `x <= 40mm` (item 3's unnamed mirror image, recorded for #6655/#6692).)*

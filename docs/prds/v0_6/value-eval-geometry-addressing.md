...
- ✅ Fire when a **geometry-typed consumer** builtin's required geometry/selector argument fails to resolve — `adjacent_faces(b, <unresolved-selector>)`, `normal(b, pt)`, `closest_point(b, pt)`, `centroid(b)`, `volume(b)`, `area(b)`, `length(e)`, `distance(b1, b2)`, … — any builtin that requires a realized kernel handle. (Post-R3 these will also cover field-valued consumers like `peak_deviation(track, <unresolved-selector>)`, `displacement_at(h, <unresolved-selector>, dir)` once selector→node resolution is wired.)
...
- **R1a — value-eval `E_EVAL_UNRESOLVED` emission at typed-consumption sites.** Detect: a geometry-typed consumer builtin (`adjacent_faces`, `normal`, `closest_point`, `centroid`, `volume`, `area`, `length`, `distance`, …) on the pure value-eval surface receives a geometry/selector-typed arg that cannot be resolved (stays `Value::Undef` post-eval); emit `E_EVAL_UNRESOLVED` (error) with the offending let-decl span. Scope-guard against non-consumed editor incompleteness. *Signal (as implemented, task #4651):* `reify check` on a geometry module with `adjacent_faces(b, top_face)` / `normal(b, pt)` emits `E_EVAL_UNRESOLVED` at Error severity (was: silent `Undef`); LSP `eval_cached` on an un-consumed `let top_face = single(faces_by_normal(b,…))` emits **nothing** (editor-incompleteness guard). *Note:* `reify eval` routes geometry-bearing modules through `build()` (which calls `check()` then `run_unified_pass`), so `reify check` is the CLI witness surface for R1a — not `reify eval`. *Post-R2/R3 note:* once selector→node resolution is wired (Rung 3), field-valued consumers like `peak_deviation(track, faces_by_normal(box(),…))` will also produce content-stable signals on the eval/check surface. `grammar_confirmed=true`.
...
- **R3d — dependency-ordered selector/handle resolution (the TIMING axis, shared leaf, task #4900).** Mint each geometry handle and topology selector at its topological slot in the eval walk — AFTER its upstream params are resolved, BEFORE any consumer reads it — so that value-eval consumer cells (e.g. `let n_top = loc`) see a finite minted `Value::Selector` / `Value::GeometryHandle` at READ time rather than `Value::Undef`. Applied at all three eval entry points: `Engine::eval`, `eval_cached`, `engine_edit`. *Signal (as implemented, task #4900):* `reify eval` / `eval_cached` / `edit_param` on a module whose value-eval cell consumes a kernel-free minted selector returns a finite resolved value instead of silent `Value::Undef`, verified by the kernel-free test suite in `value_eval_consumes_minted_selector.rs`. *Dependency shape:* R3c (task #4655) depends on R3d (the TIMING fix must land before the integration-gate e2e can pass); task #4370 AXIS-2 (`@optimized` FEA max_von_mises>0 witness) **reuses** R3d's relocation without re-implementing it.
- **R3e — same-pass consumer re-eval after in-walk selector mint (the residual TIMING gap, task #4907).** Inside a `structure def` template walk, a consumer that is *also* downstream of an `@optimized` compute-node dispatch in the *same* pass (e.g. `peak = peak_deviation_at(track, loc)`, where `track` is an `@optimized` compute node and `loc` is an R3d in-walk-minted selector) can be scheduled BEFORE the mint that resolves `loc` fires elsewhere in the walk — R3d only fixes topo-ordered-AFTER reads, so this consumer reads a stale pre-mint `Value::Undef` and is never re-checked. Root-caused by the #4655 steward under esc-4655-120 (release repro). Fix: each walk tracks cells the R3d in-walk mint retry actually resolved (`minted_in_walk`), then runs a scoped post-mint re-eval pass (topo order, trigger-set expansion) over Undef/non-auto/has-default cells — excluding `@optimized` `UserFunctionCall`s — whose dependency trace reads a minted cell, via the cache-ful `reeval_cone_cell` write-back. Applied at all three eval entry points, inside each one's own walk. *Signal (as implemented, task #4907):* the R3e trio in `value_eval_consumes_minted_selector.rs` (mirroring R3d's) — a same-pass consumer of both an `@optimized` compute node and an in-walk-minted selector resolves to a finite non-Undef value through `Engine::eval`, `eval_cached`, and `engine_edit`. *Dependency shape:* depends on R3d (extends its `minted_in_walk` bookkeeping); unblocks R3c (task #4655's integration gate hits this same-pass inversion in the printer scenario).
- **R3f — post-walk mint consumer re-eval for geometry-LET-backed selector targets (the residual R3e gap for LET-backed targets, task #4946).** A geometry LET (`let loc_box = box(...)`) lowers to a realization with no value cell of its own (`entity.rs:1327-1330`), so its `GeometryHandle` — and any selector target it backs (e.g. `loc = faces_by_normal(loc_box, dir, tol)`) — resolves ONLY via the POST-WALK whole-module mints (`mint_symbolic_geometry_handles_into_values` then `mint_symbolic_topology_selectors_into_values`), which run after the per-template walk R3d/R3e instrument. Such a target never enters `minted_in_walk`, so R3e's post-mint re-eval — keyed on that set — structurally cannot rescue a same-pass consumer (e.g. `peak = peak_deviation_at(track, loc)`) reading it; the consumer stays `Value::Undef`. Fix: `mint_symbolic_topology_selectors_into_values` returns the `HashSet<ValueCellId>` of cells it flipped Undef→non-Undef, fed into the SAME `re_eval_consumers_of_in_walk_mints` / `_from_graph` R3e already uses, as an additional call site keyed on the post-walk flip instead of the in-walk mint — `mint_symbolic_geometry_handles_into_values`'s flips are deliberately never tracked or fed in at all (feeding them in would permanently pin a direct, non-selector consumer of a bare geometry LET's placeholder symbolic handle to a symbolic-only result, preempting `build()`'s later real-kernel post-process passes; regressed `restrict_field_b5_integration` when tried, per the exclusion rationale at each of the three call sites). Applied at all three eval entry points, immediately after each one's post-walk mint calls (`engine_edit`'s via `edit_source`, which owns `module` and the post-walk mints — `edit_param` has neither and cannot mint a geometry-LET handle at all). *Signal (as implemented, task #4946):* the R3f trio in `value_eval_consumes_minted_selector.rs` (mirroring R3d/R3e's) — a same-pass consumer of a geometry-LET-backed selector target resolves to a finite non-Undef value through `Engine::eval`, `eval_cached`, and `Engine::edit_source`. *Dependency shape:* residual of R3e/#4907 (same consumer-re-eval machinery, different trigger source); unblocks R3c / task #4655's integration gate (the printer scenario's `loc_box`-style geometry-LET targets).

**Dependency shape:** R1a independent (lands first). R2a → R2b. R3d shared leaf (TIMING axis); R3e depends on R3d (residual same-pass gap); R3f depends on R3e (residual LET-backed-target gap, same machinery); R3a; R3b depends on R2b + R3a; R3c depends on R3b + R3d + R3e + R3f; 4370 AXIS-2 reuses R3d. 4122's existing edges (3823, 4092) flow into R3a/R3b. Cross-PRD: coordinate the shared `E_EVAL_UNRESOLVED` with the unified-build-dag cutover (η/θ) per §6 — no hard edge, but the no-double-fire boundary is a decompose-time check.

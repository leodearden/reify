...
- **Grammar fixtures** (committed at `tests/prd-gate/fixtures/sketch_*.ri`, currently FAIL — they are task α's target): the block form, `auto(seed)` in call position, `aux let` inside a sketch, bare constraint members.
...
- **α — Grammar + AST: `sketch` block, `auto(seed)`, lowering.** `tree-sitter-reify` (grammar, corpus, fixture tests), `reify-syntax/ts_parser.rs`, `reify-ast` (`SketchDecl`). Signal: the committed `tests/prd-gate/fixtures/sketch_*.ri` fixtures parse with 0 ERROR nodes; corpus tests assert the CST; unlocks γ. (intermediate)

...
| `placement-relations-belt.md` (5435–5448) | supersedes-in | belt's δ leaf (zero-auto relate verification, `W_RELATE_NO_AUTO`) was **dropped at decompose, superseded by this PRD's α #5415** (belt capability-manifest "Batch-shape changes" §1); belt μ #5445 hard-deps 5415; belt η/ι (#5441/#5443) populate δ #5418's `IndeterminateReason` carrier if landed — coordinate, don't duplicate | **this PRD (α)** owns zero-auto relate semantics (silent satisfied pass + ζ ledger row); belt keeps solver/whitelist mechanisms | added by seam review 2026-07-25 — reciprocal of the belt-side manifest record |

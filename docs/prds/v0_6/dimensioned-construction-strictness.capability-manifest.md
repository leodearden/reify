...
| the alias-extension requirement is real (registry-only sweep under-counts) | `grep:crates/reify-core/src/dimension.rs:514` `NAMED_DIMENSIONS`; PRD §6.2 lists 16 `.ri`-declared dimensioned aliases absent from it (`Torque` `stdlib/ports_mechanical.ri:29`, `Stress` `stdlib/analysis.ri:13`, …) (**`Torque` cite retired by η**, task 5785, merge `61300d09`: the alias is gone from `ports_mechanical.ri` and `Torque` is now a `NAMED_DIMENSIONS` entry — re-measured 2026-08-03 at `dimension.rs:576-663`, 52 rows / 51 distinct `DimensionVector`s. The alias-extension count is now **15**, not 16; `Stress` and the other exemplars are unaffected and the under-counting claim still holds.) | PASS (accurate when written; `Torque` exemplar retired by η, count now 15) |

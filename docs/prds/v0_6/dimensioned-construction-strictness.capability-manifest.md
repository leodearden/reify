...
| Capability | Evidence | Verdict |
|---|---|---|
| the corpus gate α drives exists, is severity-blind and accumulates every hit | `grep:crates/reify-compiler/tests/harness_compilation_surface/examples_smoke.rs:198` (`no_example_emits_ctor_field_conformance_diagnostics`), `≥40`-file floor at `:215-220`, one-panic accumulation at `:230-236` | PASS |
...
| the alias-extension requirement is real (registry-only sweep under-counts) | `grep:crates/reify-core/src/dimension.rs:576` `NAMED_DIMENSIONS` (**re-anchored 2026-08-03**; the original evidence was taken at `:514` pre-η, and at HEAD that line is inside `DimensionVector::to_display_units` (`:504`), not the registry); PRD §6.2 lists 16 `.ri`-declared dimensioned aliases absent from it (`Torque` `stdlib/ports_mechanical.ri:29`, `Stress` `stdlib/analysis.ri:13`, …) (**`Torque` cite retired by η**, task 5785, merge `61300d09`: the alias is gone from `ports_mechanical.ri` and `Torque` is now a `NAMED_DIMENSIONS` entry — re-measured 2026-08-03 at `dimension.rs:576-663`, 52 rows / 51 distinct constant identifiers / **49** distinct `DimensionVector` values — three alias pairs share a vector: `STIFFNESS`/`TRANSLATIONAL_STIFFNESS`, `ABSORPTION_COEFF`/`CURVATURE`, and `IMPULSE` registered twice as `"Impulse"`/`"Momentum"`. The alias-extension count is now **15**, not 16. The other exemplars all still exist — but three of their four line cites in §6.2 have since drifted, re-measured there; use §6.2's re-measured cites, not the originals. The under-counting claim still holds.) | PASS (accurate when written; `Torque` exemplar retired by η, count now 15) |
...
| the corpus gate's panic prose encodes the **old** ruling (the §4.3 trap is real) | `grep:crates/reify-compiler/tests/harness_compilation_surface/examples_smoke.rs:230-236` — read verbatim at HEAD: *"a false positive from the conformance walker, not a broken example… do NOT add a SKIP_SET entry"* | PASS |
...
| the deliberate `Int`-leniency pin to restate (not merely delete) exists | `grep:crates/reify-compiler/tests/harness_statement_semantics/constraint_def_compile_tests.rs:1490-1522` `int_literal_for_length_param_no_constraint_arg_type_mismatch`, whose doc comment states the leniency as **intent** | PASS |
...
| ζ₂ | the corpus dir is compile-gated by directory walk (no new registration due) | `grep:crates/reify-compiler/tests/harness_compilation_surface/examples_smoke.rs` walks `examples/` recursively; `INDEX.md` states it verbatim | PASS |
| ζ₂ | the bidirectional INDEX invariant test exists | `grep:crates/reify-compiler/tests/harness_compilation_surface/examples_smoke.rs:548` `best_practices_index_matches_corpus_directory` | PASS |

...
- **ρ #6778 — Static-semantics rejection wave (§3, §4, §7, §8).** Typed-code reject fixtures (needs ι); per-clause coded emissions migrated under κ as reached. *Signal:* per-clause verdicts with typed codes; every asserted rejection observed to fire.

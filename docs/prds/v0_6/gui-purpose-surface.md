...
**Ownership:** this is `check-diagnostic-truthfulness.md` territory under the binding G4 ruling (§7), and **#5748 is in flight on exactly it** — its own charter names *"(c) `--purpose` (:693-824): unconditionally `Engine::new(checker, None).eval(&compiled)` … Same defect as (a)"*. #5748 routes that arm through `with_registered_kernel` + build, but does **not** add `set_capture_repr_tol(true)` + `tessellate_realizations` — so the `RepresentationWithin` residue survives it. **Leaf ι files that residue as a finding against PRD 2.** This PRD fixes none of it. *(Superseded 2026-08-27: the residue was adopted by driver-contract α #6773 as an explicit acceptance clause — `driver-contract-implementation.md` §8.3, commit `1d487a7d1f`. Leaf ι now only re-verifies, and files against driver-contract, if the defect escapes all of #5748/#6803/#6773.)*
...
|---|---|---|---|---|
| **`check-diagnostic-truthfulness.md` — #5748 (`in-progress`, claimed) *(status note 2026-08-27: #5748 is pending/unclaimed at merge-phase; statuses in PRD prose rot — see the task store)*, #5403 (`pending`)** | **reserved territory** | `cmd_check` routing, `finish_check`, build-diagnostic collection, exit codes, `--strict` | **that PRD, by a binding G4 ruling** (*"PRD 2 ONLY — no contested ownership"*) | **edge: α ← #5748.** α is behaviour-preserving and runs *after* #5748's rewrite of the same function. The §2.3 measurement residue is **filed to** PRD 2 by leaf ι — not fixed here. *(Superseded 2026-08-27: residue adopted by driver-contract α #6773 per `driver-contract-implementation.md` §8.3; ι files only if it escapes #5748/#6803/#6773.)* |
...
- **All `cmd_check` semantics** — routing, exit codes, `finish_check`, `--strict`, diagnostic collection. Reserved to `check-diagnostic-truthfulness.md` by binding G4 ruling; #5748/#5403 in flight. The §2.3 measurement residue is **filed to** that PRD by leaf ι, not fixed here. *(Superseded 2026-08-27: residue adopted by driver-contract α #6773 per `driver-contract-implementation.md` §8.3; ι files only if it escapes #5748/#6803/#6773.)*

...
_Convention: an item in this list that is resolved after authoring is **dispositioned in place** — marked RESOLVED with a date/session and a cite to where the decision is recorded — never deleted or renumbered, because tracked source cites these items by ordinal: `crates/reify-compiler/stdlib/modal_analysis.ri` ×2 (≈:43, ≈:343), `crates/reify-compiler/stdlib/modal_analysis_fns.ri` (≈:31), and `crates/reify-compiler/tests/harness_mechanics/modal_options_validation_tests.rs` (≈:2515) — all citing "§12 open-question-3". This list is therefore **append-only**._

...
_Convention: an item in this list that is resolved after authoring is **dispositioned in place** — marked RESOLVED with a date/session and a cite to where the decision is recorded — never deleted or renumbered, because tracked source cites these items by ordinal (e.g. "§12 open-question-3"); grep `§12 open-question-` across `crates/` for the current site set rather than trusting a snapshot list here, since a new citing site would otherwise leave an enumeration silently incomplete. This list is therefore **append-only**._
...
3. **`Part` shape** — opaque handle vs structured (geometry + material + topology index). **RESOLVED (Leo, 2026-08-31, Part-investigation session).** `Part` stays an **opaque nominal marker** (part identity is a join key, not a payload); the growth path is **convergence, not enrichment** — when modal analysis gains a real-geometry input, `ModalResult.part` is populated from the same `GeometryHandleRef` identity that `CarriedTopology.part` already carries (`crates/reify-eval/src/compute_targets/result_topology.rs`), so the structured geometry+material+topology alternative named above is explicitly **not** taken. Canonical record: `docs/prds/v0_6/result-field-vacuity-closure.md` §2.1 "The `Part` vacancy" — also cited by `assembly-modal-connection-graph.md`'s foundations decision 8 — so this entry is a pointer, not a restatement. Pt (#4578, done) shipped only the first half suggested above ("minimal opaque structure_def first"); the disposition above is recorded in that canonical PRD, not decided here or left open in Pt. Note: `modal_analysis.ri`'s `structure def Part` block and adjacent `ModalResult` field comments still promise the geometry+material+topology growth this disposition rules out, and cite done #4578 as a future owner — that prose is stale against this entry; correcting it is owned by `result-field-vacuity-closure.md` §2.7 "The doc-truth rot riding on the vacancy" / decision D6 (leaf ε), not by this PRD.
...
## 13. Re-citation closeout (task 4548 steps 9–16)
...
| `pose3-type` + `location-id-type` | **#4577** (blocked, LIVE) | **CITED** `TODO(pose3-type/location-id-type, #4577)` / `FIXME(location-id-type, #4577)` — survives | trajectory.ri:85/:104; modal_analysis.ri:446/:513/:560/:625 |
...
**Phase-B re-citation residuals — superseded by the step-15 rebase.** Steps 9–14 also (a) de-marked a prose continuation at `trajectory.ri:324` that the line-based detector had mis-read as a stray `mechanism-type` marker, and (b) catalogued out-of-scope doc-comment mirrors of the routed placeholders in `crates/reify-compiler/tests/trajectory_stdlib_compile.rs` (`force/velocity/acceleration/modal-result`) and `crates/reify-stdlib/src/flexures/common.rs` (`range-angle`). Both are now **MOOT**: when #4576 / #4579 / #4580 landed and went **done**, they removed those families' `.ri` markers *and* their mirror doc-comments upstream — the `modal_result` block at `trajectory.ri:322` is now the prose note "…tightened by task 4579", and the `trajectory_stdlib_compile.rs` / `flexures/common.rs` mirrors are gone (verified absent on the rebased tree, step-16). The only out-of-family `mechanism-type` marker remaining at `trajectory.ri:320` is intentionally untouched — it has its own owner and is correctly detector-flagged, outside 4548's family.

**Invariant verification (step-16).** `reify-audit --pattern PTODO` over the rebased working tree with the **liveness lane active** (real tasks DB via `REIFY_PTODO_TASKS_DB`) reports **zero in-family findings** across all four touched stdlib `.ri` modules (`modal_analysis.ri`, `trajectory.ri`, `kinematic.ri`, `fea_multi_case.ri`), and **no `orphaned` / `unknown-id` finding cites `#4574`–`#4580`**. Every task-listed in-family marker ends in exactly one of two states: **CITED** to a live non-terminal owner (vec3 → #4575; pose3/location-id → #4577 — both `blocked`, hence non-terminal per `ptodo.rs::is_terminal_status` = `done|cancelled`), or **DELETED/RESOLVED** (land-now markers removed by 4548 itself; range/part/modal-result/scalar markers removed upstream by their now-`done` siblings #4576/#4578/#4579/#4580). **4548 project invariant satisfied: no task-listed marker ends this task uncited or citing a terminal owner.**

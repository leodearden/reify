...
5. **Result-type column**: `Const(Type)` | `ArgAware(fn(&[Type]) -> Option<Type>)`. An `ArgAware` returning `None` (mis-shaped args) emits a **diagnostic** (`E_BuiltinArgShape` family) — never a silent first-arg guess. The existing per-slot dimension checks (`check_builtin_arg_types` vocabulary) migrate into the arg-slot column, plus (ratified 2026-08-07) a new `SameDimensionAs(slot)` constraint — the migrated vocabulary cannot express "both args same D, D free", which `floor`@2 / `atan2` / `remap` need.
6. **Dimensioned-numeric rulings.** Runtime scalars are SI-normalized (spelled units do not survive to eval — verified by the `floor(2.5mm)`→`floor(0.0025)` probe), so unit-respelling invariance is inherent and a bare dimensioned `floor` has no coherent answer. Therefore: `floor/ceil/round(x: Scalar<DIMENSIONLESS>) -> Int`, dimensioned arg → diagnostic with hint; **new two-arg form** `floor(x: Scalar<D>, quantum: Scalar<D>) -> Scalar<D>` = `floor(x/quantum)·quantum`, any positive same-dimension quantum (`floor(2.7mm, 0.5mm)` == `2.5mm`; `quantum <= 0` → eval diagnostic). `sinh/cosh/tanh(x: Scalar<DIMENSIONLESS>) -> Real`. `log10` dimensionless-only — log-of-dimension is unrepresentable in the rational-exponent dimension algebra and low-utility; the hint teaches `log10(x/1mm)`. *Amended 2026-08-07 (ratified):* (a) the two-arg form gets a **snap strategy** — the literal `floor(x/quantum)·quantum` is wrong on ~5% of exact-multiple inputs at natural quanta in f64 (measured: `floor(0.3mm, 0.1mm)` → 0.2mm via 0.0003/0.0001 = 2.9999999999999996; q=0.1mm 5.5% wrong, 1deg 4.3%; the 2.7/0.5 worked example is exactly representable and cannot catch it). Spec: r = x/q; k = floor(r); take k+1 when (k+1) − r ≤ ε·max(1, |r|) with ε a documented relative tolerance (~1e-12); `ceil` symmetric; `round` = snap then tie, tie rule **half-away-from-zero** (matching current 1-arg `round`, probed `round(±2.5)` → ±3). Negative x: the result is the greatest multiple of quantum ≤ x (toward −∞, matching `f64::floor`). The two-arg form never routes through the Int cast (the 1-arg `as i64` saturates). (b) **`exp` and `log` (= ln) join the dimensionless-only arg ruling** — same log-of-dimension rationale as `log10`; today `log(2mm)` silently evaluates ln(0.002) with result-only typing and no arg check. (c) **Dimensionless numeric arg slots admit `Int`** — overload resolution has no Int→Real widening (deliberate, coerce.rs:236-246), so a Scalar-only slot would statically reject `floor(5)` that eval accepts. (d) **`mod` is pinned**: `mod(x: Int, y: Int) -> Int` (matches eval, numeric.rs:89-98); dimensioned wrap-around is expressible as `x − floor(x, q)`. (e) **Diagnostics/hints are dimension-aware**: an ANGLE-argument diagnosis teaches `floor(x, 1deg)` / `log10(x/1rad)`; the `sinh/cosh/tanh` diagnostic states the trig-accepts-Angle vs hyperbolic-dimensionless asymmetry (hyperbolic args are ratios/rapidities) and teaches `sinh(x/1rad)` — `sinh(90deg)` evaluates silently today, so real code may hit the flip.
7. **Heterogeneous `Value::Map` returns → nominal stdlib structures, per family** (the joint precedent: eval returns a Map, the compiler types a `StructureRef`): `AxisAngle { angle: Angle, axis: Vector3 }` for `orient_to_axis_angle`, `Twist { angular, linear }` for `transform_log`/`transform_exp`, flexure/stackup/world result structures likewise; and (ratified 2026-08-07) a pair structure (e.g. `StressEnvelope`) for `min_max_stress`; `linear_combine`/`result_for` declare `ElasticResult` (and `linear_combine` should mint the `StructureInstance` rather than an `ElasticResult`-shaped Map); a `Snapshot` element structure for `sweep_grid` (`world` is already listed; `Snapshot` was not). This **resolves task 5380's open rulings** to a single pattern.
...
`{ name: &'static str, id: BuiltinId (generated), family: Family, binding: BindingKind, arity: Arity (Exact|Range|Variadic), arg_slots: &[ArgSlot], result: ResultSpec, basis: Basis }` with `ResultSpec = Const(Type) | ArgAware(fn(&[Type]) -> Option<Type>)` and (ratified 2026-08-07) `Basis = Ruling(&'static str) | Physics(&'static str) | Doc(&'static str) | Artifact` — `Artifact` meaning "preserves today's behaviour; no independent justification" (legal but conspicuous). Same-name arity overloads (`offset`@2/@3, `floor`@1/@2) are distinct rows in one name-group; `lookup(name, argc)` disambiguates. Same-name same-arity kind overloads (`union`/`difference`/`intersect` CSG-vs-selector) are one row whose `ArgAware` resolver dispatches on operand kinds — the row's doc names both dispositions.
...
- **I-REG-7** (ratified 2026-08-07): every row carries a `basis`; a registry-crate lint test enforces presence and reports the `Artifact` count as a visible ratchet (target zero); `Artifact` rows may not cite implementation behaviour as justification — that is what the tag itself declares.

### 7.3 Migration-step contract (per family τ)

(1) rows added; (2) compiler family arm replaced by registry lookup; (3) eval dispatcher arm replaced by `BuiltinId` match; (4) covered disjointness tests deleted + drift-test ledger updated; (5) parity rows active; (6) examples-corpus diagnostics byte-identical **except** corrections enumerated in the task text (each formerly-mistyped name's new type is a listed, reviewed change); (7, ratified 2026-08-07) each τ task text enumerates its `basis: Artifact` rows with one sentence each on why the row was left unresolved (derived from the rows; an empty enumeration asserts zero `Artifact` rows in that family). New gate-resident tests carry their drift-guard registrations same-diff (run-all-classification manifest, wallclock bounds, nextest partitions).

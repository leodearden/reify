...
| BT3 | consumer (ε) | **Cross-scope solved-auto surface read** (the F-inherit ζ #4826 deferral) | a scope reads another scope's `auto` cell that the merged solve resolves | `reify eval` surfaces the **co-solved** value (not a frozen/undef) — impossible before β. The *movement* of that child `auto` (as distinct from the merged write-back that makes it **readable**, which is M-WHOLE's own β) is delivered by the joint-drive seam's per-trial SCC recompute — `docs/prds/v0_6/whole-model-joint-drive-seam.md` β = **task 5189**, landed |
| BT4 | consumer (β+δ) | Merged solve beats the frozen cascade | same model solved both ways | merged whole-assembly `cost(self.descendants)` **< strictly** the bottom-up frozen baseline; the co-optimised child `auto` differs from its frozen value. **Producer:** the joint-drive mechanism this outcome depends on is delivered by `docs/prds/v0_6/whole-model-joint-drive-seam.md` β = **task 5189** (landed) over that PRD's α = **5188** (objective-position `cost()` expansion + `dependent_cells` order) and δ = **5334** (Gap C cluster formation) — **not** by M-WHOLE's own β/δ, which supply the merged builder/write-back and the back-end but do not by themselves move a child `auto` inside a trial |
...
- **ε — CI `.ri` end-to-end (integration-gate leaf).** Modules: `examples/`, `crates/reify-eval/tests`. Committed `examples/whole_model_cost_min.ri` + eval test: parent `minimize cost(self.descendants)` jointly drives child dims; `reify eval` shows the co-solved child `auto` (differs from frozen), the **BT3** cross-scope solved-auto surface read, and merged cost **<** frozen baseline. **Observable (leaf):** the CI example + eval test (BT3+BT4). Prereq: **β, γ, δ**.
  - **Post-decompose reconciliation (2026-07-28, task 5190).** ε was dispatched as task **5017**, and it now carries a real `depends_on` edge onto **5189** — the joint-drive-seam PRD's β leaf (`dependencies = [5014, 5015, 5016, 5125, 5189]`, verified in Taskmaster). That is the edge the seam PRD §10 promised at resolution time, and it makes 5189 **upstream** of ε for DAG-direction purposes. ε's BT3/BT4 premise — that a merged solve actually *moves* the coupled child `auto` rather than re-freezing it — became true on main when 5189 merged (`3e54addf4a`), over 5188 (α) and 5334 (δ, Gap C cluster formation, Amendment A1).

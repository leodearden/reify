...
- **Objective fold is consumed abstractly.** β assembles the `ObjectiveSet` and hands it to the solver's weighted fold; it must **not** hard-code raw-`f64` weight semantics. When the future `M-SHADOW` PRD flips `ObjectiveTerm.weight` `f64 → Value::Scalar`, β/γ/δ remain compatible with no rework (they scale a single-aspect `Money` objective, dimensionally safe under either representation). This fold-abstraction is what lets 4785 proceed **independently** of the multi-aspect line (§8) — no hard sequencing dependency.
...
| BT3 | consumer (ε) | **Cross-scope solved-auto surface read** (the F-inherit ζ #4826 deferral) | a scope reads another scope's `auto` cell that the merged solve resolves | `reify eval` surfaces the **co-solved** value (not a frozen/undef) — impossible before β. The merged write-back that makes the cell **readable** is M-WHOLE's own β; the *movement* of that child `auto` has a **separate producer** in the joint-drive seam — bound in the capability manifest's BT3 row |
| BT4 | consumer (β+δ) | Merged solve beats the frozen cascade | same model solved both ways | merged whole-assembly `cost(self.descendants)` **< strictly** the bottom-up frozen baseline; the co-optimised child `auto` differs from its frozen value. **Producer:** the joint-drive mechanism this outcome depends on comes from `docs/prds/v0_6/whole-model-joint-drive-seam.md`, **not** from M-WHOLE's own β/δ — which supply the merged builder/write-back and the back-end, but do not by themselves move a child `auto` inside a trial. The binding (task ids, landed SHAs, DAG direction) is stated once, in the capability manifest's **BT4 row** |
...
**Cross-PRD coordination (no hard dependency; re-decided with Leo 2026-07-05):** an earlier plan to sequence 4785 after sibling **4786**'s dimensioned-weight change was **retired** when 4786's expanded PRD (committed `7b07741d58`) resolved its fork to **Option 1** — a units-coherence *guard* (`E_OBJECTIVE_MIXED_DIMENSION`), keeping `ObjectiveTerm.weight` as `f64` — and **deferred** the `f64 → Value::Scalar` dimensioning to a future, unauthored **`M-SHADOW`** PRD. Since that breaking IR change is now far-future and 4785 consumes the fold abstractly (§5.2), 4785 proceeds **independently**. The only near-term overlap is that 4786's α edits the **same triplicated fold sites** (`solver.rs` `eval_objective_set`, `engine_eval.rs` provenance fold, `registry.rs` `eval_rank_cost`) that 4785's β/δ modify — a **merge-churn** concern handled by ordinary land ordering / file-locks, **not** a semantic dependency. 4785 is single-aspect `Money`-only, so it passes 4786's Option-1 coherence guard trivially.
...
| `multi-aspect-objective-units-coherence.md` (M-UNITS, 4786) | **independent** (fold-site overlap only) | the triplicated WeightedSum fold sites β/δ modify; 4786-α also edits them (adds a coherence guard) | each owns its own edits; 4785 consumes the fold abstractly | independent — merge-churn managed by land order |
| `M-SHADOW` (future, unauthored) | consumes-eventually | `ObjectiveTerm.weight` `f64 → Value::Scalar` dimensioning (deferred out of 4786) | M-SHADOW (future) | future; 4785's fold-abstraction (§5.2) needs no rework when it lands |
...
| `whole-model-joint-drive-seam.md` | consumes | objective-position `cost()` expansion + per-trial SCC recompute + Gap C cluster formation, under the §3.5 ConstraintSolver seam; supplies M-WHOLE **BT3/BT4**'s joint-drive premise (this PRD's β merges and writes back; the seam is what makes a child `auto` actually *move* inside a trial) | joint-drive-seam PRD (α = 5188, δ = 5334, β = 5189) | landed |

No reciprocal-ownership ambiguity: the weight-dimensioning IR change is owned by the future `M-SHADOW` PRD (deferred out of 4786); this PRD owns the clustering actuator + merged builder + back-end and consumes the fold abstractly. The §3.5 ConstraintSolver seam's contract owner is unchanged (`kinematic-constraints-*`); this PRD extends the `DimensionalSolver` impl, not the seam shape.

## §9 — Decomposition plan

Greek labels; task IDs assigned at decompose time. **All tasks proceed independently of the multi-aspect line** — 4785 consumes the fold abstractly (§5.2, §8), so there is no cross-PRD dependency; the shared fold-site edits with 4786-α are a merge-churn concern only.
...
- **β — merged cross-scope `ResolutionProblem` builder (cluster-aware freeze).** Modules: `reify-eval` (`engine_eval.rs` builder + `resolve_order` driver loop), `reify-ir` (`ResolutionProblem`). Union cluster auto cells/constraints/objectives into one problem; write back N scopes. **Intermediate — unlocks γ-coupling / δ / ε.** Prereq: **α**.
- **γ — `cost(self.descendants)` subtree-cost objective.** Modules: `reify-eval` / `reify-compiler` (aggregate `cost(collection)` semantic over #3988/#3991/#4292). **Observable (leaf):** `reify eval` shows `cost(self.descendants)` evaluating to the summed `Money` over descendants. **Also unlocks ε.** Prereq: landed #3988/#3991/#4292.
- **δ — optimiser back-end: NM + clustering + deterministic multistart, emits `RankedSolveResult`.** Modules: `reify-constraints` (`solver.rs`, `registry.rs`). Best-of-K fixed-start multistart over merged clusters; emit ranked candidates + `BestFound`; keep trait pluggable (no seeded solver built). **Observable (leaf):** `reify` surfaces a `RankedSolveResult` with K candidates + `BestFound` on a merged cluster. Prereq: **β**; landed F-result.
- **ε — CI `.ri` end-to-end (integration-gate leaf).** Modules: `examples/`, `crates/reify-eval/tests`. Committed `examples/whole_model_cost_min.ri` + eval test: parent `minimize cost(self.descendants)` jointly drives child dims; `reify eval` shows the co-solved child `auto` (differs from frozen), the **BT3** cross-scope solved-auto surface read, and merged cost **<** frozen baseline. **Observable (leaf):** the CI example + eval test (BT3+BT4). Prereq: **β, γ, δ**.
  - **Post-decompose reconciliation (2026-07-28, task 5190).** ε was dispatched as task **5017**, and it now carries a real `depends_on` edge onto the joint-drive-seam PRD's β leaf — the edge the seam PRD §10 promised at resolution time, which makes that leaf **upstream** of ε for DAG-direction purposes. ε's BT3/BT4 premise — that a merged solve actually *moves* the coupled child `auto` rather than re-freezing it — became true on main when that leaf landed. The task ids, landed SHAs, and the verified dependency edge are recorded **once**, in the capability manifest's BT4 row and its DAG-direction paragraph; they are deliberately not restated here, so a future id or SHA change has a single site to update.

...
| `cost_robustness_tradeoff(cost_expr, λ)` override | `examples/cost_robustness_tradeoff.ri` (CI): λ=1 → the pure-cost optimum, λ=0 → the robust centre (`build_centrality_objective`'s max-min-slack argmax, 31/3 mm), λ=0.5 strictly between, all three separated from the solver's constraint-derived seed — shown via `reify eval`. Both anchor names carry §2.4's precision note (#5715). |
...
- **Precision note on the two anchor names (#5715) — the single owner; cited by §1, §6, §8.1 and §8.2, which do not restate it.**
  - **λ=1 → "boundary"** is the MONOTONE-cost specialisation of the general **λ=1 ≡ argmin cost**. A cost with an interior optimum minimises *there*, not on a boundary. The zero-margin boundary form needs explicit tight `AutoParam` bounds and is verified at the solver layer (`crates/reify-constraints/tests/cost_robustness_tradeoff_blend.rs`); `.ri`-compiled autos get `bounds: None`, so it is not observable at the eval layer.
  - **λ=0 → "Chebyshev centre"** is the unit-gradient specialisation of the general **λ=0 ≡ the engine's centrality objective**. `collect_slack_terms` folds each inequality's slack un-normalised by its gradient, so a scaled constraint outweighs its geometric distance and the argmax is the max-min-RAW-slack point — the contract-correct target whenever the two differ. (Note this diverges from `constraint-solver-completion.md` §3.4, which specifies the *normalised* min-slack; the shipped fold is raw.)
  - Worked derivation of both, with measured values: the header of `examples/cost_robustness_tradeoff.ri`, which OWNS that derivation. Do not restate it here or in the language spec.
...
- **"λ=1 → boundary, λ=0 → robust centre" (branch 3):** λ=0 reduces to the existing Chebyshev-centre objective (shipped `build_centrality_objective`); λ=1 reduces to pure cost (shipped penalty solve); the normalised blend is this PRD's task γ. In-set. **Pass.** (Both anchor names as qualified in §2.4's precision note.)
...
- **`cost_robustness_tradeoff(c: Money, λ: Real) -> <objective>`.** Compiler recognises it in objective position; `λ` must be a compile-time-known `Real` in [0,1] (else a named diagnostic); first arg must be `Money`-dimensioned (else diagnostic). Solver: two anchor solves (min `c`; max min-slack), normalise each to its anchor range, minimise `λ·ĉ + (1−λ)·(−r̂)`. **Invariants:** λ=0 ≡ Chebyshev-centre objective; λ=1 ≡ pure-cost penalty solve; replaces the §2.2 floor when present; deterministic (anchors solved with the fixed seed). Both anchor names read under §2.4's precision note — the mechanism contract here is unchanged, but "boundary" and "Chebyshev centre" are its unit-gradient / monotone-cost specialisations.
...
| **Tradeoff λ=1 (compiler+solver)** | `minimize cost_robustness_tradeoff(<money>, 1.0)` | resolves to the boundary (pure cost); no floor — monotone-cost case, §2.4 precision note |
| **Tradeoff λ=0** | `… , 0.0)` | resolves to the robust centre (== Chebyshev-centre result) — unit-gradient case, §2.4 precision note |
...
- **Task γ — Compiler + solver: `cost_robustness_tradeoff(cost_expr, λ)` special-form (recognition + typing + normalised two-anchor blend); arg diagnostics.**
  - **Observable signal:** `examples/cost_robustness_tradeoff.ri` (CI): `reify eval` shows λ=1 → the pure-cost optimum, λ=0 → the robust centre (`build_centrality_objective`'s max-min-slack argmax, 31/3 mm), λ=0.5 → strictly between, all three separated from the solver's constraint-derived seed (both anchor names per §2.4's precision note, #5715); `cost_robustness_tradeoff(<non-money>, 2.0)` emits named diagnostics, no panic. **Leaf.** `grammar_confirmed=true`. Crates: reify-compiler, reify-constraints.

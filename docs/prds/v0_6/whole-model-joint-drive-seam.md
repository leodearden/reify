...
| BT-11 (solver, β) | `cost_robustness_tradeoff` is a THIRD, unconverted scoring site | joint-drive cluster with `minimize cost_robustness_tradeoff(...)`, money expression reads a dependent cell (`RivetedPanel.rivets.line_cost`) | a cost-dominant and a robustness-dominant λ produce MATERIALLY different solutions, and the cost-dominant one lands nearer the cost argmin (the cost axis stays live across λ instead of collapsing to `0.0`) |
| BT-12 (engine, β) | Per-sub parameter overrides are honoured in the folded instance-path cost cell | `sub rivets = Rivet(unit_cost: 0.90USD)` overriding a Costed template default; expanded objective reads the instance path | the instance-path `line_cost` folds against the OVERRIDE, not the template default (§12 Q4) |

BT-5 is the vertical-slice leaf's user-observable signal (closing G2) — it **presupposes δ landed** (Amendment A1): without Gap C's seed, no cluster forms and BT-5 is structurally unreachable. BT-1/-3/-4 face the producer (engine) side; BT-2/-6/-7 are the safety backstops; BT-8/-9/-10 face the cluster-formation seam (δ); BT-11/-12 are β-local boundary tests authored after this table's first landing (task 5189) — BT-11 faces the solver's tradeoff-scoring seam, BT-12 the engine-side per-sub-override seam (renumbered from a collision with δ's BT-10 by task #5764).
...
2. **Multistart interaction (δ).** Per-trial recompute lives inside each Nelder-Mead run, so best-of-K multistart should compose transparently; confirm no start-set caching assumes a frozen objective. Decide in β.
   - **ANSWERED (β, task 5189 step-6): the seeding does not, the SCORING did.** `multistart_points` is a pure function of the problem and needed no change; both post-solve scoring sites scored a STALE objective. Both now route through one shared scoring-map helper that reuses the same in-order fold as `build_trial_values`, so the optimiser, the ranker and α's post-solve write-back cannot measure different objectives. Boundary test: BT-11 (`crates/reify-constraints/tests/joint_drive_per_trial_recompute.rs`).
3. **Gate-resident test registration.** If β's eval test lands as a `tests/infra/test_*.sh` or a heavy/wall-clock-bounded crate test, its drift-guard registration (`run-all-classification.manifest` / `test_no_new_wallclock_upper_bounds.sh` / `.config/nextest.toml`) must ride the **same diff** (overlay rule). A plain non-heavy crate eval test needs none. Decide in β.
   - **ANSWERED (β, task 5189): NO registration required.** β ships no `tests/infra/*.sh` and no test near the 20-minute nextest ceiling, so no manifest row, no wall-clock-guard row, no `.config/nextest.toml` override. Placement instead followed the harness-layout ratchet: the eval-side tests were added to the EXISTING `crates/reify-eval/tests/joint_drive_expansion_boundary.rs` (a new top-level file under a consolidatable crate is refused by `check-harness-baseline-registration.sh`; grandfathering is superseded per esc-5056-11), while `crates/reify-constraints/tests/joint_drive_per_trial_recompute.rs` is new because `reify-constraints` is not a consolidatable crate.
4. **Instance-path alias emission (β, task 5189 step-10).** The expanded objective reads INSTANCE-PATH ids (`RivetedPanel.rivets.line_cost`) while the only foldable declaration is TEMPLATE-keyed (`Rivet.line_cost`), so `build_dependent_cells` both normalises its lookups through δ's `normalize_cell_id` and emits an instance-path ALIAS entry per surviving cell.
   - **v1 BOUNDARY (recorded, not deferred silently):** an alias is emitted only for a structure reachable by EXACTLY ONE non-collection instance path. `N` runtime paths share ONE template `default_expr`, so without per-path rescoping every path would fold to the identical value — the same reason `Engine::post_process_cross_sub_value_cells` skips collection subs. Collection subs, and any structure with ≥2 instance paths, therefore keep the pre-#5189 behaviour (template-keyed entry only).
   - **PER-SUB PARAMETER OVERRIDES ARE HONOURED (β, task 5189 step-15).** The alias expr is RESCOPED via `map_value_refs`, not cloned verbatim, so `sub rivets = Rivet(unit_cost: 0.90USD)` folds against the override rather than the template default. Two guards: a non-auto read of the alias'd template's own members moves to the instance path; an AUTO read stays TEMPLATE-keyed, because that is the namespace the solver inserts trial scalars into. Tests: `instance_path_alias_expr_is_rescoped_but_keeps_auto_reads_template_keyed` (unit, `engine_eval.rs`) and BT-12 (e2e, `joint_drive_expansion_boundary.rs`). This closes an unrecorded correctness hole — a verbatim alias clone made the solver minimise the template-default objective and then clobbered the correctly-elaborated per-instance value in the write-back. Single-instance-path rescoping now exists; extending it to the ≥2-path and collection cases remains a future leaf.

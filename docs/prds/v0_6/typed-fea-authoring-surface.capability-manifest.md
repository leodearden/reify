...
>
> **One type named here has since been retyped.** α's `direction : List<Real> = [0,0,-1]` (`:30`, `:33`) is now `Vector3<Dimensionless> = vec3(0.0, 0.0, -1.0)`, landed by task 5905 (merge `217b8e1647`) and live at `crates/reify-compiler/stdlib/fea_multi_case.ri:344` (`PointLoad`) and `:519` (`Gravity`). For *why* a direction takes the dimensionless slot, see the canonical `RULING (task 5848)` at `crates/reify-compiler/stdlib/trajectory.ri:97-101` — not restated here.
>

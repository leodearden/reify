...
| The arch test exists and fails on a bypass | landed | `gui/src-tauri/src/tests/debug_write_tool_routing_tests.rs` — `every_debug_write_tool_routes_through_the_delta_choke_point` sweeps the real `debug_server.rs` (5/5 routed); `bypassing_fixture_is_flagged`, `seam_named_only_in_a_comment_does_not_count`, `seam_named_only_in_a_string_does_not_count` and `seam_named_but_never_called_does_not_count` pin the negative half; `the_break_glass_knob_downgrades_a_real_bypass_to_a_warn` exercises the break-glass, whose live branch only ever runs on a red tree. What the scan does and does not reach: `docs/debug-mcp-contract.md` → “Two seams, ONE stated exception” | PASS |
| The check rests on behaviour, not on a name | landed | `every_refresh_baseline_seam_actually_refreshes` — every `*_and_refresh_baseline` fn must reach `compute_delta` (7 seams today). The no-private-emit half is `no_write_tool_handler_emits_privately` + `a_private_emit_in_the_delegated_helper_is_flagged` + `a_private_emit_through_the_event_bus_wrapper_is_flagged`, and its real-file negative is read together with the spliced positive control `the_private_emit_sweep_fires_on_the_real_file_when_mutated` rather than taken on faith. Grammar span and residuals: `docs/debug-mcp-contract.md` → “Two seams, ONE stated exception” | PASS |
| A new write tool cannot skip it silently | landed | write tools are ENUMERATED from the `"reify_*" =>` dispatch arms with an empty read-only exemption set, and every name the `ToolDef` registry advertises must appear in that scan (`advertised_tools_must_all_appear_in_the_dispatch_scan`) — two independent textual enumerations, so a tool whose arm the scanner cannot read reds as unenumerated instead of vanishing from the sweep. A sixth tool therefore reds until classified — the gap `ai-native-editing.md` §10 task θ names. Redundant, not airtight: a drop stays silent only if BOTH enumerations miss the SAME tool | PASS |

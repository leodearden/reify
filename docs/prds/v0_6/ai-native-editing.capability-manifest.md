...
| The arch test exists and fails on a bypass | landed | `gui/src-tauri/src/tests/debug_write_tool_routing_tests.rs` — `every_debug_write_tool_routes_through_the_delta_choke_point` sweeps the real `debug_server.rs` (5/5 routed), `bypassing_fixture_is_flagged` pins the negative half, and `seam_named_only_in_a_comment_does_not_count` + `seam_named_only_in_a_string_does_not_count` pin that only a CALL satisfies it — every behavioural check reads a view with comments AND string-literal contents blanked, so a seam named in a doc comment, a tracing message or a `json!` field does not count. The break-glass is exercised rather than merely shipped: `the_break_glass_knob_downgrades_a_real_bypass_to_a_warn` drives the warn path against a fixture, since the live branch only ever runs on a red tree | PASS |
| The check rests on behaviour, not on a name | landed | `every_refresh_baseline_seam_actually_refreshes` — every `*_and_refresh_baseline` fn must reach `compute_delta` (7 seams today); `no_write_tool_handler_emits_privately` covers the "never a private emit" half, following the same ONE delegation hop the seam check does (`a_private_emit_in_the_delegated_helper_is_flagged`), so `reify_open_file` — whose handler delegates its whole body to `open_path_into_engine` — is swept rather than exempt. That half spans a GRAMMAR — the `PRIVATE_EMIT_IDENTIFIERS` token set plus any `.emit(` call — so read its reach literally: `emit_typed` (the library's sanctioned `event_bus` wrapper) and `delta_to_events` are the LIBRARY's real emission surface and can fire against `debug_server.rs` today, while `emit_delta` (private to the `reify-gui` BINARY) and `.emit(` (needs an `AppHandle` `DebugServerState` does not carry) are FORWARD-LOOKING arms that pin the grammar for the day those shapes become reachable. The real-file negative is consequently not read as coverage on faith: `the_private_emit_sweep_fires_on_the_real_file_when_mutated` splices an `emit_typed` call into a handler taken from the dispatch scan rather than hardcoded — so the control follows a rename instead of silently ceasing to mutate anything — and asserts the sweep fires on the mutant while staying silent on the original. Stated residual: the one-hop cap is fail-CLOSED for the seam check but fail-OPEN for the emit check, so an emit further than one hop — or one named by no token in the grammar — is governed by point (b)'s prose, not by the gate | PASS |
| A new write tool cannot skip it silently | landed | write tools are ENUMERATED from the `"reify_*" =>` dispatch arms with an empty read-only exemption set, AND that set is cross-checked for equality against the names the `ToolDef` registry advertises (`advertised_tools_must_all_appear_in_the_dispatch_scan`) — two independent textual enumerations, so a tool whose arm the scanner cannot read reds as unenumerated instead of vanishing from the sweep. A sixth tool therefore reds until classified — the gap `ai-native-editing.md` §10 task θ names. Redundant, not airtight: a drop stays silent only if BOTH enumerations miss the SAME tool | PASS |

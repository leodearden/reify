...
| A new write tool cannot skip it silently | landed | write tools are ENUMERATED from the `"reify_*" =>` dispatch arms with an empty read-only exemption set, so a sixth tool reds until classified — the gap `ai-native-editing.md:144` names | PASS |

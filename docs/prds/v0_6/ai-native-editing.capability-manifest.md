...
| The arch test exists and fails on a bypass | landed | `gui/src-tauri/src/tests/debug_write_tool_routing_tests.rs` — `every_debug_write_tool_routes_through_the_delta_choke_point` sweeps the real `debug_server.rs` (5/5 routed), `bypassing_fixture_is_flagged` + `seam_named_only_in_a_comment_does_not_count` pin the negative half | PASS |
| The check rests on behaviour, not on a name | landed | `every_refresh_baseline_seam_actually_refreshes` — every `*_and_refresh_baseline` fn must reach `compute_delta` (7 seams today); `no_write_tool_handler_emits_privately` covers the "never a private emit" half | PASS |
| A new write tool cannot skip it silently | landed | write tools are ENUMERATED from the `"reify_*" =>` dispatch arms with an empty read-only exemption set, AND that set is cross-checked for equality against the names the `ToolDef` registry advertises (`advertised_tools_must_all_appear_in_the_dispatch_scan`) — two independent textual enumerations, so a tool whose arm the scanner cannot read reds as unenumerated instead of vanishing from the sweep. A sixth tool therefore reds until classified — the gap `ai-native-editing.md:144` names. Redundant, not airtight: a drop stays silent only if BOTH enumerations miss the SAME tool | PASS |

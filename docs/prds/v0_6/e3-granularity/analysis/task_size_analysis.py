...
tr = q(RUNS, """SELECT task_id, outcome, cost_usd, duration_ms, agent_invocations,
                ...
                FROM task_results WHERE project_id='dark_factory'""")
...
phase_rows = q(RUNS, """SELECT e.task_id AS tid, e.phase AS phase, sum(e.cost_usd) AS c
                        ...
                        WHERE r.project_id='dark_factory' AND e.cost_usd IS NOT NULL
...
inv_rows = q(RUNS, """SELECT task_id AS tid, model, sum(cost_usd) AS c
                      FROM invocations WHERE project_id='dark_factory' GROUP BY task_id, model""")
...
jr = q(J, """SELECT params, created_at FROM write_ops
             WHERE project_id='dark_factory' AND operation='set_task_status' AND success=1""")
...
json.dump([{k: v for k, v in r.items() if k != 'phases'} for r in rows],
          open("/tmp/claude-1000/-home-leo-src-dark-factory/55a263de-1a3f-4728-ad69-78d1d09661a2/scratchpad/task_rows.json", "w"))
print("\nrows dumped to scratchpad/task_rows.json")

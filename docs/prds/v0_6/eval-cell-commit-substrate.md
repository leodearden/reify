...
- **One ordering core** (P5) — `dirty.rs` sorts delegate to the `engine_fixpoint` Kahn core, reducing the surface where scheduling-order bugs (INV-EVAL-5 class) hide.
...
4. **P5.** `dirty.rs` `topological_sort`/`compute_levels` delegate to the `engine_fixpoint` Kahn core (`run_unified_pass_seeded`, engine_fixpoint.rs:273); `resolve_order` stays separate (different domain). Determinism pinned by the existing byte-identical-schedule tests.
...
7. **`edit_source` is neither deleted nor migrated now** — it is a product question (is warm source-edit latency a current pain?). Filed as a DECISION task for Leo (§9), deadline 2026-08-06: wire-after-P1 if yes, delete if unanswered.
...
| B6 | Ordering-core parity | any dirty-cone schedule | `dirty.rs` sorts (now delegating to the Kahn core) produce **byte-identical** schedules to the pre-delegation baseline on the determinism corpus | producer: Kahn core; consumer: dirty.rs |
...
- **γ** *(leaf)* — migrate **engine_eval.rs** sites (`eval`, `eval_cached`, guarded-group param cells, `reeval_cone_cell` at :4918, the annotation-args post-pass at :4357, and the structural-query/self-datum post-pass at :3451-3473) onto α+β; unrecorded sites → `CacheLeg::Skip`. Deps: α, β. Signal (B1/B2/B3): parity fixture — a warning swallowed on the cached path resurfaces; determinacy identical cold-vs-warm. INV-EVAL-1, INV-EVAL-2.
...
- **ν** *(leaf)* — `dirty.rs` `topological_sort`/`compute_levels` delegate to `run_unified_pass_seeded`; `resolve_order` untouched. File: dirty.rs. Signal (B6): existing byte-identical-schedule determinism tests stay green + a differential test asserts old-vs-new order identical on the corpus. Strengthens INV-EVAL-5 (single scheduling core; test).
...
- **π** *(DECISION bookmark, deferred — NOT flipped to pending)* — Leo product question: "is warm source-edit latency a current pain? wire `edit_source` after P1 if yes; delete if unanswered by **2026-08-06**." No INV until resolved.
...
- **`edit_source` migration/deletion** — gated on the π DECISION.

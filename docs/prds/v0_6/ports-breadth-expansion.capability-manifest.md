...
| `Torque` alias in param position | grammar-fixture | `pub type Torque = Force * Length / Angle` — shipped `ports_mechanical.ri:28` (**cite site retired by η**, task 5785, merge `61300d09`: the alias is gone from `crates/reify-compiler/stdlib/ports_mechanical.ri` and `Torque` is now a built-in `NAMED_DIMENSIONS` entry — `(DimensionVector::TORQUE, "Torque")`, `reify-core/src/dimension.rs:627`. `Torque` in param position **still resolves**, now via the registry and with no module import needed. The `:28` above was already off by one when written — every other doc cites `:29` — and is left as-recorded rather than silently corrected.) | PASS (verified pre-η; capability still holds post-η by a different mechanism) |

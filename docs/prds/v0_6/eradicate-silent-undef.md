...
| placement-relations-belt ε #5438 (relate operand-error exit regression slice) | this PRD generalizes | check Error-severity exit gate (ε #5403); #5438 is regression-fixtures-only with hard dep 5403, escalates (never implements) if 5403 is cancelled, and guards that relate codes never enter ε's allowlist | **this PRD (ε #5403)** owns the mechanism; **#5438** owns the relate regression fixtures | added by seam review 2026-07-25 — reciprocal of the belt-side record |

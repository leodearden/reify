...
- **trampoline-emission-sites-wired** — capability→producer (substrate): `grep:crates/reify-eval/src/engine_compute.rs:654`, `engine_eval.rs:9113,10124`, `engine_admin.rs:1535` all emit the `"@optimized target {:?}: no registered compute trampoline"` line — all four confirmed present on main this session (exact match to the PRD's own citations, zero drift). **PASS**.

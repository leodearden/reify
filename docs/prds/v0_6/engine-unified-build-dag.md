...
`enum BuildScheduler { LegacyMultiPass, UnifiedDag }` (default Legacy) + `REIFY_BUILD_SCHEDULER=unified|legacy` + `feature = "unified-dag"`. Stages map to the decomposition (§8): driver+cycle (δ) → executors+resolution+C7 (ε) → differential corpus (ζ) → acceptance (η) → warm/incremental (θ) → cutover+legacy delete (ι).
...
- **ι — cutover + legacy removal (Stages 4–5).** Flip default to `UnifiedDag` after N green CI runs; keep legacy as env kill-switch for one release; then delete the legacy loop bodies, the `BuildScheduler` enum, and `detect_let_cycle`'s let-local body. *Modules:* `crates/reify-eval/src/`. *Signal:* **leaf** — default scheduler is `UnifiedDag` in CI; legacy paths removed. *Prereq:* θ + operational green-CI confidence (human-gated; see Open Questions). *grammar_confirmed: true.*

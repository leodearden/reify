...
**The body below is the AS-AUTHORED design record.** The §4.x grammar/fixture ERROR-node tables, the §8/§8.B decomposition metadata, and the §8/§8.B leaf *instruction* bodies (what each task was told to do, as scoped on 2026-05-27) are dated *pre-implementation* artefacts, retained as provenance for why the decomposition was shaped the way it was — they are not current statements of fact about the language. Where an as-authored instruction was later overtaken by the F1 flip, it is annotated inline as superseded (see §8 task ε). §10 remains a live statement of what is still out of scope **except** for its two struck-through entries, which record scope that has since shipped and are annotated as such inline.
...
- **Layer B: substrate now exists** → DCE named-field grammar (α/β) **plus** generic-enum grammar, both hard prerequisites for any Layer-B task, and both landed (`data-carrying-enums.md`, `generic-data-carrying-enums.md`). *As authored,* under the then-default F1, no Layer-B grammar task was to be filed in this batch. F1 was resolved the same day in favour of building Layer B: it is decomposed here (§8.B), filed as tasks 4035–4040, and shipped.

## §5 — Resolved design decisions

- **D0 — Two layers, Option-based recovery first.** Error handling is delivered as Layer A (recovery over the existing `Option<T>`, no new substrate) then Layer B (`Result<T,E>` carrying an error payload, built on generic DCE). The split is load-bearing: Layer A shipped first against existing substrate, and Layer B chained onto the generic-enum substrate once that landed. Both shipped in v0.6. (Fork F1, resolved 2026-05-27, settled that Layer B is decomposed here — see §8.B.)

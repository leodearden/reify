...
| CI inheritance example | `examples/objective_inheritance.ri` resolves under `reify eval` in CI, exercising ordering + inheritance end-to-end. |
...
| BT3 | Acyclic coupling reorder | scope `B` reads scope `A`'s auto cell; source order is `B` then `A` | after ordering, `A` resolves first; `B`'s dependent cell computes from `A`'s **solved** value (not default); `reify eval` shows the corrected value (INV-1) |
...
- **β — Dependency-ordered scope resolution (read-DAG topo) replacing source-order.** Graduate `detect_scope_coupling`'s read-set extraction into a pre-solve **stable** topological sort (ties → source order); SCC fallback + retained `W_SCOPE_COUPLING`; rewire the resolution loop (`engine_eval.rs:2906`) to walk `resolve_order(module)`. Modules: `reify-eval` (`engine_eval.rs`). *Intermediate* — unlocks γ. Signal: unlocks γ; BT2/BT3/BT4 behaviors (surfaced via γ + ζ). `grammar_confirmed: true`.
...
- **ζ — Back-compat regression corpus + CI `.ri` inheritance example.** Commit (1) a recorded-baseline regression corpus proving INV-2 byte-identity on the back-compat set (BT1, BT2, BT7), (2) `examples/objective_inheritance.ri` exercising ordering + inheritance under `reify eval` in CI (BT3, BT5), and (3) diagnostic fixtures asserting δ/ε fire (BT8, BT9). Modules: `reify-eval` tests, `examples/`, `tests/prd-gate/fixtures` — `[]`. **Leaf, terminal — what M-WHOLE depends on.** Signal: the corpus + example run green in CI; the back-compat corpus asserts byte-identity (G6 branch-3) and the example resolves the inheritance/ordering behavior. Depends on γ, δ, ε. `grammar_confirmed: true`.

...
1. **CP-SAT is never wired.** `SolverRegistry::production()` leaves both the `logical` and CrossDomain `fallback` slots `None` (`crates/reify-constraints/src/registry.rs:63`, re-anchored 2026-08-25 — was `:42`), so every discrete component lands on `DimensionalSolver`, whose trial values are structurally `Value::Scalar` only — a natural Bool-auto formulation fails with the **misleading** `constraints could not be satisfied (max absolute residual: 1.00e0)`.
2. **`let`-indirection severs constraint↔auto connectivity** in three one-hop-blind layers (§2.2). Constraints that read autos *through a `let`* are silently dropped before any solver sees them: the autos report `W_UNDERDETERMINED` + `undef`. This bug is solver-independent — it breaks **continuous** formulations too (probe `dcm_let_cont.ri`, §2.3), and the spike proved CP-SAT alone does not fix it. **All three layers landed — see §2.2 (verified in-tree 2026-08-25).**
3. **The discrete domain channel is missing.** `AutoParam.bounds` is *always* `None` (`crates/reify-eval/src/engine_eval.rs:1775`, re-anchored 2026-08-25 — was `:1439`, inside `build_auto_param_list`), so `Int` autos can never enumerate; `@solver_hint("discrete_set", collection)` is parsed and validated (`crates/reify-compiler/src/annotations.rs:279`) but has **zero solver-side readers** (audit finding M-008) — stock-size selection over a catalog is exactly its intended semantics and it is dead at the IR seam.
4. **Honesty gaps.** `CpSatSolver::solve()` hardcodes `unique: true` (`cpsat.rs:365`/`:454`, re-anchored 2026-08-25 — was `:202/243`) — a determinacy lie for multi-solution problems (the discrete twin of in-flight #5388); and under an objective CP-SAT silently ignores it *and* no `W_SOLVER_OPTIMALITY_UNPROVEN` fires.
...
| Discrete domain channel: `Int` bound-mining + `discrete_set` → `AutoParam.domain` (δ) | `CpSatSolver::build_variable_domain` (existing fn, `cpsat.rs:110`, re-anchored 2026-08-25 — was `:43` — today errors for unbounded `Int` and cannot see `Real` catalogs); user surface: the stock-size pick example (ε). |
...
| `CpSatSolver::solve_all` + the `solve_ranked` override (β) | The engine objective path (`solve_ranked_with_dispatch`, `engine_eval.rs:5874`/`:7243`, re-anchored 2026-08-25 — was `:2897`, unrelated objective-cloning code) → `SolverRegistry::solve_inner` per-component dispatch (§2.1; dispatch call `registry.rs:449`, shared core `registry.rs:135`, re-anchored 2026-08-25 — was `solve_ranked`/`:280`), landed via F-result γ #4804 / M-WHOLE δ #5016. F-result §0.1 names **PRD 2** the sole owner of this override and of `ProvenOptimal`. Forward consumer: the `solutions()` surface (bookmark θ). |
...
- **Registry per-component dispatch (landed, M-WHOLE δ #5016):** `SolverRegistry::solve_inner` routes each decomposed component through the domain solver's own `solve_ranked` (dispatch call at `registry.rs:449`, inside the shared core documented at `registry.rs:135`) — the CP-SAT override becomes reachable the moment γ populates the slots. ~~Note: `registry.rs:244` currently passes `dependent_cells: Vec::new()` into sub-problems — one of α's three layers.~~ **LANDED 2026-08-03/04 (#5189 esc-5189-5, #5720) — verified in-tree 2026-08-25.** `registry.rs` now builds real per-component `dependent_cells` (`sub_dependent_cells`, `registry.rs:318`, filtered by the `dependent_auto_reads` subset test) and threads them into each sub-problem (`registry.rs:417`); the `Vec::new()` this note originally described no longer exists. See §2.2 item 3 below.
- **`dependent_cells` machinery (landed, #5188):** `ResolutionProblem.dependent_cells` carries the topo-ordered coupled non-auto cells. ~~It is consumed **only** by post-solve write-back (`materialize_dependent_cells`, `engine_eval.rs:1684`); `build_dependent_cells` (`engine_eval.rs:1553`) is the reusable transitive-closure builder.~~ **Anchors + "only" corrected — verified in-tree 2026-08-25.** `build_dependent_cells` (the reusable transitive-closure builder) is at `engine_eval.rs:2373`; `materialize_dependent_cells` (post-solve write-back) is at `engine_eval.rs:2716`. Post-solve write-back is no longer the only consumer: per §2.2 item 3, the same field now also feeds the shared pre-solve `fold_dependent_cells` (`solver.rs:315`).
- **CP-SAT solver (landed, unwired):** forward-checking backtracker with domains — `Bool` intrinsic, `Int` from `param.bounds` capped at `MAX_INT_DOMAIN = 1000`, `Enum` **constraint-mined** from variant literals (`build_variable_domain`, `cpsat.rs:110`, re-anchored 2026-08-25 — was `:43–115`, whose start landed on unrelated routing/capability-predicate doc prose). The Enum arm is the in-tree precedent for δ's `Int` bound-mining.
...
### §2.2 — The let-tracing gap: three one-hop-blind layers (α's charter — **DISCHARGED, all three layers landed — verified in-tree 2026-08-25**)

**Scope of the 2026-08-25 verification:** source-anchor verification only — each item below was checked against the current in-tree function bodies/call sites during this amendment pass. The boundary tests this DISCHARGED stamp implies (B1, line 157: `dcm_let_cont` resolves `a=6,b=4` with no `W_UNDERDETERMINED`; B2, line 158, and D1, line 142: byte-identical direct formulations) were **not re-run** here. Separately, #5954's own landing record (the last of the four cited landings) carries an explicit, still-unresolved caveat: *"HYPOTHESIS (NOT VERIFIED, worth its own task): any real model whose auto is floored ONLY by a derived cell will report a false Infeasible once this task puts that constraint in the right component ... The corpus was NOT surveyed for that shape."* This bears directly on D1 (line 142, no-regression) and is carried forward here as a **named residual risk** — not resolved by the DISCHARGED stamp above — for ε's integration gate (§8) to inherit. Follow-up filed (agent-followup, spawned from task #5974, ticket `tkt_0RSVKSTK7742JMXEVJY90YRD3P`, pending curator assignment).

1. ~~`filter_constraints_reading_autos` (`engine_eval.rs:1410`) keeps a constraint only if it **directly** reads an auto id → let-indirected constraints never enter the `ResolutionProblem`.~~ **LANDED 2026-08-01 (#5467, GREEN `79f28b099f`) — verified in-tree 2026-08-25.** `filter_constraints_reading_autos` now lives at `engine_eval.rs:1734` (the old `engine_eval.rs:1410` anchor is stale and is now unrelated geometry-consumer doc prose) and takes two more arguments: `reaching: &HashSet<ValueCellId>` (the auto-reaching non-auto cells, from `CellReadIndex::cells_reaching`, `engine_eval.rs:2144`) and `index: &CellReadIndex<'_>` for namespace bridging (`normalize_ref`, `engine_eval.rs:2003`). The filter is now a two-disjunct test: the original raw `auto_ids` probe, OR — guarded by `!reaching.is_empty()` — a raw-or-normalised probe against `reaching`, which admits let-indirected constraints. The fn's own doc comment records the asymmetry as deliberate: the `auto_ids` disjunct is left EXACTLY as it was, so an empty `reaching` makes the second disjunct vacuously false and behaviour on models with no dependent cells is byte-identical to pre-α (this is what keeps D1 exact and B2 satisfied).
2. ~~`decompose_into_components` (`crates/reify-constraints/src/decompose.rs:110–150`) builds connectivity edges only from **direct** `ValueRef`s ∩ auto params → even admitted constraints can't couple through a `let`.~~ **LANDED 2026-08-03 (#5954), unified under #5467 — verified in-tree 2026-08-25.** `decompose_into_components_with_reads` (`crates/reify-constraints/src/decompose.rs:676`) now expands each constraint's syntactic refs through `expand_refs_through_dependent_cells` (`decompose.rs:290`, the same map `dependent_cell_auto_reads` builds; expansion call at `decompose.rs:738`) *before* intersecting with auto params — so a constraint that reaches an auto only through a `let` (e.g. `a + side >= K` where `let side = SIDE_COEFF*c`) now unions `a` and `c` into one component.
3. ~~**No per-trial fold:** neither `DimensionalSolver` residual evaluation nor CP-SAT's forward-check materializes `dependent_cells` per trial assignment, and `registry.rs:244` passes `Vec::new()` into sub-problems — so a constraint over `let f = if up then 1.0 else -1.0` cannot be evaluated against a trial `up`.~~ **LANDED (#5189, #5720, #5954/#5467) — verified in-tree 2026-08-25.** A single `fold_dependent_cells` helper (`crates/reify-constraints/src/solver.rs:315`) now materializes `dependent_cells` per trial, consumed by both `DimensionalSolver` residual evaluation (`solver.rs:219`) and CP-SAT's forward-check (`cpsat.rs:307`) — decision 9's (a)+(b) solver-side twins (§3) now share this one helper. `registry.rs` builds real per-component `dependent_cells` (`registry.rs:318`) and threads them into each sub-problem (`registry.rs:417`), replacing the `Vec::new()` this item originally described. **Decision 9 is only partially realized, not fully — corrected 2026-08-25:** `materialize_dependent_cells` (`engine_eval.rs:2716`, decision 9's post-solve write-back clause) still runs its own per-cell fold rather than delegating to `fold_dependent_cells`, deliberately (per its own doc comment) because write-back must also thread determinacy state and a runtime-diagnostic sink the shared helper doesn't carry; decision 9's (c), the mixed leaf callback, awaits ζ (unlanded).

~~Fix shape (α): transitive closure in (1) and (2) **reusing `build_dependent_cells`**; **one shared** per-trial fold helper used by both solvers (§3 decision 9); thread `dependent_cells` through registry sub-problems. The infrastructure exists; the fix is mechanical.~~ **LANDED — verified in-tree 2026-08-25.** All three items above are struck; see each for its landed anchor (item 1: `engine_eval.rs:1734` via `cells_reaching`; item 2: `decompose.rs:290`'s `expand_refs_through_dependent_cells`; item 3: the shared `fold_dependent_cells` at `solver.rs:315` plus `registry.rs:318`/`417`). This fix-shape sentence is retained for historical context only — it no longer describes open work.
...
- **α — Let-tracing: transitive constraint↔auto closure + shared per-trial dependent-cell fold. LEAF LANDED 2026-08-03/04 (#5189, #5720, #5954, #5467) — verified in-tree 2026-08-25 (source anchors; see §2.2 for the per-layer detail and its residual-risk note).** ~~registry sub-problems get real `dependent_cells`, replacing `registry.rs:244`'s `Vec::new()`~~ (sub-clause superseded by the leaf stamp above, kept for history — current anchors: `registry.rs:318`/`:417`, §2.2 item 3).
  Modules: `crates/reify-eval/src/engine_eval.rs` (transitive `filter_constraints_reading_autos`, now `engine_eval.rs:1734`, via `build_dependent_cells`; `dependent_cells` threaded into per-template problems), `crates/reify-constraints/src/{decompose.rs, solver.rs, cpsat.rs, registry.rs}` (transitive decompose edges via `expand_refs_through_dependent_cells`, `decompose.rs:290`; the ONE fold helper (§3.9), landed as `fold_dependent_cells` at `solver.rs:315`, used in `DimensionalSolver` residual eval (`solver.rs:219`) + CP-SAT forward-check (`cpsat.rs:307`); registry sub-problems get real `dependent_cells` (`registry.rs:318`, threaded at `registry.rs:417`)).
...
- **β — CP-SAT honest enumeration core: `solve_all` + cap-2 `unique` + the `solve_ranked` argmin override.**
  ...
  **Intermediate** (CP-SAT unreachable in production until γ) — consumer: γ wiring, ε gate, ζ leaf callback; behaviour pinned by B4/B5/B6 via γ/ε. Unit tests in-src.
  Prereqs: α (the fold) — **landed**, see §8 α entry / §2.2. `grammar_confirmed=true`.
...
- **γ — Default-ON wiring: `production()` populates `logical` = CpSat + CrossDomain `fallback` = `DiscreteFirstFallback` (all-discrete arm).**
  ...
  **LEAF signal:** `reify eval` on the natural let-indirected Bool hexagon fixture (n2 shape) yields exact `Bool`s satisfying the balance constraints, no misleading residual error (B3); strict two-solution fixture reports honestly non-unique (B4); determinism B11. Baselines probe-verified (§2.3).
  Prereqs: α (**landed**, see §8 α entry / §2.2), β. `grammar_confirmed=true`.
...
α → β → γ → δ → ε → η
         ...
         (ζ also ← β)
```
*α landed 2026-08-03/04 (#5189/#5720/#5954/#5467, see §8 α entry / §2.2) — its downstream prereq edges (α→β, α→γ) are satisfied; β/γ/δ/ζ/ε/η/θ status is unchanged by this pass.*
...
7. **Fixture naming/location** for the committed probe fixtures (`docs/prds/v0_6/fixtures/discrete_*.ri` vs `crates/reify-eval/tests/fixtures/`). ~~Decide in ε; the PRD-level probes stay under `docs/prds/v0_6/fixtures/`.~~ **SUPERSEDED 2026-08-21 (Leo).** This question weighed the docs path against a *crate-local* path only; it never considered `tests/prd-gate/fixtures/` and so never ruled on the actual two-location conflict, and the ε it deferred to (#5471) has no fixture-location scope in its record. The standing standard is now: committed `.ri` fixtures that are PRD or capability-manifest evidence live in `tests/prd-gate/fixtures/`, cited by full repo-relative path; `docs/prds/**/fixtures/` is deprecated and is being emptied by #6431. Do not cite this row as a constraint on relocation — #6077 and #6369 both did, in error.

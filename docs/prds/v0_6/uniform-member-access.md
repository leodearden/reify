...
- **`self.<sub>.<member>` bypasses `E_PRIV_MEMBER_ACCESS` entirely** (added 2026-07-27 from esc-5424-3; re-verified on main `b6728d75b6`, fresh debug binary). `sub i = Inner()` + `let leak = self.i.secret` on a `priv param secret` reports `All constraints satisfied` (exit 0) and `reify eval` prints the priv value (`Test.leak = 0.005 m`) — while the *identical* read without the `self.` prefix (`i.secret`) and the `let`-instance form (`m.secret`) both correctly emit `E_PRIV_MEMBER_ACCESS`. So it is also a `self.X` ≡ `X` equivalence violation (spec §8.6, task 4615). Root cause: the non-collection sub branch (`expr.rs:3910-3945`) resolves via `scope.sub_member_types` and returns a `value_ref` with **no visibility consult**; `member_type_map_from_template` (`entity.rs:4383`) maps *all* `value_cells` with no visibility filter, so priv params are in the map and the gate at `expr.rs:6636` is never reached. **Owned by task η** — the fix is the routing, not a point patch (see §8 η). Unlike the tolerated pinned gaps in `priv_member_visibility_tests.rs` (`…_not_yet_gated`, :927), this one does **not** fail closed.
- Public `param` declared inside a block-form `where { }` guarded group is falsely reported as `structure 'Inner' has no member 'g'` through a `let`-instance chain (same esc-5424-3 probe run). `template_has_member` (`expr.rs:214-218`) scans `value_cells`/`ports`/`sub_components` but **not** `guarded_groups[].members`, while its sibling `template_member_is_priv` (`expr.rs:234-251`) does — the C1-iv lockstep duplication this PRD exists to remove. In scope for task α.
...
- **D8 — Enum ctor binding.** User-module `enum` defs must resolve as enum types in ctor param positions (the current lowering conflates `Enum(Fit)` with "structure type `Fit`"). Fix is in this PRD's ctor/member type-resolution blast radius. Note (scope-bounded): overall ctor field-type *conformance* is globally unenforced at check time (verified 2026-07-20 — `Widget(label: 42)` checks clean); this PRD fixes the enum **over-rejection** wart only and does not take on the general conformance gap (silent-accept class — eradicate-silent-undef territory / follow-up). *(Amended 2026-08-20: ζ landed as #5429, merge `bc8f74a4d4`. The D8 rule's cross-compilation-unit and cross-phase residuals — prelude-trait obligation collisions, enum-variant payload phase ordering, shadow-set hygiene — are owned by `docs/prds/v0_6/enum-shadow-coherence.md`.)*
...
| `docs/prds/v0_6/enum-shadow-coherence.md` (2026-08-20) | extends ζ | the D8 shadowing rule's residual seams — `LocalEnumShadowScope`/`build_local_enum_shadow_set` phase coverage + the prelude-trait obligation collision in conformance checking | that PRD | amendment recorded here 2026-08-20 |
...
- **α — Uniform member-path resolver in reify-compiler** (M1, D1, D6). Intermediate → unlocks β, η; regression floor: boundary rows 5, 7, 11. Crates: reify-compiler. *Consumer:* β/η + every leaf below. Also lands the **test-only characterization pin** for the §2 `self.<sub>.<member>` priv bypass — α does *not* fix it (that would require the forbidden parallel visibility map); η flips the pin.
...
- **ζ — Enum-typed ctor param binding for user-module structures** (M5, D8). **LEAF.** Signal: `member_enum_ctor.ri` — check exits 0, constraint satisfied, eval shows bound variant (boundary row 9). Deps: none (parallel-safe; type_resolution seam). *(Amendment 2026-08-20: landed as #5429, merge `bc8f74a4d4`. The "parallel-safe; type_resolution seam" premise proved false — the mechanism reached the whole-module name environment (the `lib.rs` phase sequence) and the prelude-compile conformance boundary; the residuals are owned by `docs/prds/v0_6/enum-shadow-coherence.md`.)*
- **η — Route sub member access through the uniform resolver; retire bespoke matchers** (M7). Intermediate → unlocks ι. Signal: cross-sub + priv suites green with matchers deleted (boundary row 6), **plus a NEW `E_PRIV_MEMBER_ACCESS` on the `self.<sub>.<priv member>` shape**. Deps: α, γ (behavioral parity target must exist first). G7 driver: `no-lockstep-duplication`.
  - **η owns the §2 `self.<sub>.<member>` priv bypass** (Leo, 2026-07-27, resolving esc-5424-3). Routing that site through α's resolver *makes a new diagnostic appear* — that is the fix, **not** a regression η introduced, and it must not be suppressed to keep a suite green. "Behavioral parity" for η therefore means: existing assertions hold **except** the characterization pin α lands for this shape, which η must flip from "asserts the leak" to "expects exactly one `E_PRIV_MEMBER_ACCESS`". If that pin passes unchanged, the bypass survived and η is not done.
  - *Why not fix it earlier (α, or standalone):* `ExprScope` carries only flattened `name → Type` maps and no visibility, so a point fix needs a **new parallel `sub_member_visibility` map** — precisely the lockstep duplication C1-iv/INV-5 forbids and η exists to delete. α instead pins the gap as a test-only characterization (no production change), converting an invisible landmine into a failing test that tells η's implementer the new diagnostic is intended.

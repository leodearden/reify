...
- **Per-name disposition** — a name is compliant iff exactly one of:
  ...
  2. **Allowed**: the registry entry's line in `units.rs` carries an inline `// pdoccover:allow — <reason>` (PTODO's site-local escape philosophy; reason mandatory).

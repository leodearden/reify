...
| 8 | β | integration: `examples_smoke.rs` + drivebelt test consume the migrated surface (anti-orphan gate) | **wired-on-main**: `crates/reify-compiler/tests/harness_compilation_surface/examples_smoke.rs` walks `examples/*.ri` and compiles each (drivebelt header self-documents this); `crates/reify-eval/tests/drivebelt_trait_bounds.rs` is the dedicated integration test. β's migration is verified by these existing CI gates — the C-as-integration-gate for the breaking change. | **PASS** |

...
| **subsumes #5588 item 1** | DAG-direction: #5588 is `pending`, deps `[5540]`, and does **not** depend on any leaf here — no inversion. Item 2 (`<HasAxis & HasRadius>`) is explicitly **out of scope**, hard-blocked on `Type::TraitObject` call-site conformance. Decompose-time obligation: rescope #5588 to item 2. | PASS |
...
| the trait is extensible and the fix is pre-specified | **absorbs #6523** (`pending`, low). `decompose.rs`'s own doc block above `domain_of_auto` names the fix — move the capability question behind `ConstraintSolver` — and calls today's `crate::cpsat::can_enumerate` reach-in unsound. DAG-direction: #6523 depends on no leaf here. | PASS |

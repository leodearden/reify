...
five migrated corpus ctor sites, so no wrong answer is reachable today —
...
`crates/reify-eval-fea-tests/tests/r3b_modal_selector_displacement.rs:489`. The reasoning —
including why `alpha` is typed `Frequency` rather than `AngularVelocity` — lives in
`modal_analysis.ri`'s ANGULAR-RATE TRAP comment (:130-145), which already names this PRD as the
...
deliver the cycles/s-vs-s⁻¹ separation this site was handed off for; that gap is this
convergence work's to resolve, not something C1-C4 already covers. Stamped **2026-09-01 at
`da22522934`** (a `main` merge commit).

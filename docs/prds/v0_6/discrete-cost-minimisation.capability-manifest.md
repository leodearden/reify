...
| `ResolutionProblem.dependent_cells` field + post-solve consumer exist (#5188) | `grep`: `materialize_dependent_cells` `engine_eval.rs:1684`; registry gap α fills: `dependent_cells: Vec::new()` `registry.rs:244` (delivered_check `registry-threads-real-dependent-cells` corrected 2026-09-25, #7739: now `grep:dependent_cells:\s*sub_dependent_cells` → `present` in `crates/reify-constraints/src/registry.rs`, the per-component slice in `SolverRegistry::solve_inner`; the original `Vec::new()` → `absent` form matched a `#[cfg(test)]` ResolutionProblem fixture builder and falsely blocked #5469/#6077/#6207/#6431. Accepted gap: it proves the field init, not the fold contents; behavioural cover is `crates/reify-constraints/tests/joint_drive_per_trial_recompute.rs`) | **PASS** |

...
| `index-dir-flag-documented` | γ landed `--jcodemunch-index-dir` with its four-rung resolution order (flag → `$JCODEMUNCH_INDEX_DIR` → `$CODE_INDEX_PATH` → `$HOME/.code-index`) and neither skill document mentioned it. Grep `--jcodemunch-index-dir`, expect **present** in `SKILL.md` + `references/cli-invocation.md`. Provenance esc-6108-7 — an ownership gap surfaced from task 6108; no sibling filed, μ already owns this drift class and is wired on 6108. | **PASS** |
| `refusal-codes-documented` | γ landed three codes (`E_JC_INDEX_STALE`/`_EMPTY`/`_UNREADABLE`) that hard-exit 125 on an all-jcodemunch `--pattern` set; neither skill document named any. Grep the **prefix** `E_JC_INDEX_` — not one token — so a fourth *code* cannot silently escape the gate; the freshness path's one deliberately **token-less** refusal (HEAD unreadable) is beyond any grep's reach by design and is held by prose in `references/cli-invocation.md` §4.1 instead. Same provenance. **No expect-absent check on `leodearden/reify` under `.claude/skills`, and none may be added**: μ must be free to *name* the retired identity while explaining its retirement. | **PASS** |

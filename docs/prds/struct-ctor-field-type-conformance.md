...
- New codes minted (ε only: `CtorUnknownField`, `CtorArity`) are fieldless unit variants appended to `DiagnosticCode` in `crates/reify-core/src/diagnostics.rs` — the serde `rename_all = "PascalCase"` derive supplies the wire form, and `#[non_exhaustive]` means no downstream match breaks. **Nothing fails automatically when a variant is added:** `DiagnosticCode` derives no `strum` trait, so no completeness sweep exists to trip, and nothing scans declared-vs-used codes. ε must therefore hand-write the opt-in per-variant `--lib` test into that file's inline `mod tests` in the same diff — that hand-written test is the only discipline there is.
...
- **ε — Unknown-field/over-arity tightening at Warning (severable).** Removes `__arg{i}` leniency semantics behind the same knob; new codes + their hand-written per-variant `--lib` tests (D2: no completeness sweep fires for `DiagnosticCode`). *Signal (intermediate):* warnings fire on fixtures 11/12; corpus effects flow into β.

...
- New codes minted (ε only: `CtorUnknownField`, `CtorArity`) are fieldless unit variants appended to `DiagnosticCode` in `crates/reify-core/src/diagnostics.rs` — the serde `rename_all = "PascalCase"` derive supplies the wire form, and `#[non_exhaustive]` means no downstream match breaks. **Nothing fails automatically when a variant is added:** `DiagnosticCode` derives no `strum` trait, so no completeness sweep exists to trip, and nothing scans declared-vs-used codes. Every test below is therefore opt-in and belongs in ε's own diff:
  - **Normative:** the *behavioural* test — compile the §7 row 11/12 fixtures and assert the emitted diagnostic carries `code == Some(CtorUnknownField)` / `Some(CtorArity)` at the severity the D4 knob is set to (`Warning` at ε). This is §8's stated ε signal, and the only assertion that catches a code minted but never wired to the ctor site.
  - *Optional smoke:* the variant-exists + `Diagnostic::error(..).with_code(c).code == Some(c)` round-trip in that file's inline `mod tests` (exemplar `eval_cycle_and_unresolved_codes_exist_and_attach`). It proves that two unit variants are distinct and that a one-line builder setter stores its argument — cheap, but no substitute for the behavioural test.
  - *If a unit-level assertion is wanted anyway,* prefer the wire form over mere existence — the PascalCase string is a real downstream contract. **Feature caveat:** that wire form exists only under `reify-core`'s optional, non-default `serde` feature (`#[cfg_attr(feature = "serde", …)]`; `[features] serde = ["dep:serde"]`, no `default`). Such a test must carry `#[cfg(feature = "serde")]` — ungated it fails to build under default features — and must be run as `cargo test -p reify-core --features serde --lib`, **not** a bare `--lib`, which compiles it out silently. Exemplar: `eval_codes_serialize_to_pascalcase_wire_strings`.
...
- **ε — Unknown-field/over-arity tightening at Warning (severable).** Removes `__arg{i}` leniency semantics behind the same knob; new codes + their hand-written tests, the normative one behavioural (D2: no completeness sweep fires for `DiagnosticCode`, so the fixture-11/12 assertion on emitted code + severity is the real check). *Signal (intermediate):* warnings fire on fixtures 11/12; corpus effects flow into β.

...
- The full event-journal infrastructure beyond what's needed for `EventKind::error` emission (other event kinds are tracked separately).
...
- A failure-injection test asserts: forced panic in a leaf node → leaf `Failed` → mid node `Pending` with chain pointing to leaf → realization event `EventKind::error` emitted exactly once for the leaf.
...
4. **Implement the `Failed` failure path**: catch evaluation failures (panic boundary, explicit error returns from kernel ops, out-of-bounds index, missing-key map lookup) and produce `Failed { error: ErrorRef }`. Emit an `EventKind::error` realization event. Add failure-injection tests.

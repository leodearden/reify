...
### Future-lever note (NOT filed) — per-host parallel warm verifies (the invariant-3 "pool")

Invariant 3's "small worktree pool" fork has a concrete realization now `xfsprogs` is installed. Captured so it is not re-derived; **deferred** (a per-host XFS volume is novel substrate — §6 stays "no novel substrate" *because* this is not a current phase) and **last in priority** (see end). **Scope:** these lanes are the **merge worker's speculative verify slots** — Lever C's `_speculation_slot` / chain-invalidation pipeline (`plans/merge-throughput-multihost-verify-prd.md`) run at depth K>1, with CAS-advance of `main` still strictly serial + ordered — **not** task worktrees, which keep their own `target/` (§11 forbids reusing a task target for a merge).
...
- **Load-bearing trap — path sensitivity (this very invariant 2).** A base built at path X, reflinked into a lane at a *different* path Y, carries stale absolute paths in fingerprints/DWARF → **every fingerprint invalidates → the lane recompiles the whole workspace** and the CoW benefit evaporates. So any base-snapshot pool MUST bind-mount each snapshot to **one canonical worktree path** per lane (a per-lane mount namespace; the verify sandbox is the seam). Layer-0 independent worktrees sidestep this entirely (each has its own fixed path).
...
- **Sharing one persistent `target/` across concurrent merges** — safe only at `_MERGE_AHEAD_BOUND=1` (invariant 3). The sanctioned way to *lift* this (K independent worktrees, or a CoW reflink-snapshot pool with the path-sensitivity fix) is the §10 Future-lever note — deferred, needs an XFS volume, and the last lever to reach for.

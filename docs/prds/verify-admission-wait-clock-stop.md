...
- The only real exit-75 → `REQUEUED` contract in dark-factory is the warm-lane **seed / disk-guard** path (`git_ops.py`, `WarmLaneRequeue`) — it fires during lane provisioning **before** verify runs, and is unrelated to the verify subprocess's exit code. Even ENOSPC during verify (`VerifyInfraError`, the in-process warm-marker write, in `verify.py`) ends **BLOCKED**, not requeued.
- There is **no uncapped / penalty-free requeue class** for verify, **no exit-code transient class** (OCCT-slot / ENOSPC are not exit-code based), and **no heartbeat / liveness** signal — the only hang detection is the per-command wall-clock `asyncio.wait_for` from spawn (in `verify.py`). A verify blocking on a semaphore is **indistinguishable from a hang** today.

So the fix **cannot** be reify-side alone, and it **cannot** route through a requeue: it needs a dark-factory change. The corrected docs (CLAUDE.md, dark-factory-orchestrator.yaml, the existing semaphore PRD) are scoped into this work (§9 leaf γ).
...
| DF streams verify stdout/stderr line-by-line (to add marker parsing) | **present** | `verify.py`'s streamed-log path |
...
- **D2 — Uniform mechanism across semaphore + PSI gate (user, 2026-06-25).** One marker protocol, one shared emitter, one DF consumer. The PSI gate (`cpu_admit requeue`) has the *identical* latent bug (its MAX_WAIT exit-75 also → `blocked`); both adopt the clock-stop wait. The `compile_gate` (`cpu_admit admit`) already admits-on-timeout (never exit-75, bounded 300s) so it is **not** a starvation source and is out of scope for clock-stop (may adopt later, no urgency). **Reversed (task 4920, once task 4838/`dark_factory:1916` deployed 2026-06-27):** the DF exclusion of the `@@REIFY_CLOCK_*@@` span from `verify_command_timeout_secs` is marker-based and gate-agnostic, so admitting-on-timeout was no longer the safer default — `compile_gate` (`cpu_admit admit`) was moved INTO clock-stop scope instead. It now HOLDS until PSI drops on either dimension (CPU or memory), reusing the `psi_pressure` reason token, rather than admitting under sustained pressure. See §6 and §10 for the superseded specifics.
...
- **D5 — Continuous wait is gated until the DF seam ships (graceful degradation).** Reify ships the marker emission + the unlimited-wait *capability* but keeps `REIFY_TEST_SEMAPHORE_WAIT` **finite** (3600, < `verify_command_timeout_secs`=7200) in `dark-factory-orchestrator.yaml` until DF-δ is deployed. Pre-seam, a finite wait exits cleanly (status-quo `blocked`, with the N=3/4839 relief) rather than blocking past the wall-clock into exit-124. The flip to `unlimited` + enabling the DF knobs happens together at deploy (leaf ε / task 4838). This mirrors the dead branch's gating discipline ("activating unlimited before the seam is strictly worse").
...
- **`compile_gate` clock-stop.** ~~Out of scope (D2) — it admits-on-timeout and is bounded, not a starvation source.~~ **Superseded by task 4920:** now IN scope — `compile_gate` sets a clock-stop reason and holds until PSI drops (see D2 addendum). Like the semaphore (previous bullet), indefinite starvation under *permanent* saturation is an accepted limitation here too, not a defect — the hold stays clock-stop-excluded from `verify_command_timeout_secs` (so it heartbeats rather than hangs) but makes no forward progress on its own; the durable fix is the same **dispatch admission** lever, not a further verify-layer change.
...
| `verify_command_timeout_secs` / `REIFY_TEST_SEMAPHORE_WAIT` co-tuning + the `unlimited` flip | **reify** | `dark-factory-orchestrator.yaml` (reify-owned). WAIT stays finite < timeout pre-seam; flips to `unlimited` at deploy. |
...
- **γ (reify) — docs correction + contract surfacing.** Correct the false exit-75→requeue premise in `CLAUDE.md` ("Test concurrency"), `dark-factory-orchestrator.yaml` (:80-82 + the WAIT knob block), and `docs/prds/test-run-concurrency-semaphore.md` §4/§6/§7; document the clock-stop marker contract + the dark-factory dependency; co-tune the WAIT<timeout comments. **Signal:** grep shows the corrected text; dark-factory-orchestrator.yaml cites the marker contract + the DF task; the existing PRD §6 "No dark-factory change" is replaced with the seam pointer. **deps:** α (so the doc matches the shipped markers).
...
- **Amend:** the branch had a *bare* heartbeat with **no STOP/START framing** for DF to consume, and was **semaphore-only**. This PRD adds the explicit STOP/START contract (so DF knows exactly which spans to exclude) and applies the uniform emitter to the **PSI gate** too (D2). **Further amend (task 4920):** D2's "compile_gate is out of scope" call is itself reversed once the seam deployed — `compile_gate` (`cpu_admit admit` mode) now shares the same uniform emitter and the `psi_pressure` reason token, holding until PSI drops instead of admitting-on-timeout.

...
- **`compile_gate` clock-stop.** ~~Out of scope (D2) — it admits-on-timeout and is bounded, not a starvation source.~~ **Superseded by task 4920:** now IN scope — `compile_gate` sets a clock-stop reason and holds until PSI drops (see D2 addendum).

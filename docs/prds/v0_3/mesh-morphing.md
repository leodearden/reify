...
**Two-axis ComputeNode routing (resolved 2026-05-12 by [`docs/prds/v0_3/compute-node-contract.md`](compute-node-contract.md) §6; axis-1 corrected 2026-06-23 by [`docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md`](../v0_6/volume-mesh-realization-and-morph-wiring.md) §3 D1):**

- **Axis-1 — Mesh-morph as ComputeNode:** **no** (corrected — previously read "yes"). Morph does **not** route through `Engine::insert_compute_node`. It is a **§3.2 realization-kind-dispatch producer**: a morph arm in `dispatch_volume_mesh`, called from `execute_realization_ops`, producing a `VolumeMesh` realization exactly like the Gmsh tet/hex/wedge arms — not a §3.4 ComputeNode wrap. **Rejected alternative:** the original 2026-05-12 resolution wrapped the morph dispatch point in a ComputeNode specifically to receive the cache / warm-state / cancellation / significance-filter machinery via the dispatch seam (compute-node-contract §2–§5). This is now rejected: the realization-read-api PRD (tasks 4507–4513, done) made `ComputeFn` purity a HARD invariant — trampolines *read* realizations, never *produce* them, and `ComputeOutcome` has no `Realized` variant by deliberate design; wrapping morph in a ComputeNode would re-open that closed seam. The cache/warm-state benefits are delivered instead by the existing realization cache (input-hash keyed) + `RealizationProjectionStore`. Engine-wiring slice: `docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §8 task β (task #4744 — morph arm in `dispatch_volume_mesh`); the §3.2 foundation (tet path) landed as task α (#4743).
...
These two axes are orthogonal. The "essentially `solve_elastic_static`" sentence above describes axis-2 algorithmic equivalence (same BVP, same primitives, different inputs) — not routing through the stdlib `solve_elastic_static` ComputeNode entry point (which, per the axis-1 correction above, neither axis of morph routes through — morph is not a ComputeNode consumer at all).
...
- **Debug-MCP RPC seam-owned by `docs/prds/v0_3/gui-event-channel-inventory.md`** — the `morph_stats` Debug-MCP RPC (inventory §2.3 / Phase 3 task θ) surfaces live morph-engine stats via the debug bridge; upstream prereq chain is task 2949 → 2948 → **task #4744** (`docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §8 task β — morph arm in `dispatch_volume_mesh`; re-homed from cancelled task 2947, which specified `reify-eval` calling into `reify-mesh-morph` directly, a Cargo cycle). Emitter registration in `debug.rs` is decomposed in the inventory PRD. See also `docs/gui-event-channels.md`.
- **`docs/prds/v0_3/compute-node-contract.md`** — the ComputeNode dispatch-seam contract. §6 (consumer-policy table, corrected 2026-06-23) clarifies that mesh-morph is **not** a ComputeNode consumer (axis-1 = no) — it is a §3.2 realization-kind-dispatch producer — and that its internal composition does not route through stdlib FEA-as-ComputeNode (axis-2 unchanged). §8 task κ (task 3429) and this PRD's task μ (task 3430), which had proposed a ComputeNode-wrapped morph and its prose disambiguation respectively, are both **superseded** — task κ/3429 is cancelled; task μ/3430 closed as done, its correction carried forward by task δ (#4745) — morph wires via §3.2 realization-kind dispatch, not a §3.4 ComputeNode. The live engine-wiring slice is `docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §8 task β (task #4744 — morph arm in `dispatch_volume_mesh`, dep on task α #4743's §3.2 foundation); the prose correction (this note + the axis-1 note above) is that PRD's task δ (task #4745).

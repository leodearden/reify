...
- **`docs/prds/v0_3/compute-node-contract.md`** — the ComputeNode dispatch-seam contract. §6 (consumer-policy table) clarifies that mesh-morph is a ComputeNode consumer (axis-1 = yes) and that its internal composition does not route through stdlib FEA-as-ComputeNode (axis-2 unchanged). §8 task κ (task 3429) is the engine-wiring slice that inserts the morph ComputeNode at the VolumeMesh-realization dispatch point. This PRD's task μ (task 3430) implements the prose disambiguation independently of the α–κ implementation chain.

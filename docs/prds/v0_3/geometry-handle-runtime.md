...
| **GR-031 composed-stress envelope unblock.** Existing task 3468 (which depends on SIR-α + this PRD for typed envelope helpers consuming geometry-bearing structures). | Phase 6 done. | Task 3468 unblocks; its tests pin envelope `.max_von_mises` member access on a geometry-bearing structure. |
...
- **GR-031 (cluster C-29 composed/derived stress recovery)** — Notes row appended: "Functional unblock mechanism revised: `docs/prds/v0_3/structure-instance-runtime.md` SIR-α + `docs/prds/v0_3/geometry-handle-runtime.md` GHR-ζ. Task 3468 (already filed) executes against the joint foundation."

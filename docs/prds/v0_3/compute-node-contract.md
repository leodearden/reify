...
**Cross-seam meta-policy umbrella.** `engine-integration-norm.md` (GR-017 / cluster C-14) is the cross-seam meta-policy that catalogs the family of engine plug-in seams; its §3.4 is the listing entry for ComputeNode dispatch — listing-only, deferring all normative content to this contract. CN-contract retains sole authority over cancellation (§2), pending lifecycle (§3), OpaqueState (§5), consumer policy (this section), and trampoline signature (§4 registry / §8 integration DAG); engine-integration-norm.md §3.4 does not redefine or supplement any of those. The relationship is hierarchical: **CN-contract** is the single-seam contract (gold-standard B+H exemplar for one seam); **engine-integration-norm** is the cross-seam umbrella (lists CN-contract as one of seven). For the general two-seam composition pattern where a ComputeNode wrap (§3.4) composes with a realization-kind dispatch branch (§3.2), see engine-integration-norm.md §7.

**Correction (v0_6 PRD task δ, #4745):** mesh-morph is no longer an instance of that two-seam composition pattern. Per `docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §3 D1 (confirmed 2026-06-23), morph is a pure §3.2 realization-kind-dispatch producer (morph arm in `dispatch_volume_mesh`, called from `execute_realization_ops`), not a §3.4 ComputeNode — see the corrected Mesh-morphing row and negative-case rationale below. `engine-integration-norm.md` §7's mesh-morph worked example still narrates the superseded §3.4-wrapped framing; that instance-correction is owned by this PRD per its §6 relationship table but is tracked as a separate concern (naturally handled by the §3.2 dispatcher implementer, tasks α/β) and is outside this task's footprint — not edited here.
...
| **Mesh-morphing** | **No — §3.2, not §3.4** | Realization output, mesh-size-expensive, warm-state-beneficial — but morph is a §3.2 realization-kind-dispatch producer (morph arm in `dispatch_volume_mesh`, called from `execute_realization_ops`), exactly like the Gmsh tet/hex/wedge arms; it does not route through ComputeNode. **Corrected** (`docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §3 D1, confirmed 2026-06-23): this row previously read "Yes" under a since-rejected §3.4-ComputeNode-wrap framing — see the negative-case rationale below for the full analysis. Axis-2 is unchanged: morph's internal composition still does not dispatch through stdlib FEA-as-ComputeNode (calls `solve_cg` directly). The §8 task μ follow-up originally filed to correct this row's phrasing is itself superseded by the v0_6 PRD's task δ (#4745), which performs this correction. |
...
**Negative-case rationale (mesh-morph is §3.2, not §3.4 — corrected by the v0_6 PRD task δ, #4745):** Mesh-morph engine wiring does **not** insert a ComputeNode at the morph dispatch point. Morph is a realization-kind-dispatch arm for `ReprKind::VolumeMesh` inside `engine_build.rs::dispatch_volume_mesh`, called from `execute_realization_ops` — exactly like the Gmsh tet/hex/wedge arms — producing a `VolumeMesh` realization directly; the morph operation's body composes FEA primitives directly per the existing mesh-morphing PRD (axis-2, unchanged). Live wiring owner: task #4744 (`volume-mesh-realization-and-morph-wiring.md` §8 task β — morph arm in `dispatch_volume_mesh`); tet-path foundation landed as task #4743 (α). Re-homed from cancelled #3429/#2947 — 2947 specified `reify-eval` calling into `reify-mesh-morph` directly, a Cargo cycle (cancelled 2026-05-30); 3429 assumed the realization-producing-ComputeNode framing rejected just below.

**Rejected alternative — why morph is not a §3.4 ComputeNode.** The original framing (`mesh-morphing.md`, resolved 2026-05-12) wrapped the morph dispatch point in a ComputeNode to receive the cache / warm-state / cancellation / significance-filter machinery via the dispatch seam (§2–§5). This is now rejected: the realization-read-api PRD (`docs/prds/v0_6/realization-read-api.md`, tasks 4507–4513, done) made **`ComputeFn` purity a HARD invariant** (§3.2-1) — trampolines *read* realizations, they never *produce* them, and `ComputeOutcome` has only `Completed`/`Cancelled`/`Failed`, deliberately with no `Realized` variant. Wrapping morph in a ComputeNode would re-open that closed seam. The cache/warm-state benefits the §3.4 wrap was meant to provide are already delivered without it: the realization cache (input-hash keyed) memoizes realizations, and `RealizationProjectionStore` memoizes the readable content; FEA warm-start preservation across a morph is a consequence of element-connectivity preservation (a morph-algorithm property) living on the FEA solve ComputeNode's own `OpaqueState`, independent of whether morph itself is §3.2 or §3.4. See `docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §2 and §3 D1 for the full analysis. Both axes (axis-1 = §3.2 dispatch, not ComputeNode; axis-2 = internal composition doesn't call stdlib FEA-as-ComputeNode) are consistent under this contract.
...
| **Mesh-morph — row corrected, does not apply.** Originally framed as "morph runs as a ComputeNode"; corrected (v0_6 PRD task δ, #4745): morph is a §3.2 realization-kind-dispatch producer, not a ComputeNode consumer, so this contract's §7 (a CN-seam boundary-test sketch) does not cover it. | N/A — no ComputeNode seam is exercised by morph. | Moved: the real producer/consumer boundary tests for morph (VolumeMesh demand→execute→read; morph-arm eligible/ineligible ticks with connectivity preservation; FEA warm-start preserved across a morph) live in `docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §7 (tasks α/β boundary-test sketch), not here. |
...
### Phase 9 — Mesh-morph as ComputeNode consumer *(superseded — see correction below)*

**Correction (v0_6 PRD task δ, #4745):** Task κ below (filed as task 3429) is **superseded and cancelled**. 3429 was found unactionable/blocked: it assumed a *realization-producing* ComputeNode — a seam this contract's §4 deliberately left open — which the realization-read-api PRD (tasks 4507–4513, done) has since closed by making `ComputeFn` purity a HARD invariant (no `ComputeOutcome::Realized` variant). Mesh-morph is **not** a ComputeNode consumer; it is a **§3.2 realization-kind-dispatch producer** (morph arm in `dispatch_volume_mesh`, called from `execute_realization_ops`), exactly like the Gmsh tet/hex/wedge arms. Disposition: 3429 cancelled, re-homed to `docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §8 tasks **α** (#4743, §3.2 foundation — landed) and **β** (#4744, morph-or-remesh arm — live, current wiring owner). See the §6 negative-case rationale above for the full rejected-alternative analysis. The task block below is retained for historical record; do not implement it as written.

- **Task κ** *(historical; superseded, do not implement as written)* — Mesh-morph engine wiring via ComputeNode at VolumeMesh realization dispatch.
  - **Observable signal:** A `.ri` parametric design (e.g. `prj/printer_v01/printer.ri` if appropriate; otherwise a synthetic fixture). Varying a non-structural parameter (Stage A + Stage B eligibility) triggers `dispatch_volume_mesh` → ComputeNode-wrapped morph → reused FEA warm-state on subsequent solve. CLI `--verbose` stats show `morphed: true` rather than `remeshed`; mesh-morph PRD's ≥10× wall-clock reduction benchmark passes at 100K elements (task 2953 acceptance). *(Superseded by task β's signal: CLI `--verbose` / `morph_stats` Debug-MCP RPC reports `morphed: 1` on an eligible tick, `ineligible`/`remeshed` on a topology-changing tick — same ≥10×/100K benchmark, task 2953 rewired onto β.)*
  - **Prereqs:** η, ζ. Plus mesh-morphing PRD tasks 2945 (BoundaryAssociation producer side, currently absent per mesh-morph audit M-005) and 2946 (OCCT Projector concrete impl, currently absent per audit M-006). *(Both since landed — 3591/3763 and 3535 respectively.)*
  ...
  - **Supersedes:** task 2947 acceptance. *(2947 was independently cancelled 2026-05-30 — Cargo-cycle defect; see v0_6 PRD §2.)*

### Phase 10 — Companion correction tasks

- **Task μ** *(superseded — see correction)* — Mesh-morphing PRD prose correction: explicitly state axis-1 = "morph routes through ComputeNode" and axis-2 = "morph's internal composition does not call stdlib FEA-as-ComputeNode" — they are orthogonal; current PRD prose is read-ambiguous, this contract resolves the ambiguity. **Corrected:** axis-1 itself was wrong, not merely ambiguous — morph does **not** route through ComputeNode (§3.2 realization-kind dispatch instead; see the §6 negative-case rationale). Task μ (filed as task 3430) is superseded by `docs/prds/v0_6/volume-mesh-realization-and-morph-wiring.md` §8 **task δ** (#4745), which performs this correction (this document, plus `mesh-morphing.md` and the `engine_build.rs` G-allow comment).
  - **Observable signal:** `docs/prds/v0_3/mesh-morphing.md` updated; cross-reference to this contract; no code changes; doc lint passes. *(Signal carried forward unchanged to task δ.)*

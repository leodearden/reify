...
- **Task ε** — `Engine.geometry_kernels: BTreeMap<String, Box<dyn GeometryKernel>>` shape; `with_registered_kernels` constructor; `execute_realization_ops` consults `dispatcher::dispatch` per op and routes to the named kernel from the BTreeMap.
  - **Observable signal:** A `.ri` file declaring two BRep boxes and computing their union evaluates to a `Value::Geometry`; engine inspection confirms an intermediate Mesh realization node in the graph (BRep → Mesh) and a final Mesh realization (Manifold-Boolean output). Re-running hits the realization cache (dispatch-count instrumentation).

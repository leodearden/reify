...
> **The flip anticipated just above has LANDED (task 5905, merge `217b8e1647`) — and the natural typed form is not the one sketched here.** `mode_shape` is now `Field<Point3<Length>, Vector3<Dimensionless>>`, live at `crates/reify-compiler/stdlib/solver_buckling.ri:187` — **not** the `Field<Point3<Length>, Vector3<Length>>` sketched at `:152` and `:183`. The codomain is dimensionless because a mode shape is a normalized eigenvector, not a displacement; the canonical basis is the `RULING (task 5848)` at `crates/reify-compiler/stdlib/trajectory.ri:97-101`, deliberately not restated here.
>
> The domain is unchanged at `Point3<Length>`, so this paragraph's "without surface-API change" prediction held. One naming note for anyone grepping the stdlib: the declared structure is `BucklingMode` (`solver_buckling.ri:185`), not the `Mode` this PRD sketched — renamed because the intra-stdlib pub-name-collision gate (NS-P2, task 5496) rejects two modules declaring `structure def Mode`. Before that gate, buckling and modal analysis both declared one and resolved *inconsistently* — compile-side last-wins (modal) vs eval-side first-wins (buckling), type-checking clean and then evaluating to `Undef` (rationale: `crates/reify-compiler/src/stdlib_loader.rs:542-548`). The surviving `Mode` is `modal_analysis.ri:200`, an unrelated modal-analysis struct with a different field shape (`frequency`, `shape`, `participation_mass`, `damping_ratio`), so a bare `grep "structure def Mode"` lands on the WRONG type.

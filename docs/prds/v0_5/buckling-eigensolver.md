...
>
> The domain is unchanged at `Point3<Length>`, so this paragraph's "without surface-API change" prediction held. One naming note for anyone grepping the stdlib: the declared structure is `BucklingMode` (`solver_buckling.ri:185`), not the `Mode` this PRD sketched — `grep "structure def Mode"` finds nothing.

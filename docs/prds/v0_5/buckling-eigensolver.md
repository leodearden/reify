...
> **The flip anticipated just above has LANDED (task 5905, merge `217b8e1647`) — and the natural typed form is not the one sketched here.** `mode_shape` is now `Field<Point3<Length>, Vector3<Dimensionless>>`, live on `BucklingMode` in `crates/reify-compiler/stdlib/solver_buckling.ri` — **not** the `Field<Point3<Length>, Vector3<Length>>` sketched in the `structure def Mode` block above and repeated in the `fn mode_shape` helper signature below. The codomain is dimensionless because a mode shape is a normalized eigenvector, not a displacement; the canonical basis is the `RULING (task 5848)` in the `Vec3<Q>` header of `crates/reify-compiler/stdlib/trajectory.ri`, deliberately not restated here.
>
> The domain is unchanged at `Point3<Length>`, so this paragraph's "without surface-API change" prediction held. One naming note for anyone grepping the stdlib: the declared structure is `BucklingMode` (`structure def BucklingMode` in `solver_buckling.ri`), not the `Mode` this PRD sketched — renamed because the intra-stdlib pub-name-collision gate (NS-P2, task 5496) rejects two modules declaring `structure def Mode`. Before that gate, buckling and modal analysis both declared one and resolved *inconsistently* — compile-side last-wins (modal) vs eval-side first-wins (buckling), type-checking clean and then evaluating to `Undef` (rationale: `crates/reify-compiler/src/stdlib_loader.rs:542-548`). The surviving `structure def Mode` — today the only one in the stdlib — is `modal_analysis.ri`'s, an unrelated modal-analysis struct with a different field shape (`frequency`, `shape`, `participation_mass`, `damping_ratio`), so a bare `grep "structure def Mode"` lands on the WRONG type.
>
> *Cites in this note are by SYMBOL, not `file:line`, deliberately: a symbol cite survives arbitrary edits above it, and this PRD is live. The as-of-2026-08-08 measured offsets for the 5905 retype — with the per-cite drift history — are recorded once, in the 2026-08-08 row of §10.9.1 in `docs/notes/dimensioned-construction-blast-radius-2026-07-29.md`; that row is the canonical home for the numbers, and it re-measures rather than assumes.*

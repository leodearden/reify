...
| Scenario | Preconditions | Postconditions |
|---|---|---|
| **Shadow-lint suppression.** A `.ri` file with `structure def Outer { param x : Real = 1; sub inner : Inner { ... } } @allow(shadowing) structure def Inner { param x : Real = 1 }`. | Phase-1 ships shadow-lint consumer reading `@allow`. | `reify check` emits W_SHADOW for Outer.x ↔ Inner.x by default; with `@allow(shadowing)` on Inner, the warning is suppressed. CLI exit code unchanged (warning suppression doesn't change pass/fail). |
...
- **Task γ — Shadow-lint reads `@allow(shadowing)` (LEAF; G2 user-observable signal).**
  - **What:** Wire the shadow-lint single-pass walker (per `docs/prds/shadowing-warning.md`) to consult `@allow` annotations on each entity it visits. When the entity has `@allow(shadowing)`, suppress W_SHADOW emission for that entity's shadowed names. Aggregation across multiple `@allow` annotations per §6.
  - **Observable signal:** A `.ri` file under `examples/m_allow_shadowing.ri` (new) containing a deliberately shadowing inner structure annotated with `@allow(shadowing)`, parsed with `reify check`, emits **zero** W_SHADOW diagnostics for that entity. A second `.ri` file with the same shadow shape but without the annotation emits exactly one W_SHADOW. Both verified via CLI text diff in `crates/reify-runner/tests/cli_allow_shadowing.rs`. Closes shadowing-warning.md acceptance criterion 6.

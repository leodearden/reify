...
**Preferred selector form.** Use function-call predicate/role selectors (`faces_by_normal(body, +Z)`, `face(body, …)`, `created_by_feature(…)`) or the already-spec'd `@region(surface, predicate)` ad-hoc form (see the `@region` example in the code block above and the table row). A `let`-bound selector content-hashes its query tree (excluding ephemeral kernel handles) and is therefore re-eval-stable — this is the language's naming mechanism; no new string namespace is introduced. *(The `faces_by_normal(body, +Z)` two-arg form is the **P0 target surface** using direction-literal sugar; it is not yet shipping. The compiling form today is the three-arg `faces_by_normal(solid, vec3(0,0,1), 1deg)` — see the v0.1 compatibility note below.)*
...
**Geometry selector stability.** Predicate selectors (`faces_by_normal(body, +Z)` *(P0 target; shipping form: `faces_by_normal(solid, vec3(0,0,1), tol)`)*, `face(body, …)`) resolve on any kernel that can answer the geometric predicate. Provenance/role selectors (`created_by_feature(…)`) resolve only where a construction-history attribute table exists; they are fail-closed on raw mesh, SDF, and voxel representations with no history.
...
**Layer A landed (v0.6): `Option` recovery via `unwrap_or` / `or_else` / `or_default` / `map_or` / `is_some` / `is_none` / `get_or` (`fallback` is an alias of `unwrap_or`) — see `examples/m6_fallback_recovery.ri` for a runnable end-to-end example. Layer B landed (v0.6): `Result<T, E>` error-handling as a generic data-carrying prelude enum (`Ok { value: T }` / `Err { error: E }`) with match-on-`Result` payload binding and the Result-specialized recovery combinators `unwrap_or` / `or_else` / `fallback` / `is_ok` / `is_err` / `map_err` / `ok_or` — see `examples/m6_result_recovery.ri` for a runnable end-to-end example. Both layers are orthogonal to graph-`Failed` (D1, above). `try`/`catch` and the postfix `?`-propagation operator (fork F-Question) remain deferred — see §18 item 4 and `docs/prds/v0_6/result-and-fallback.md`.**
...
`cost(<collection-expr>)` is the cost-aggregation builtin (M-WHOLE γ): it sums the `line_cost` of every `Costed`-conforming element of a collection expression -- typically `self.descendants` -- into a single `Scalar<Money>`, silently excluding non-`Costed` elements rather than erroring on their absent `line_cost`. It reaches DEPTH-1 descendants only: direct `sub`s and top-level collection elements, not a `Costed` cell nested a further level down. `cost(filter(self.descendants, Costed))` is an equivalent explicit passthrough. See `examples/cost_subtree_aggregate.ri`, covered end-to-end by `crates/reify-eval/tests/cost_subtree_aggregate_eval.rs`. Bound to a plain expression (e.g. via `let`), `cost(...)` is purely observational: it reads already-resolved child cost and does not by itself put a parent objective in joint control of child `auto` parameters -- that distinct, separately-gated capability is described under "Subtree / whole-model cost" below. A scope that is itself `Costed` (the BOM cost-rollup trait -- see `docs/prds/v0_6/io-lifecycle-bom-cost.md`) may instead write `minimize self.line_cost` whenever `line_cost` is closed-form in the scope's own auto parameters. See `examples/continuous_cost_min.ri` for a runnable end-to-end example.
...
**Deferred capabilities -- where the rest lives.** This section covers only single-aspect, closed-form, in-scope cost objectives. Four extensions are named successors to the owning PRD, `docs/prds/v0_6/continuous-cost-minimisation.md` -- the first has since landed with the v1 boundaries noted below; the remaining three are still deferred:

- **Subtree / whole-model cost.** Landed (M-WHOLE β/δ + the joint-drive seam): a parent's `minimize cost(self.descendants)`, written INLINED in the `minimize` -- never behind a `let` -- forms a merged solve cluster spanning parent and child, so the child's `auto` parameters are jointly resolved under the parent's cost objective rather than treated as frozen constants read bottom-up. See `docs/prds/v0_6/whole-model-objective-coupling.md` (M-WHOLE) and `docs/prds/v0_6/whole-model-joint-drive-seam.md` for the coupling mechanism. Worked example: `examples/whole_model_joint_drive.ri`, asserted end-to-end by `crates/reify-eval/tests/joint_drive_expansion_boundary.rs::bt5_parent_objective_drives_child_auto_strictly_below_the_frozen_cascade` (the child auto resolves strictly below its frozen-cascade freeze; the merged whole-assembly cost strictly below the frozen baseline). This is a v1 slice with real boundaries, not an aspirational note: the aggregate must stay inlined in the `minimize` to couple -- `let subtree = cost(...)` followed by `minimize subtree` forms no cluster at all, pinned by `objective_must_inline_the_aggregate_to_couple` in `crates/reify-eval/src/resolve_order.rs`; only depth-1 `Costed` descendants participate; the coupled structure must be reachable by exactly one non-collection instance path for the parent-side surfacing to resolve; and a merged cluster wider than `WHOLE_MODEL_CLUSTER_DIM_CAP` (12 auto dimensions) degrades to the ordinary bottom-up frozen-approximate solve plus a `W_COUPLING_APPROXIMATED` diagnostic instead of failing outright.

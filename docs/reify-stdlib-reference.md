...
These are the registered builtin names (`crates/reify-stdlib/src/joints.rs:674,691,703,717`). Earlier drafts of this section used bare `axis`/`range`/`ratio`/`offset`, which return `Undef` — those names are not registered. No bare aliases are provided: Reify's builtin namespace is flat and global, so an unqualified `axis`/`range` would collide across unrelated stdlib modules; the `joint_`-prefixed spelling is the collision-safe, self-documenting form and is the only one that ships.

**Jacobian.** `joint_jacobian` is a live builtin (`crates/reify-stdlib/src/joints.rs:731`, delegating to
`joint_jacobian_value` at `:775`) that returns the analytic Jacobian column

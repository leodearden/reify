...
| INV-GEO-3 | Every kernel's warm-state classifies every per-handle side table as persist/clear/rebuild; round-trips are drift-guarded | type+test (exhaustive `Self` destructure; per-kernel state-inventory test) | enforced(type+test) — occt (κ); proposed — manifold (μ2) / gmsh (μ3) | kernel-seam-contracts |

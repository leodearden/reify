...
| INV-BUILD-1 | Per-build engine state resets exactly once per build/tessellate entry point; per-build vs must-survive classification is explicit | type (`reset_per_build_state` exhaustive destructure; later `BuildSession` ownership) | enforced(type) | engine-build-hardening |
| INV-BUILD-2 | Version/snapshot IDs are allocated and read through exactly one API each | type + lint (allocator helper; grep-gate on raw `next_version_id` arithmetic) | enforced(type+lint) | engine-build-hardening |
| INV-BUILD-3 | A cache-hit short-circuit produces the same observable side-effect set as the path it short-circuits | test (extracted `probe_realization_cache` + parity tests; the #3226 empty-`topology_attribute_table` postcondition on a cache-served handle is a fail-closed `debug_assert!`, so that sub-postcondition's release-mode share leans on INV-BUILD-1, now `enforced(type)` as of task ι/#5069 — a compile-time guarantee rather than an unenforced proposal, which strengthens this invariant's release-mode standing) | enforced(test) | engine-build-hardening |
| INV-GEO-1 | Mesh handoffs satisfy the written interchange contract: producer obligations (closed, consistently wound, finite, index-valid) vs consumer-declared capabilities (weldedness) | type+test (MeshContract validator at the seam; fail-closed with `REIFY_MESH_CONTRACT` break-glass. Coverage boundary — sites 1–2 only: site 1 (tessellate→ingest conversion) is scoped to the BRep→Mesh tessellate producer, so MarchingCubes (Voxel→Mesh) output is gated only when its consumer is site 2 (Manifold ingest) and passes ungated to an OpenVDB/gmsh consumer; site 3 (gmsh) is covered separately by the #4876 preflight, immediate-enforce with no knob) | enforced(type+test) | kernel-seam-contracts |
| INV-GEO-2 | Handle identity is (kernel, id); `extract_*` results are stable per parent within a session | type+test (KernelHandle-keyed tables — landed by #4351: `TopologyAttributeTable.entries` is keyed by `KernelHandle`, so a cross-kernel `GeometryHandleId` collision is unrepresentable, pinned by `topology_attribute_table_distinguishes_same_id_across_kernels` in `reify-ir` + `crates/reify-eval/tests/cross_kernel_attribute_collision_e2e.rs`; conformance property tests — OCCT + Manifold `extract_faces`/`extract_edges` handle-stability arms in `handle_stability_conformance.rs`; engine-build share — cache-hit fail-closed assert delivered by task θ/#5064, which retired the #4349 defensive eviction) | enforced(type+test) | kernel-seam-contracts + engine-build-hardening |
...
| INV-GEO-4 | Stub and real kernels satisfy one shared contract-test suite | test (suite instantiated under the `not(has_occt)` stub cfg; `has_occt` real-kernel arm not yet wired — tracked as a follow-up, see `assert_kernel_contract!` docstring) | enforced(test) | kernel-seam-contracts |
...
| INV-GUI-2 | Every engine-mutation entry point (GUI command, debug server, MCP/AI tools) flows through the same delta choke-point | test (architecture test) + routing. Coverage boundary — the AI/MCP entry point only: `gui/src-tauri/src/tests/debug_write_tool_routing_tests.rs` structurally enforces that every `reify_*` write tool reaches a `*_and_refresh_baseline` seam and never emits privately, enumerating the dispatch arms so a new tool must route or go red, and asserting each seam really reaches `compute_delta`. Fail-closed (default-ASSERT, no read-only exemptions, non-vacuity floors), with a `REIFY_INV_GUI_2_BYPASS=1` break-glass. The GUI/debug/FS-watcher half has NO structural guard — it is covered behaviorally by the `<name>_emits_fea_diagnostics` cluster in `gui/src-tauri/src/tests/engine_tests.rs` | proposed | gui-state-sync (core + GUI/debug/watcher); ai-native-editing (AI/MCP entry point) |
...
| INV-FEA-3 | NaN never enters an ordering | lint+test (`f64::total_cmp`/finite-guards; grep gate on `partial_cmp(...).unwrap_or` scoped to the FEA numeric solve path — NOT all numeric crates: deliberately-excluded set, per-site census and widening trigger in `docs/prds/compute-fea-hardening.md` "Resolved design decision 9") | proposed | compute-fea-hardening |

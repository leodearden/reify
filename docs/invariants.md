...
| INV-GEO-2 | Handle identity is (kernel, id); `extract_*` results are stable per parent within a session | type+test (KernelHandle-keyed tables, #4351, still `proposed`; conformance property tests, `enforced(test)` — OCCT + Manifold `extract_faces`/`extract_edges` handle-stability arms, `handle_stability_conformance.rs`) | enforced(test) | kernel-seam-contracts + engine-build-hardening |

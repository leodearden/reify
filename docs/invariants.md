...
| INV-GEO-2 | Handle identity is (kernel, id); `extract_*` results are stable per parent within a session | type+test (KernelHandle-keyed tables, #4351, still `proposed`; conformance property tests, `enforced(test)` — OCCT + Manifold `extract_faces`/`extract_edges` handle-stability arms, `handle_stability_conformance.rs`; engine-build share — cache-hit fail-closed assert — delivered by task θ/#5064) | enforced(test) | kernel-seam-contracts + engine-build-hardening |

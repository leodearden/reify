...
| INV-GEO-1 | Mesh handoffs satisfy the written interchange contract: producer obligations (closed, consistently wound, finite, index-valid) vs consumer-declared capabilities (weldedness) | type+test (MeshContract validator at the seam; fail-closed with `REIFY_MESH_CONTRACT` break-glass) | enforced(type+test) | kernel-seam-contracts |

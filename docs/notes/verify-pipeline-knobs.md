...
**Cross-repo seam:** the merge-worker trivial-pass classifier is dark-factory code and **must be wired to consult this script** before taking the config-only fast-path. Reify ships the oracle; dark-factory does the wiring (tracked separately as a non-blocking follow-up to esc-4288-206).

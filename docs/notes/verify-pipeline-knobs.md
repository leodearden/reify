...
- **Closed DF-side (dark-factory task 4212, 2026-08-19):** DF's `SEMAPHORE_TIMEOUT` detector parses `disposition=` and fires only for a non-soft (`fatal`) sentinel; a `disposition=soft` sentinel is now advisory and falls through to per-tool dispatch instead of being classified as a starvation abort. The three wrapper basename anchors (`lib_test_semaphore.sh`, `cargo-test-occt-gated.sh`, `lib_lane_x_flock.sh`) stay ungated because all three take `slot_acquire`'s `fatal` default and so are unaffected by the gate. A sentinel with no `disposition=` field keeps DF's pre-4212 classify behavior — closing this needed no second reify-side change, since the field was already on the wire.

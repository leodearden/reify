...
- **`@@REIFY_SLOT_TIMEOUT@@`** `reason=<reason> lock=<lock-base> slots=<N> waited=<secs>` — emitted ONCE to stderr on a finite-WAIT `slot_acquire` deadline (the rc=75 path). Never emitted under `WAIT=unlimited`, never on an uncontended acquire — both are structural (the emit sits inside the finite-mode branch), not conditional.

...
| 2026-08-08 | `217b8e1647` (task 5905, landed after this ledger's `08c6c42be9` anchor) | **Two cited field TYPES moved.** §8's stdlib-census row `solver_buckling.ri:176` (`:1799`) — the field is now `mode_shape : Field<Point3<Length>, Vector3<Dimensionless>>` at **`:187`**. §9.2's `Gravity` row (`:1889`, cited `:515`) — `direction : List<Real>` is now `Vector3<Dimensionless>` at **`:519`**; struct head **`:517`**, `magnitude` **`:518`** (the `:516` cite in §9.1). Offset drift was re-measured **per cite** rather than attributed wholesale, and the attribution differs from the obvious guess: every cite in this ledger was CORRECT at its `08c6c42be9` anchor, and `PointLoad :315` has **not** moved at all — it still resolves today. `PressureLoad 418`, `TractionLoad 446`, `BodyForce 476`, `Gravity 515`, `Gravity.magnitude 516` read **419 / 447 / 477 / 517 / 518** today, but most of that drift (+2/+3) PREDATES 5905, which moved each by exactly **−1**; `mode_shape` went `176 → 178` before 5905 and `178 → 187` within it (its own 9-line rationale block). Re-measure before citing any of them. | **NEUTRAL** — no count changed. Proved in three parts, each re-verified against the file rather than assumed: (i) §8 counts SITES, and its `solver_elastic.ri` row (`:1800`) is annotated "(domain always dimensioned; codomain varies)" — `mode_shape`'s DOMAIN is still `Point3<Length>`, so the site stays counted and §8's **34 sites / 9 files** (`:1786`) is unchanged; only the cell's type text and the `:176`→`:187` anchor drift. (ii) §8.2 (`:1806-1807`) already excludes `Vector3<Dimensionless>` BY NAME — `examples/dimensionless_unification.ri:50-51`, "dimensionless, no residual" — so the retyped `direction` fields stay excluded exactly as `List<Real>` was and no site ENTERS the census. (Path spelled in full deliberately: that exclusion basis is the one file cited in this row that does **not** live under `crates/reify-compiler/stdlib/`, and §8.2's own shorthand at `:1806` omits the prefix, so a reader re-grepping the stdlib census directory for it finds nothing and cannot tell whether the basis was removed.) (iii) §9.2's `Gravity` **Yes** verdict rests on "`Acceleration` is a named dimension already in the §0.1 name set" — i.e. on `magnitude`, not on `direction` — so it is untouched; and §9.1's "`Gravity.magnitude` … is the **ONLY** dimensioned Scalar field in the entire FEA-load cluster" (`:1875`) still holds, since 5905 introduced no dimensioned Scalar. | Not an error in the ledger — the tree moved under a point-in-time anchor, exactly as §10.9's own rule predicts ("anchors are point-in-time … every figure must be re-verified at whatever HEAD it is being cited from"). Filed because a §8 consumer who greps `solver_buckling.ri:176` finds nothing and cannot otherwise tell whether the site was migrated out of the census or the ledger was wrong — the same legibility gap the `:2277` guard was filed to close, one anchor later. Why the field takes a dimensionless slot is settled canonically in tracked source (the `RULING (task 5848)` at `trajectory.ri:97-101`, restated for this cluster at `fea_multi_case.ri:335-343`) and is deliberately not restated here. **No figure in this ledger edited.** |

...
Drive is by **anchored (winch) capstan, not friction wrap**: both rope ends are fixed to a 48mm grooved drum (D/d = 8 on the 6mm rope), so the coupling is positive — zero slip. A friction capstan would accumulate micro-slip under the rapid, unbalanced reversals and resonant excitation of high-acceleration printing (unacceptable for positional accuracy), and the relative rope/drum creep would shorten tendon life — both rule it out. The drum reels rope onto one side as it pays out the other; its axial length is sized for the full per-axis feed (~1.3m → ~8.6 active turns on the drum's ~151mm pitch circle at a 7mm helical lead) plus 4 dead (anchor) wraps — ~12.6 turns, ~88mm of grooved length — and it is helically grooved so the wrap lay is deterministic. That lead is deliberately a *clearance* one: at 7mm against the 6mm rope each wrap keeps a land ridge between it and its neighbour, where a lead equal to the rope diameter would merge adjacent seats and let the wraps wedge (drifting the effective diameter, hence the transmission ratio). Because the wrap band migrates axially (~one lead/turn, ~60mm over full travel), each strand runs through a **passive translating fairlead** that tracks the band and holds the fleet angle ≈0 — a guide, not a tensioner, so it adds no compliance; its 63mm stroke is that migration rounded up to a whole turn. Larger drum/idler D/d trades footprint for rope bend-fatigue life (a tendon-life lever). Exact figures live in `prj/printer_v01/dev_capstan.ri`, which evaluates and is gated end-to-end — go there rather than to this paragraph when precision matters.

...
Drive is by **anchored (winch) capstan, not friction wrap**: both rope ends are fixed to a 48mm grooved drum (D/d = 8 on the 6mm rope), so the coupling is positive — zero slip. A friction capstan would accumulate micro-slip under the rapid, unbalanced reversals and resonant excitation of high-acceleration printing (unacceptable for positional accuracy), and the relative rope/drum creep would shorten tendon life — both rule it out. The drum reels rope onto one side as it pays out the other; its axial length is sized for the full per-axis feed (~1.3m of rope at ~151mm per turn — π × the 48mm pitch diameter — is ~8.6 active turns at the 7mm helical lead) plus 4 dead (anchor) wraps, and it is helically grooved so the wrap lay is deterministic. That lead is deliberately a *clearance* one: at 7mm against the 6mm rope each wrap keeps a land ridge between it and its neighbour, where a lead equal to the rope diameter would merge adjacent seats and let the wraps wedge (drifting the effective diameter, hence the transmission ratio). Because the wrap band migrates axially (one lead per active turn, ~60mm over full travel), each strand runs through a **passive translating fairlead** that tracks the band and holds the fleet angle ≈0 — a guide, not a tensioner, so it adds no compliance; its stroke is that migration rounded up to a whole turn. Larger drum/idler D/d trades footprint for rope bend-fatigue life (a tendon-life lever). Exact figures live in `prj/printer_v01/dev_capstan.ri`, which evaluates and is gated end-to-end — go there rather than to this paragraph when precision matters.
...
| D | Toolhead carriage + dock | 4-tool Maxwell kinematic mount on the carriage's REAR face (horizontal engagement, cam lock on an Ioni-HC drive), rear-lintel dock (4 stations at 80 mm pitch, 2 fitted for v1), electrical/cooling/air connectors. Modelled 2026-09-03 (`ToolBody`/`DToolhead`/`ToolDock` in `printer.ri`, with the pickup-trajectory clearance study) |
...
3. **Kinematic constraint primitives** — would be valuable for verifying counter-mass synchronicity, toolchanger dock pickup paths, Z-tilt range. Not reify's current strength. Workaround: static representations + numerical hand-checks; defer kinematic constraints as a future reify feature. *Status 2026-09-03:* open-chain FK is usable — the Z-lift (three prismatic columns) and the dock pickup path (plunge + traverse) are modelled as mechanisms with per-pose clearance margins (`sweep` + `min_clearance`) and a collision gate built from FK-posed sampled swept unions checked by penetration volume. No swept-volume primitive exists and distance queries are blind to full containment on boolean bodies, so the sampled-union volume gate is the honest form. Closed loops remain unusable.
...
| 5 | Toolhead + dock: dock geometry on frame, 2 tools for v1 (Rapido HF or equivalent), kinematic mount, electrical/cooling, tool change cycle test. Dock modelled 2026-09-03; commissioning is gated on the Y-rail lengthening and the rear drive-web externalization (the dock sits in the rear strip both of those free up) | Yes | Yes |
...
- **Rear-lintel toolchanger dock, tool on the carriage's rear face** (2026-09-03). The gantry's lower rail band sweeps the whole footprint at the height parked tools must sit (nozzle plane and up), so tools park outside that sweep: the rear strip between the back wall and the gantry's rear limit (the front strip is the open access aperture; there is no side room). The dock is a lintel spanning the rear pillars — it doubles as the rear top tie the 620 mm back wall cannot provide — with four stations at 80 mm pitch, each two forward-opening pin pockets; a tool hangs on two rear pins with its Maxwell balls facing forward. Engagement is HORIZONTAL (plunge onto the balls, cam lock, extract) because the head has no Z and the bed's Z cannot lift a tool. Consequences accepted: the nozzle sits ~91 mm behind the carriage centre, so the rail-lengthening work must add that much Y reach at the front (or the bed loses it), and the dock is unreachable until the rails are lengthened and the rear drive web is externalized — both already mandated for full travel. Tool envelope 60×60×150 mm / 1.2 kg; head payload 1.85 kg inside the 2 kg drive-budget assumption; cam preload 300 N against ~200 N needed at 100 m/s² (the 1000 m/s² stretch needs ~1.8 kN — beyond a cam; open item for the stretch round). The under-carriage alternative (raise the motion plane ~130 mm, no nozzle offset, taller pillars/lintel) was presented and not chosen.

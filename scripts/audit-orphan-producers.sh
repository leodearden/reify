...
# reify-test-support. Two of its publics — ignore_hygiene::extract_ignore_reason
# and git_env::{REPO_REDIRECT_VARS, sanitize} — are reached from reify-audit's
...
# assumed-safe one. What justifies those two `pub`s instead, and the open
# question about the shape of the arrangement, are argued once in `sanitize`'s

...
_mws="$(grep '^MAX_WAIT_SECS=' "$GENERATE_SCRIPT" | head -1 | sed 's/[^=]*=//; s/[^0-9].*//')"
_mlws="$(grep '^MAX_LOCK_WAIT_SECS=' "$GENERATE_SCRIPT" | head -1 | sed 's/[^=]*=//; s/[^0-9].*//')"

assert "MAX_WAIT_SECS parsed from generate script" test -n "$_mws"
assert "MAX_LOCK_WAIT_SECS parsed from generate script" test -n "$_mlws"

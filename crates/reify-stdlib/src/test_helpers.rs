//! Shared test helpers for reify-stdlib unit tests.
//!
//! Imported in each submodule's `#[cfg(test)] mod tests` via `use crate::test_helpers::*;`.
...
pub(crate) use assert_real_approx;
...
pub(crate) use assert_scalar_approx;
...
pub(crate) use assert_vector3_approx;
...
pub(crate) use assert_orientation_approx;
...
/// Build a 3-component `Value::Point` with LENGTH dimension (1.0, 2.0, 3.0).
pub(crate) fn make_point3_len() -> Value {
    Value::Point(vec![
        Value::length(1.0),
        Value::length(2.0),
        Value::length(3.0),
...
/// Build a 3-component `Value::Point` with LENGTH dimension (1.0, 2.0, 3.0).
///
/// Alias for [`make_point3_len`].
pub(crate) fn make_point3_length() -> Value {
    Value::Point(vec![
        Value::length(1.0),
        Value::length(2.0),
        Value::length(3.0),
...
/// Build a 2-component `Value::Point` with LENGTH dimension (1.0, 2.0).
pub(crate) fn make_point2_length() -> Value {
    Value::Point(vec![Value::length(1.0), Value::length(2.0)])
}

/// Build an identity `Value::Orientation` (w=1, x=0, y=0, z=0).
pub(crate) fn make_identity_orientation() -> Value {
    Value::Orientation {
        w: 1.0,
        x: 0.0,
        y: 0.0,
...
pub(crate) fn make_matrix(rows: &[&[f64]]) -> Value {
    Value::Tensor(
        rows.iter()
...
pub(crate) fn make_dimensioned_matrix(rows: &[&[f64]], dim: DimensionVector) -> Value {
    Value::Tensor(
        rows.iter()
            .map(|row| {
                Value::Tensor(

...
mod tests {
    ...
    /// `reify_eval::modal_ops::tests::trampoline_shapes_modal_result_with_rayleigh_damping`,
    /// which independently recomputes ω = 2π·f from the emitted
    /// `Mode.frequency` and compares `damping_ratio` against it at 1e-12 — the
    /// test that genuinely observes the producer's scale.
    ///
    ...
    /// ⇒ ζ=0.1 — is the sibling `rayleigh_damping_ratio_mass_proportional`
    /// above and is deliberately NOT restated here.
    ///

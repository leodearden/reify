...
/// Construction of Point3<Length> as Value::Point with 3 length components.
#[test]
fn construct_point3_length() {
...
/// Construction of Vector3<Length> as Value::Vector with 3 length components.
#[test]
fn construct_vector3_length() {
    let expr = CompiledExpr::literal(
        Value::Vector(vec![
            Value::length(4.0),
            Value::length(5.0),
...
/// Construction of Point2<Length> as Value::Point with 2 length components.
#[test]
fn construct_point2_length() {
    let expr = CompiledExpr::literal(
        Value::Point(vec![Value::length(7.0), Value::length(8.0)]),
        Type::point2(Type::length()),
    );
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
    assert_eq!(result, Value::Point(vec![Value::length(7.0), Value::length(8.0)]));
}

/// Construction of Vector2<Dimensionless> as Value::Vector with 2 Real components.
#[test]
fn construct_vector2_dimensionless() {
    let expr = CompiledExpr::literal(
    ...
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
    assert_eq!(result, Value::Vector(vec![Value::Real(1.0), Value::Real(2.0)]));
}

// ─── step-1: Basic .x / .y / .z on Point3<Scalar[m]> ───

/// .x on a Point3 returns component[0].
#[test]
fn eval_point3_x_returns_first_component() {
    let point = CompiledExpr::literal(
        Value::Point(vec![
            Value::length(1.0),
            Value::length(2.0),
            Value::length(3.0),
    ...
    let expr = CompiledExpr::method_call(point, "x".to_string(), vec![], Type::length());
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
    assert_eq!(result, Value::length(1.0));
}

/// .y on a Point3 returns component[1].
#[test]
fn eval_point3_y_returns_second_component() {
    ...
    let expr = CompiledExpr::method_call(point, "y".to_string(), vec![], Type::length());
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
    assert_eq!(result, Value::length(2.0));
}

/// .z on a Point3 returns component[2].
#[test]
fn eval_point3_z_returns_third_component() {
    ...
    let expr = CompiledExpr::method_call(point, "z".to_string(), vec![], Type::length());
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
...
fn eval_vector3_x_returns_first_component() {
    let vector = CompiledExpr::literal(
        Value::Vector(vec![
            Value::length(10.0),
            Value::length(20.0),
            Value::length(30.0),
    ...
    let expr = CompiledExpr::method_call(vector, "x".to_string(), vec![], Type::length());
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
...
fn eval_vector2_xy_preserves_angle_dimension() {
    ...
    let expr_x = CompiledExpr::method_call(vector.clone(), "x".to_string(), vec![], Type::angle());
    let expr_y = CompiledExpr::method_call(vector, "y".to_string(), vec![], Type::angle());
    let values = ValueMap::new();
...
/// .x on a Point2<Scalar[m]> returns the first component.
#[test]
...
/// .y on a 1-component Vector (N=1, index 1 out of bounds) returns Undef.
#[test]
fn eval_vector1_y_out_of_bounds_returns_undef() {
    let vector = CompiledExpr::literal(
        Value::Vector(vec![Value::length(1.0)]),
        Type::point2(Type::length()), // type annotation irrelevant to eval
    );
    let expr = CompiledExpr::method_call(vector, "y".to_string(), vec![], Type::length());
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
    assert_eq!(result, Value::Undef);
}

/// .z on a 1-component Vector (N=1, index 2 out of bounds) returns Undef.
#[test]
fn eval_vector1_z_out_of_bounds_returns_undef() {
    let vector = CompiledExpr::literal(
        Value::Vector(vec![Value::length(1.0)]),
        Type::point2(Type::length()),
    );
    let expr = CompiledExpr::method_call(vector, "z".to_string(), vec![], Type::length());
    let values = ValueMap::new();
    let result = eval_expr(&expr, &EvalContext::simple(&values));
    assert_eq!(result, Value::Undef);
}

/// .x on a non-Point/Vector value (Value::Int) returns Undef.
#[test]
...
/// .x on a Value::List (distinct from Point/Vector) returns Undef.
#[test]
...
// --- Vector add/sub arithmetic ---

/// Vector3<Length> + Vector3<Length> produces component-wise sum as Value::Vector.
#[test]
...
/// Vector3<Length> - Vector3<Length> produces component-wise difference as Value::Vector.
#[test]
...
/// Point3<Length> + Vector3<Length> returns Value::Point (point displaced by vector).
#[test]
...
/// Vector3<Length> + Point3<Length> returns Value::Point (commutative, vector + point).
#[test]
...
/// Point3<Length> - Point3<Length> returns Value::Vector (displacement vector).
#[test]
...
/// Adding vectors of different N (3 vs 2) returns Undef.
#[test]
...
fn vector3_add_different_dimension_returns_undef() {
    ...
    let right = CompiledExpr::literal(
        Value::Vector(vec![Value::angle(0.1), Value::angle(0.2), Value::angle(0.3)]),
...
/// Scalar<Mass> * Vector3<Length> gives Vector with Mass*Length dimension per component.
#[test]
...
#[test]
fn undef_operand_in_vector_binop_propagates() {
    let undef_ref = {
...
/// Vector with a Undef component + Vector propagates Undef through component-wise add.
#[test]
fn vector_with_undef_component_in_add_propagates() {
    let left = CompiledExpr::literal(
...
/// Negating Vector3<Length> negates all components; result is Value::Vector.
#[test]
...
/// Negating Point3<Length> negates all components; result is Value::Point.
#[test]
fn negate_point3_negates_all_components() {
    let operand = CompiledExpr::literal(
        Value::Point(vec![Value::length(1.0), Value::length(2.0), Value::length(3.0)]),
        Type::point3(Type::length()),
    );

...
// FIXME(#4692): SingleKernelHolder must delegate EVERY optional `GeometryKernel`
// capability method (extract_edges, extract_faces, extract_vertices, execute_split,
// ingest_mesh, attribute_hook, densify_grid_to_sampled, ...) to its inner kernel. The
// trait's default impl silently masks missing delegation — returning None/not-supported
// instead of the inner kernel's real result. Deferred (not just a mechanical add):
// un-masking extract_faces/extract_edges surfaces a latent feature_datum analytic-dedup
// bug — OCCT's real sub-faces/sub-edges yield a 2nd, non-coaxial axis candidate that
// fails to dedup (→ FeatureDatumAmbiguous on feature_datum_axis.ri), so delegation must
// land together with the feature_datum fix tracked in #4692.

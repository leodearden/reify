...
//! | BT9  | CLI surface (S1)                    | `reify-cli/tests/harness_cli/cli_undef_self_describing.rs` fns `eval_undef_emits_note_with_complete_cause_set`, `eval_determined_emits_no_undef_notes`, `eval_explain_undef_surfaces_all_undef_cells`, `eval_default_gates_out_unbound_param_subject_lines` |
...
//! | BT12 | Surface agreement (S4)              | NOT covered anywhere upstream — inherently an integration test no single dependency task could write. Added (genuinely new) as SET-level anchors across `reify-cli/tests/harness_cli/cli_undef_self_describing.rs`, `gui/src-tauri/src/tests/engine_tests.rs`, `reify-lsp/src/hover.rs` (see those files; see design_decisions for why set-level, not byte-identical strings) |
//!

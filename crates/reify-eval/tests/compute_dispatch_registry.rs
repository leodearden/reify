...
fn instance_scope_optimized_param_default_reading_a_later_sibling_is_silent() {
    ...
    // INSTANCE scope now agrees — and the plain `q` control is what makes this a
    // property of the param loop rather than of the reuse gate. `q` is a call to
    // a non-`@optimized` fn, so `resolve_optimized_instance_cell` returns
    // `NotOptimized` for it and never runs the read comparison; it degraded
    // before the fix and resolves after it just like `p`. Whatever fixed one
    // fixed both.
    for member in ["p", "q"] {
        let template_id = ValueCellId::new("InnerFwd", member);
        let instance_id = ValueCellId::new("OuterFwd.a", member);
        assert_eq!(
            eval_result.values.get(&instance_id),
            eval_result.values.get(&template_id),
            "instance scope must agree with template scope at OuterFwd.a.{member}: \
             `elaborate_child_params_only` visits params in dependency order, so \
             the default evaluates against a `child_values` that already holds \
             `seed`. An `Undef` here is the declaration-order gap returning"
        );
        assert_eq!(
            eval_result.values.get(&instance_id),
            Some(&Value::Int(0)),
            "OuterFwd.a.{member} must be the body-inlined sentinel Int(0), the \
             same value template scope holds — asserted alongside the equality \
             above so this cannot pass by both scopes degrading together"
        );
    }

    // THE TRIPWIRE. `OuterFwd.a` carries no ctor override at all, and the loop
    // is dependency-ordered, so this instance's inputs are value-identical to
    // the template's and there is nothing left for a decline to name. It stays
    // absent only while the registered-target gate is false for param defaults.
    let declines: Vec<_> = eval_result
        .diagnostics
        .iter()
        .filter(|d| d.severity == Severity::Warning)
    ...
    assert!(
        ...
        "a forward-referencing @optimized param default must not report a \
         decline: the instance has no ctor override and the param loop is \
         dependency-ordered, so nothing about this instance differs from the \
         template. If this is now non-empty, #6750 has closed the template-scope \
         gap and flipped the registered-target gate — read the reported cause \
         before assuming the gate is wrong. Got: {:?}",
        declines
    );
}

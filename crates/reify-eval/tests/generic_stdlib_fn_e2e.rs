//! Stdlib gate tests for `pub fn identity<T>(x: T) -> T` in std.fields
...
//! `pub fn identity<T>(x: T) -> T { x }` to crates/reify-compiler/stdlib/fields.ri.
...
/// CompiledFunction named "identity" with a non-empty `type_params` list.
///
/// Proves that `pub fn identity<T>` in fields.ri type-checks correctly during
...
fn stdlib_loads_generic_fn_clean() {
    ...
    let identity_fn = fields_module
        ...
        .expect(
            "std.fields must contain a function named 'identity' (pub fn identity<T>(x: T) -> T)",
        );

    assert!(
        !identity_fn.type_params.is_empty(),
        "identity must have at least one type parameter (T), got empty type_params"
    );
}

// ── (b) identity evals end-to-end from user source ───────────────────────────

/// A user source `structure S {{ let v = identity(5mm) }}` (identity is
...
fn stdlib_generic_fn_evals_end_to_end() {
    ...
    assert!(
        errors.is_empty(),
        "compile errors for identity(5mm): {errors:?}"
    );
    ...
    assert!(
        ...
        "eval errors for identity(5mm): {eval_errors:?}"
    );
    ...
    assert_eq!(
        *value,
        ...
        "identity(5mm) should evaluate to the 5mm length scalar, got {value:?}"
    );
}

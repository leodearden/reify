            ...
            for template in &module.templates {
                ...
                for realization in &template.realizations {
                    ...
                    Engine::execute_realization_ops(
                        ...
                        &self.meta_map,
                        RealizationOutputs {
                            step_handles: &mut step_handles,
                            named_steps: &mut named_steps,
                            feature_tag_table: &mut self.feature_tag_table,
                            topology_attribute_table: &mut self.topology_attribute_table,
                            swept_kind_table: &mut self.swept_kind_table,
                        },
                        &mut diagnostics,
                        &realization.id,
            ...
            for realization in &template.realizations {
                ...
                Engine::execute_realization_ops(
                    ...
                    RealizationOutputs {
                        step_handles: &mut step_handles,
                        named_steps: &mut named_steps,
                        feature_tag_table: &mut *feature_tag_table,
                        topology_attribute_table: &mut *topology_attribute_table,
                        swept_kind_table: &mut *swept_kind_table,
                    },
                    diagnostics,
                    &realization.id,
                    realization.name.as_deref(),
...
mod tests {
    ...
    // ── RealizationOutputs aggregate API test (task 3119) ─────────────────────

    /// Drives `execute_realization_ops` through the post-refactor aggregate
    /// parameter signature.  Constructs a `RealizationOutputs { … }` literal
    /// and passes it in place of the five individual `&mut` output arguments.
    ///
    /// This test was written *before* the `RealizationOutputs` struct and the
    /// new signature were introduced (step-1 of task 3119 — the TDD "red"
    /// phase).  It fails to compile until step-2 defines the struct and
    /// updates `execute_realization_ops` to accept `outputs:
    /// RealizationOutputs<'_>`.
    ///
    /// Post-refactor assertions (happy path, `realization_name = Some("body")`):
    ///   (a) `step_handles.len() == 1`  — one handle appended via the aggregate
    ///   (b) `named_steps.get("body") == Some(&step_handles[0])` — the function
    ///       populated the aggregate's `named_steps` field
    ///   (c) no `Severity::Error` diagnostics
    #[test]
    fn execute_realization_ops_accepts_realization_outputs_aggregate() {
        use reify_compiler::{CompiledGeometryOp, PrimitiveKind};
        use reify_test_support::mocks::MockGeometryKernel;
        use reify_types::{CompiledExpr, Severity, Type};
        ...
        // (a) one handle appended via the aggregate
        assert_eq!(step_handles.len(), 1, "expected one handle appended");

        // (b) the aggregate's named_steps was populated
        assert_eq!(
            named_steps.get("body").copied(),
            Some(step_handles[0]),
            "named_steps[\"body\"] should equal the handle returned by the kernel"
        ...
        // (c) no Error diagnostics (warnings from mock-kernel seeder are expected)
        let errors: Vec<_> = diagnostics
            .iter()

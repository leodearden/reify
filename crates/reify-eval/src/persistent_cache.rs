...
/// Re-exported from [`reify_compute_contract`], which owns [`ElasticResult`],
/// [`ShellChannels`], and [`max_deflection_magnitude`] (task A / #4934 — the
/// orphan rule forces the FEA-result island, including its
/// `PersistentlyCacheable` impl and drift-guard `From<PartialElasticResult>`
...
/// below is the sole remaining reify-eval consumer.
use reify_compute_contract::{check_f64_vec_len, read_f64_slab, write_f64_slab};
...
///
/// Separate from `reify_compute_contract`'s private `ELASTIC_RESULT_FORMAT_VERSION`
/// and [`ENGINE_VERSION_HASH`]
...
/// discipline to `reify_compute_contract`'s private `ElasticResultHeader`).
///
...
/// [`check_f64_vec_len`] / [`reify_compute_contract::MAX_F64_ELEMENTS`]).
///
...
// G-allow: task #2978 (done) stale-tempfile sweep; called by the sweep_persistent_cache_at_startup engine-admin wrapper
...
// G-allow: task #2978 (done) orphan-engine-version pruning; called by the sweep_persistent_cache_at_startup engine-admin wrapper
...
mod tests {
    ...
    // ── engine-hash closure-pin tests (task 5272) ────────────────────────────
    //
    // These exercise the narrowed Cargo.lock contribution to ENGINE_VERSION_HASH.
    // Instead of hashing the WHOLE workspace lockfile, build.rs now hashes only
    // the (name, version) pins of reify-eval's build+normal (exclude-dev) closure
    // (the crate NAMES listed in crates/reify-eval/engine_hash_closure.txt). The
    // pure parse + filter helpers live in engine_hash_algo.rs (shared with
    // build.rs via include!). PRD: docs/prds/merge-gate-compile-cost.md §3 W4/§5 C4.

    /// `parse_cargo_lock_packages` extracts exactly the (name, version) pair of
    /// every `[[package]]` stanza, in file order, and NOTHING else:
    ///   (a) the top-level lockfile `version = N` line (outside any stanza) is
    ///       NOT captured as a package;
    ///   (b) non-`[[package]]` tables (`[metadata]`, `[[patch.unused]]`) end
    ///       field capture — a `name`/`version` inside them is ignored;
    ///   (c) stanzas carrying extra `source`/`checksum`/multi-line
    ///       `dependencies = [...]` lines still yield exactly name+version.
    #[test]
    fn parse_cargo_lock_packages_extracts_name_version_in_file_order_ignoring_non_package_fields() {
...
[metadata]
"some legacy metadata key" = "value"

[[patch.unused]]
name = "ghost-should-be-ignored"
version = "9.9.9"
"#;
        let pkgs = crate::engine_hash_algo::parse_cargo_lock_packages(lock);
        assert_eq!(
            pkgs,
            vec![
                ("aho-corasick".to_string(), "1.1.3".to_string()),
                ("anyhow".to_string(), "1.0.86".to_string()),
                ("zzz-workspace-crate".to_string(), "0.1.0".to_string()),
            ],
            "must capture exactly the [[package]] (name, version) pairs in file \
             order; the top-level `version = 3`, the [metadata] table, and the \
             [[patch.unused]] name/version must all be excluded"
        );
    }

    /// Lock text with no `[[package]]` stanzas yields an empty Vec (comment /
    /// blank / top-level-version / non-package-table lines contribute nothing).
    #[test]
    fn parse_cargo_lock_packages_returns_empty_for_lock_without_packages() {
        let lock = "# only comments\nversion = 4\n\n[metadata]\nfoo = \"bar\"\n";
        let pkgs = crate::engine_hash_algo::parse_cargo_lock_packages(lock);
        assert!(
            pkgs.is_empty(),
            "a lock with no [[package]] stanzas must yield an empty Vec, got {pkgs:?}"
        );
    }
    ...
    /// by the entry-layer (write_entry/read_entry/eviction/sweep) tests below,
    /// so each test gets a fresh copy.
    ///
    /// Duplicated from `reify_compute_contract::elastic_result`'s own
    /// `make_sample_result` (task A / #4934): that copy backs the
    /// ElasticResult-only serialization tests which moved with the type;
    /// this copy backs the cache-entry-layer tests that stay in reify-eval
    /// and merely need a concrete `PersistentlyCacheable` sample value.

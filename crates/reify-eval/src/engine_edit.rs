...
mod tests {
    ...
    /// Pins the contract for the `Resolution` arm of
    /// `dependent_still_present_in_graph`. Mirrors the Compute-arm precedent
    /// (`dependent_still_present_in_graph_filters_removed_compute_node`) —
    /// see that test's docstring for the structural justification.
    ///
    /// Resolution nodes are not yet seeded by `build_demand_for_graph`, so
    /// `edit_source`'s `eval_set = dirty_cone ∩ new_demand` intersection
    /// currently filters them out regardless of this arm's return value.
    /// Once Resolution-demand seeding lands, a buggy `false` here would cause
    ...
    fn dependent_still_present_in_graph_resolution_consults_new_graph_resolutions_map() {
        ...
        let rid = ResolutionNodeId::new("E", 0);
        graph_with_resolution.resolutions.insert(
            rid.clone(),
            ResolutionNodeData {
                id: rid.clone(),
                scope: "E".to_string(),
                auto_params: vec![],
                constraint_deps: vec![],
                content_hash: ContentHash::of_str("res0"),
            },
        );

        let resolution_node = NodeId::Resolution(rid.clone());

        // (a) Helper returns `true` against the graph that contains R.
        //     This is the contract the prior `false` arm violated; once
        //     Resolution-demand seeding lands, returning `false` here would
        //     cause step (6) to skip adding the stale Resolution to the dirty
        //     cone, silently retaining a stale cached value.
        assert!(
            super::dependent_still_present_in_graph(&resolution_node, &graph_with_resolution),
            "Resolution(R) must be reported as still-present against the graph that \
             contains it — this pins step (6)'s membership check for the Resolution \
             NodeId variant"
        );

        // (b) Helper returns `false` against an empty new graph.
        //     A Resolution that was itself removed must not be re-added to
        //     the dirty cone — symmetric with Value/Constraint/Realization/Compute.
        let empty_graph = EvaluationGraph::default();
        assert!(
            !super::dependent_still_present_in_graph(&resolution_node, &empty_graph),
...
structure GrowColl {
    ...
    // ------------------------------------------------------------------
    // β compare-site wiring: `edit_param`.
    //
    // These are in-crate (not `tests/`) because they read the
    // module-private `Engine::last_changed_realizations` field directly.
    // ------------------------------------------------------------------

    /// Two independent bodies plus a display-only `let` that feeds no
    /// realization — the fixture shape both `edit_param` and `edit_source`
    /// β tests need.
    ///
    /// `body_a`'s box reads `wa`; `body_b`'s reads `wb`. `label_pad` feeds
    /// only `pad_display`, so editing it must move NO realization's input
    /// cone (the PRD §6 "no-realization edit" boundary row).
    #[cfg(test)]
...
"#;
    ...
    /// An `Engine` backed by a real OCCT kernel — β's `edit_param`/
    /// `edit_source` tests need realizations to actually EXECUTE, because
    /// only `execute_realization_ops` writes α's `input_cone_hash`.
    /// Mirrors `tests/achieved_repr_tol.rs:60`.
    #[cfg(test)]
    fn make_occt_engine() -> crate::Engine {
        let checker = reify_constraints::SimpleConstraintChecker;
        let kernel = reify_kernel_occt::OcctKernelHandle::spawn();
        crate::Engine::new(Box::new(checker), Some(Box::new(kernel)))
    }

    /// Resolve the `RealizationNodeId` backing the geometry cell named
    /// `member`, via `RealizationNodeData::geometry_cell` (the GHR-δ S2
    /// link) rather than by assuming a declaration-order index.
    #[cfg(test)]
    fn realization_for_cell(
        ...
        let graph = &engine.eval_state.as_ref().unwrap().snapshot.graph;
        graph
            .realizations
            .iter()
            .find(|(_, n)| {
    ...
    /// realization behind `body_a` as changed — `body_b`'s input cone did
    /// not move, so it must be absent.
    ///
    /// This is the selectivity contract in its purest form. A regression
    /// that reports both realizations degrades γ's keyed eviction back into
    /// the wholesale flush it exists to replace; a regression that reports
    /// neither serves stale geometry.
    #[test]
    fn edit_param_reports_only_the_realization_whose_input_cone_moved() {
        ...
        let compiled = parse_and_compile_with_stdlib(TWO_BODY_SRC);
        let mut engine = make_occt_engine();
        let _ = engine.eval(&compiled);
        let _ = engine.build(&compiled, reify_ir::ExportFormat::Step);

        let ra = realization_for_cell(&engine, "TwoBody", "body_a");
        let rb = realization_for_cell(&engine, "TwoBody", "body_b");

        // PREMISE LOCK: α's production write must have populated BOTH
        // stored hashes. Without a kernel the realizations never execute,
        // every hash stays `None`, and the §11.2 "None → conservatively
        // changed" arm would make the real assertion below pass for
        // entirely the wrong reason (everything changed, always).
        {
            let graph = &engine.eval_state.as_ref().unwrap().snapshot.graph;
            for rid in [&ra, &rb] {
                assert!(
        ...
        // PRD §6 "no-realization edit" boundary row: `label_pad` feeds only
        // the display-only `pad_display` cell, so no realization's input
        // cone moves and the set must be empty — β pays nothing here.
        engine
            .edit_param(

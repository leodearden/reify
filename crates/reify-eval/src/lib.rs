            ...
            match kind {
                ...
                reify_compiler::SweepKind::Extrude => {
                    ...
                    let distance = args
                        ...
                        .find(|(n, _)| n == "distance")
                        .map(|(_, expr)| reify_expr::eval_expr(expr, &reify_expr::EvalContext::new(values, functions).with_meta(meta_map)))?;
                    // Validate that distance is a finite f64; discard the value since
                    // the Extrude variant stores the original Value (preserving units).
                    let _ = distance.as_f64().filter(|v| v.is_finite())?;

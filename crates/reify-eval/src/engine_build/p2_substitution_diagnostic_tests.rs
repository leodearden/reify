...
fn p2_substitution_suppression_cases_return_none() {
    ...
    assert!(
        p2_substitution_diagnostic(Some(&kind), true, ElementOrderTag::P2, "B_ForceTet").is_none(),
...
`element_order = P2` (P2 hex deferred). Accuracy for thin geometry is comparable to P2 tet."
    ...
    let revolve_diag = revolve_result.expect("Revolve variant must emit Some(Diagnostic) with P2");
    ...
    let sweep_diag = sweep_result.expect("SweepLinear variant must emit Some(Diagnostic) with P2");

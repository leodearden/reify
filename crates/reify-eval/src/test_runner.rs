...
"#;
    ...
    /// Regression test: `run_tests` dispatches `@optimized` FEA constraints via
    /// the registered trampoline rather than the body-inline fallback.
    ///
    /// # What this pins
    ///
    /// Without the `build_test_engine` fix, an `@optimized("solver::elastic_static")`
    /// call inside `run_tests` falls back to body-inlining (no registered trampoline).
    /// The body of `solve_elastic_static` is `ElasticResult()` — a struct with all
    /// Undef fields — so `result.max_von_mises > 0 Pa` evaluates `Undef > 0 Pa` →
    /// `Indeterminate` → `TestStatus::Indeterminate` (the *meaningless-pass* bug).
    ///
    /// With the fix the real FEA trampoline runs, computes a non-zero `max_von_mises`
    /// (≈ 3–6 MPa for a 1-kN cantilever), and the constraint evaluates to `Satisfied`
    /// → `TestStatus::Pass`.
    ///
    /// # Assertions
    ///
    /// **Primary:** no `"no registered compute trampoline"` fallback diagnostic —
    /// directly proves the trampoline was dispatched rather than skipped.
    ///
    /// **Secondary:** `status != Indeterminate` — directly pins the *meaningless-pass*
    /// regression: a real FEA result (Pass/Fail) means the constraint fired.
    ///
    /// # Note on FEA in unit tests
    ///
    /// This test intentionally invokes the real FEA solver (same codepath as
    /// `tests/solve_elastic_static_e2e.rs`) with the same minimal cantilever fixture
    /// (1 m × 100 mm × 100 mm, 1 kN tip load, fixed root).  It is the only way to
    /// prove the *end-to-end* fix — the precondition test
    /// (`build_test_engine_registers_compute_trampolines`) proves registration, but
    /// only this test proves that `run_tests` actually dispatches the trampoline and
    /// produces a firing constraint.  The coarse P1-tet mesh (~2.5K DOFs) keeps
    /// runtime well under 1 s.
    #[test]
    fn run_tests_fea_constraint_not_indeterminate() {
        use reify_constraints::SimpleConstraintChecker;
        // Minimal cantilever @test mirroring fea_cantilever_smoke.ri.
        // parse_and_compile_with_stdlib brings solve_elastic_static, Steel_AISI_1045,
        // PointLoad, FixedSupport, ConstitutiveLawInput, and ElasticOptions into scope.
        // With a 1 kN tip load the trampoline returns max_von_mises ≈ 3–6 MPa > 0 Pa.
        let source = r#"
...
"#;
        ...
        let results = super::run_tests(&module, || Box::new(SimpleConstraintChecker));
        assert_eq!(results.len(), 1, "expected exactly one @test result");
        // Primary pin: trampoline was dispatched, not body-inlined.
        assert!(
            results[0]
                .diagnostics
                .iter()
                .all(|d| !d.message.contains("no registered compute trampoline")),
            "run_tests emitted 'no registered compute trampoline' fallback diagnostic — \
             trampoline not dispatched, fix not in effect; diagnostics: {:?}",
            results[0].diagnostics
        );
        // Secondary pin: constraint fired (Pass/Fail), not silently Indeterminate.
        assert_ne!(
            results[0].status,
            super::TestStatus::Indeterminate,
            "FEA constraint in @test evaluated to Indeterminate — trampoline result \
             was Undef (meaningless-pass bug not fixed); \
             diagnostics: {:?}",
            results[0].diagnostics
        );
    }

...
mod tests {
    ...
    #[test]
    fn sweep_kind_extrude_symmetric_variant_exists() {
        let k = SweepKind::ExtrudeSymmetric;
        // Copy trait: the binding above is a copy out of the enum.
        let k2: SweepKind = k;
        // PartialEq/Eq + matches!
        assert_eq!(k, k2);
        assert!(matches!(k, SweepKind::ExtrudeSymmetric));
        // Hash: usable as a map key.
        let mut set = std::collections::HashSet::new();
        set.insert(k);
        assert!(set.contains(&SweepKind::ExtrudeSymmetric));
    }

    #[test]
    fn sweep_kind_sweep_guided_variant_exists() {
        let k = SweepKind::SweepGuided;
        let k2: SweepKind = k;
        assert_eq!(k, k2);
        assert!(matches!(k, SweepKind::SweepGuided));
        let mut set = std::collections::HashSet::new();
        set.insert(k);
        assert!(set.contains(&SweepKind::SweepGuided));
    }

    #[test]
    fn sweep_kind_loft_guided_variant_exists() {
        let k = SweepKind::LoftGuided;
        let k2: SweepKind = k;
        assert_eq!(k, k2);
        assert!(matches!(k, SweepKind::LoftGuided));
        let mut set = std::collections::HashSet::new();
        set.insert(k);
        assert!(set.contains(&SweepKind::LoftGuided));
    }

    #[test]
    fn sweep_kind_new_variants_distinct_from_existing() {
        assert_ne!(SweepKind::ExtrudeSymmetric, SweepKind::Extrude);
        assert_ne!(SweepKind::SweepGuided, SweepKind::Sweep);
        assert_ne!(SweepKind::LoftGuided, SweepKind::Loft);
        assert_ne!(SweepKind::ExtrudeSymmetric, SweepKind::SweepGuided);
        assert_ne!(SweepKind::SweepGuided, SweepKind::LoftGuided);
    }
}

...
///
/// All factors are emitted as plain decimal literals. This is a
/// self-imposed convention for the readability of the generated
/// `std.si_units` source, not a grammar constraint — tree-sitter's
/// `number_literal` grammar does accept scientific notation.
...
///
/// Used instead of `f64` formatting because plain-decimal output is a
/// self-imposed convention for the readability of the generated
/// `std.si_units` source (the tree-sitter grammar itself does accept
/// scientific notation), and f64 can't faithfully represent 1e-30 as a
/// plain decimal anyway.
...
///
/// Plain-decimal output (no scientific notation) is a self-imposed
/// convention for the readability of the generated `std.si_units` source
/// — the tree-sitter `number_literal` grammar does accept scientific
/// notation — so Rust's default `{}`/`{:?}` formatting (which switches to
/// `e`-notation for very small or very large values) is not used directly.
/// This function instead picks a precision proportional to the value's
/// decimal exponent so the full 17-significant-digit round-trip precision
/// of f64 is preserved, then trims trailing zeros.
///
/// Guarantees:

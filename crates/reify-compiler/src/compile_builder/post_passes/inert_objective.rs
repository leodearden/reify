//! The `E_OBJECTIVE_INERT` post-pass — DIC γ (task #5417, PRD
//! `docs/prds/v0_6/declared-intent-consumption-accounting.md` §3 decision 3).
//!
//! Reports every template whose declared objective provably governs nothing:
//! no solver variable can move its cost, because every cell it can reach is
//! permanently non-`auto`.
//!
//! Its own module rather than a block inside [`super`], which is the shared
//! home of the unrelated recursion / duplicate-signature / field-composition /
//! purpose passes. Only [`phase_inert_objective_check`] leaves it; everything
//! else — the proof obligations, their helpers, and the unit suite that pins
//! them — is private to this file, so the decision and its evidence read
//! together.

...
// ─── task 4794 step-1: Vec3<Q> real stdlib prelude resolution ─────────────────

/// A user module that references `Vec3<Pressure>` (not declared in user source)
/// must compile cleanly against the real stdlib prelude — `Vec3<Q>` lives in
/// trajectory.ri, seeded into the user module's alias registry by
/// `compile_with_stdlib`.
///
/// `Vec3<Pressure>` → body `Vector3<Q>` with Q=Pressure → Type::vec3(Pressure).
///
/// This is the precise, harness-free counterpart to the committed .ri signal
/// (step-4/step-5); it also guards that generalising `Vec3` in trajectory.ri
/// keeps the stdlib build clean.
///
/// RED on base: trajectory.ri:102 declares the 0-arg `pub type Vec3 = Vector3<Length>`,
/// so `Vec3<Pressure>` is an arity error (1 arg vs 0 params).
#[test]
fn vec3_alias_resolves_via_real_stdlib_prelude() {
    let source = "structure def S { param p : Vec3<Pressure> }";
    ...
    assert!(
        info_diags.is_empty(),
        "Vec3<Pressure> via real stdlib prelude must produce zero Info diagnostics; got: {:?}",
        info_diags
    );

...
/// the closure `f` with a reference to the matched `ForallConnectDecl`,
/// returning whatever the closure returns. Panics if no `ForallConnect` is
/// found in the named structure.
...
/// right side is an unsupported port shape.
///
/// Fixture: `connect air_channel -> v.inner.a` — the left side (`air_channel`) is
/// a plain `Ident` that resolves fine; the right side (`v.inner.a` after
/// substitution) is a doubly-nested `MemberAccess` that hits `_ => None` in
/// `resolve_port_name`.
///
/// Pins:
/// (a) No errors.
/// (b) Zero `CompiledConnections` and zero `forall@*` constraint labels.
/// (c) Zero `CompiledForallTemplates`.
/// (d) Exactly one `Severity::Info` diagnostic total.
/// (e) The diagnostic has a label with `message == "forall connect"` and
...
/// sides have unsupported port shapes.
///
...
/// to represent realistic nested sub-component port accesses. When nested
/// sub-components gain full port reach, a realistic fixture should be added.
///
/// Pins:
/// (a) No errors.
/// (b) Zero `CompiledConnections` and zero `forall@*` constraint labels.
...
/// `with_first_forall_constraint` selects the structure identified by name
/// when multiple structures are present, ignoring other structures.
#[test]
fn with_first_forall_constraint_selects_named_structure() {
    let source = r#"
structure T {
    param x : Int
}
...
"#;
    let variable = with_first_forall_constraint(source, "S", |f| f.variable.clone());
    assert_eq!(variable, "v", "expected bound variable 'v' from S, got {:?}", variable);
}

/// `with_first_forall_constraint` returns the first ForallConstraint when the
/// named structure contains multiple (first-match semantics).
#[test]
fn with_first_forall_constraint_returns_first_member() {
    let source = r#"
structure S {
    forall v in [1, 2, 3]: constraint v > 0
    forall w in [4, 5, 6]: constraint w > 3
}
"#;
    let variable = with_first_forall_constraint(source, "S", |f| f.variable.clone());
    assert_eq!(variable, "v", "expected first member variable 'v', got {:?}", variable);
}

/// `with_first_forall_constraint` panics with a message naming the structure
/// when the named structure has no ForallConstraint member.
#[test]
#[should_panic(expected = "no ForallConstraint found in structure NoSuchStruct")]
fn with_first_forall_constraint_panics_when_not_found() {
    let source = r#"
structure S {
    forall v in [1, 2, 3]: constraint v > 0
}
"#;
    with_first_forall_constraint(source, "NoSuchStruct", |f| f.span);
}

/// `with_first_forall_connect` selects the structure identified by name when
/// multiple structures are present, ignoring other structures.
#[test]
fn with_first_forall_connect_selects_named_structure() {
    // make_fixture produces two structures: Vent (no ForallConnect) and S (has one).
    // Requesting "S" must return the connect's bound variable, not panic.
    let source = make_fixture("v.a", "hub");
    let variable = with_first_forall_connect(&source, "S", |f| f.variable.clone());
    assert_eq!(variable, "v", "expected bound variable 'v' from S, got {:?}", variable);
}

/// `with_first_forall_connect` returns the first ForallConnect when the named
/// structure contains multiple (first-match semantics).
#[test]
fn with_first_forall_connect_returns_first_member() {
    // Structure S has two ForallConnect members; the helper must return the first.
    let source = r#"
trait Air { param d : Length }
structure def Vent {
    port inlet : out Air { param d : Length = 5mm }
...
structure def S {
    ...
    port air_channel : in Air { param d : Length = 5mm }
    forall v in vents: connect v.inlet -> air_channel
    forall w in vents: connect w.inlet -> air_channel
}
"#;
    let variable = with_first_forall_connect(source, "S", |f| f.variable.clone());
    assert_eq!(variable, "v", "expected first member variable 'v', got {:?}", variable);
...
/// `with_first_forall_connect` panics with a message naming the structure when
/// the named structure has no ForallConnect member.
#[test]
#[should_panic(expected = "no ForallConnect found in structure NoSuchStruct")]
fn with_first_forall_connect_panics_when_not_found() {
    let source = make_fixture("v.a", "hub");
    with_first_forall_connect(&source, "NoSuchStruct", |f| f.span);
}

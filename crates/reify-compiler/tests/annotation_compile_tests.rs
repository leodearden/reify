...
fn annotation_with_args_on_function_propagates() {
    let module =
        compile_module(r#"@deprecated("use new_calc") fn old_calc(x: Real) -> Real { x }"#);
    assert!(
        errors_only(&module).is_empty(),
        "errors: {:?}",
        errors_only(&module)
...
fn annotation_on_field_propagates() {
    let module =
        compile_module("field def temp_field : Point3 -> Real { source = analytical { |p| 0.0 } }");
...
fn known_annotation_invalid_context_produces_warning() {
    ...
    let module =
        compile_module("@test field def f : Point3 -> Real { source = analytical { |p| 0.0 } }");
    assert!(
        errors_only(&module).is_empty(),
        "errors: {:?}",
        errors_only(&module)
...
fn multiple_annotations_all_preserved() {
    let module = compile_module(r#"@test @deprecated("old") structure S { param x : Real }"#);
    assert!(
        errors_only(&module).is_empty(),
        "errors: {:?}",
        errors_only(&module)

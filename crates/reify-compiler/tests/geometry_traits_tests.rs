...
// ─── Task 2699 — 11 new topology-selector compile-with-stdlib tests ───────────
//
// Regression-lock: each test asserts that the named helper compiles cleanly
// and that its cell type equals the registry-mandated type from
// `GEOMETRY_TOPOLOGY_SELECTOR_NAMES` / `topology_selector_result_type`.
// Eval-side dispatch for all 11 names is deferred to task 2691; the cell
// holds `Value::Undef` at runtime until that task lands.
//
// Call shapes follow PRD §3.9 signatures. Compile-time wiring keys only on
// the function name — arg types are not validated at compile-time — so
// passing `body, body` for `adjacent_faces`/`shared_edges` (two geometry
// handles) is sufficient to exercise the registry arm, mirroring the
// `angle_between_surfaces(body, body)` precedent above.

#[test]
fn edges_let_binding_compiles_with_list_geometry_type() {
    let compiled = assert_topology_selector_let_compiles("edges(body)", "es");
    assert_helper_cell_typed(
        &compiled,
        "es",
...
#[test]
fn faces_let_binding_compiles_with_list_geometry_type() {
    let compiled = assert_topology_selector_let_compiles("faces(body)", "fs");
    assert_helper_cell_typed(
        &compiled,
        "fs",
        &Type::List(Box::new(Type::Geometry)),
...
#[test]
fn edges_by_length_let_binding_compiles_with_list_geometry_type() {
    let compiled =
        assert_topology_selector_let_compiles("edges_by_length(body, 0mm..50mm)", "ebl");
    assert_helper_cell_typed(
        &compiled,
        "ebl",
        &Type::List(Box::new(Type::Geometry)),
...
#[test]
fn faces_by_area_let_binding_compiles_with_list_geometry_type() {
    let compiled = assert_topology_selector_let_compiles(
...
#[test]
fn faces_by_normal_let_binding_compiles_with_list_geometry_type() {
    let compiled = assert_topology_selector_let_compiles(
...
#[test]
fn edges_parallel_to_let_binding_compiles_with_list_geometry_type() {
    let compiled = assert_topology_selector_let_compiles(
...
#[test]
fn edges_at_height_let_binding_compiles_with_list_geometry_type() {
    let compiled =
        assert_topology_selector_let_compiles("edges_at_height(body, 0mm, 0.01mm)", "eah");
    assert_helper_cell_typed(
        &compiled,
        "eah",
        &Type::List(Box::new(Type::Geometry)),
...
#[test]
fn adjacent_faces_let_binding_compiles_with_list_geometry_type() {
    // Passes `body, body` (two geometry handles) matching the
    // `angle_between_surfaces(body, body)` precedent — compile-time wiring
    // keys only on the function name.
    let compiled = assert_topology_selector_let_compiles("adjacent_faces(body, body)", "af");
    assert_helper_cell_typed(
        &compiled,
        "af",
        &Type::List(Box::new(Type::Geometry)),
...
#[test]
fn shared_edges_let_binding_compiles_with_list_geometry_type() {
    let compiled = assert_topology_selector_let_compiles("shared_edges(body, body)", "se");
    assert_helper_cell_typed(
        &compiled,
        "se",
        &Type::List(Box::new(Type::Geometry)),
...
#[test]
fn center_of_mass_let_binding_compiles_with_point3_length_type() {
    // density arg is a bare Real (7850.0 kg/m³ for steel) — same convention
    // as block_inertia.ri.
    let compiled = assert_topology_selector_let_compiles("center_of_mass(body, 7850.0)", "com");
    assert_helper_cell_typed(&compiled, "com", &Type::point3(Type::length()));
}

#[test]
fn moment_of_inertia_let_binding_compiles_with_tensor_2_3_moment_of_inertia_type() {
    let compiled =
        assert_topology_selector_let_compiles("moment_of_inertia(body, 7850.0)", "moi");
    assert_helper_cell_typed(
        &compiled,
        "moi",
        &Type::tensor(

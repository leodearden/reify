...
/// cleanly via the full stdlib pipeline (with prelude-aware parsing —
/// `CorrosionClass` and `BiocompatibilityClass` are resolved against the
/// stdlib without inline re-declaration), carry both traits as bounds, and
/// have value cells for both enum-typed params with correct Enum cell_type.
#[test]

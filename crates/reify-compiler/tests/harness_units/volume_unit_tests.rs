...
//! `UnitRegistry::lookup` (crates/reify-compiler/src/units.rs:1533), an exact
...
fn l_litre_unit_resolves_to_volume() {
    ...
    // all (see tests/prd-gate/fixtures/unit_curated_labels_ascii.ri:32-33).
...
fn lowercase_l_stays_unknown() {
    ...
    // cannot express a negative (torque_unit_tests.rs:33-36 documents the same
    // constraint).
    ...
    assert!(
        errors.iter().any(|d| d.message.contains('l')),
        "expected an Error naming 'l'; got: {:?}",
...
///
/// Path resolution mirrors torque_unit_tests.rs:123-127: the committed fixture
/// is the single source of truth, read from disk rather than inlined, so it
/// cannot drift out of sync. Read-only input: this test must never edit it.
#[test]

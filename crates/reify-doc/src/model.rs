...
mod tests {
    ...
    #[test]
    fn item_doc_double_flatten_round_trips_via_json_but_not_bincode() {
        // Guards the contract described in the `# Serde notes` section on `ItemDoc`
        // (model.rs lines 336-342): the double `#[serde(flatten)]` on `header` (struct)
        // + `kind` (internally-tagged enum) only round-trips via self-describing formats
        // such as JSON.  Non-self-describing formats (bincode, postcard, …) that
        // lack `deserialize_any` support will fail at runtime — this test makes that
        // failure observable as a CI regression rather than a silent data-corruption.
        let item = ItemDoc {
            header: ItemHeader {
                name: "Board".to_string(),
                doc: Some("Main PCB board.".to_string()),
        ...
        // Positive case: JSON (self-describing) round-trips correctly.
        let json = serde_json::to_string(&item).expect("serialize");
        assert!(
            json.contains("\"kind\":\"structure\""),
            "flattened kind tag present in JSON: {json}"
        );
        let back: ItemDoc = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(item, back);

        // Negative case: bincode (NOT self-describing) must NOT round-trip.
        // bincode 1.x lacks `deserialize_any`, so flatten + internally-tagged enums
        // cannot be decoded.  The assertion does not pin which step fails (serialize
        // vs deserialize) so it remains robust across bincode 1.x patch releases.
        // See the `# Serde notes` on `ItemDoc` for the full explanation.
        let bin_round_trip: Result<ItemDoc, _> = bincode::serialize(&item)
            .and_then(|bytes| bincode::deserialize::<ItemDoc>(&bytes));
        assert!(
            bin_round_trip.is_err(),
            "ItemDoc must NOT round-trip via bincode (a non-self-describing format) \
             because of the double #[serde(flatten)] on header + kind. \
             See the Serde notes on ItemDoc in this file. \
             Got: {bin_round_trip:?}"
        );
    }
}

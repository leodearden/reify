...
/// parent face and edge. The test:
/// - builds two overlapping 10mm cubes (right offset by +5mm in X so the
///   common of A ∩ B yields the overlap half-box);
/// - calls `boolean_common_with_history` on the actor handle;
/// - asserts the result is a non-empty solid (volume > 0);
/// - asserts at least one face from the left parent (A) appears in the history
///   (Modified ∪ Generated);
/// - asserts every record carries an in-range parent_index (∈ {0, 1}) and
///   a parent_subshape_index < 6 (a box has 6 faces) or < 12 (edges);
/// - asserts edges history is similarly non-empty and well-formed;
/// - asserts silent_drop_count == 0.
///
...
fn boolean_common_with_history_reports_per_parent_face_and_edge_records() {
    ...
    let left = kernel
        ...
        .expect("left box should build");
    // Box B: same shape, translated by +5mm in X so the two boxes overlap
    // by exactly half. A ∩ B = the overlap half-box [0,5mm]×[-5,5mm]×[-5,5mm]
    // (vol ≈ 500 mm³ > 0).
    let right_origin = kernel
        .execute(&ten_mm_box_op())
        .expect("right box should build");
    ...
    let (result_handle, history) = kernel
        .boolean_common_with_history(left.id, right.id)
    ...
    // (d) Modified or Generated history is non-empty.
    assert!(
        !history.face_modified.is_empty() || !history.face_generated.is_empty(),
        "BRepAlgoAPI_Common history should have non-empty Modified or Generated face records \
         for two overlapping boxes — got {} modified, {} generated",
        history.face_modified.len(),
        history.face_generated.len()
    );
    ...
    // (f) At least one face from the left parent (A) appears in the history.
    //     Note: BRepAlgoAPI_Common may not report Generated records from the
    //     right parent — similar to Cut, only the left parent's faces may appear
    //     in Modified. We assert only on parent_index==0.
    let face_records: Vec<_> = history
        .face_modified
        .iter()
        .chain(history.face_generated.iter())
    ...
    assert!(
        !history.edge_modified.is_empty() || !history.edge_generated.is_empty(),
        "BRepAlgoAPI_Common history should have non-empty Modified or Generated edge records \
         — got {} modified, {} generated",
        history.edge_modified.len(),
        history.edge_generated.len()
    );
    ...
    // (h) Silent-drop counter must be zero for a well-formed 5mm-overlap common.
    assert_eq!(
        history.silent_drop_count, 0,
        "vanilla overlap common should not silently drop any Modified/Generated child \
         — got {}",
        history.silent_drop_count
    );

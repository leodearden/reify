...
fn fillet_with_history_reports_face_records() {
    ...
    //
    // Formula: strip the 6 modified-original box faces and the 8 corner-fillet
    // faces from result_face_count; the remaining faces should all be lateral
    // fillet faces (1 per parent edge, 12 for a cube), so face_generated should
    // account for at least that many records.
    //
    // For the standard 10mm cube + 1mm fillet: result_face_count ≈ 26,

...
/// Rigid-body transform POD mirroring `Transform3Props` in ffi.rs.
///
/// `gp_Quaternion` constructor order is `(x, y, z, w)`, so the C++
/// implementation explicitly passes `gp_Quaternion(t.qx, t.qy, t.qz, t.qw)`.
/// This struct is generated by cxx into `reify-kernel-occt/src/ffi.rs.h`; the
/// declaration here lets the function signature compile independently of the
/// cxx-generated header inclusion order.

...
/// mesher.
///
/// This association is already maintained by the surface-mesh path; this type
/// exposes it on the consumer side so the Dirichlet-BC projection step can look
/// up the mapped counterpart via [`CorrespondenceMap`] — without any globally-
/// closest fallback.
///
/// The actual surface-mesh-path producer (Gmsh adapter) is stubbed today; this
/// task defines the consumer-side shape so the producer can populate
/// [`BoundaryAssociation`] without further API design.
...
/// - FEA warm-start in PRD task #15 (BC order must be bit-stable between
///   a morphed-mesh rebuild and the warm-start cache lookup).

...
// G-allow: mesh-morph engine call-site wiring; consumer task #4744 (done) — the §8 task β morph arm landed its diagnostics via diagnostics.rs (record_morphed/record_quality_remesh/record_ineligible/record_panicked), not these stats.rs counters; remains an in-scope orphan BY DESIGN (see PINS in stats_g_allow.rs); re-homed from cancelled #3429

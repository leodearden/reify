...
//!
//! ## OCCT-gated test (step-3 RED / step-4 GREEN)
//!
//! `check_dfm_overhang_emits_one_w_dfm_overhang_under_occt` exercises the
//! headline signal from PRD §7.2: `reify check <dfm-file>` must emit exactly
//! one `W_DFM_OVERHANG` diagnostic on stderr when OCCT is present.
//!
//! - Fixture `dfm_overhang.ri`: an FDM Adding conformer with
//!   `max_overhang_angle=0deg` plus a `DFMRule(Warning)` OverhangRule whose
//!   box subject (bottom face dips 90° past the build plane) always violates.
//!   No `constraint` declarations → the module exits 0 (Warning severity is
//!   non-fatal).
//! - Under OCCT: stderr carries exactly one `W_DFM_OVERHANG`; no
//!   `E_DFM_OVERHANG`; exit 0.
//! - Stub mode (no OCCT): `measure_dfm_rules` C1 no-op guard fires
//!   (`default_kernel_name` is None → no kernel → no handles → no diagnostic).
//!   exit 0, no `W_DFM_OVERHANG` (C1 graceful degradation, never a false
//!   positive).
//!

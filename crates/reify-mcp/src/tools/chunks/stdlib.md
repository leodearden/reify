...
**Modify:** `fillet(solid, radius)` or `fillet(solid, edges, radius)`, `fillet_all(solid, radius)`, `chamfer(solid, distance)` or `chamfer(solid, edges, distance)`, `chamfer_asymmetric(solid, edges, d1, d2)` (the edge selector is mandatory here, unlike `chamfer`), `shell(solid, thickness, faces…)` (thickness first, then optional face indices to remove), `shell_open(solid, thickness, faces)`, `offset_solid(solid, distance)`, `thicken(solid, offset)`, `draft(solid, angle, plane)` or `draft(solid, faces, angle, plane)`, `offset_curve(curve, distance)`

...
/// planar-faced solid (`crates/reify-kernel-occt/src/handle.rs`), i.e. what the
/// production surface→volume path receives BEFORE `RepairConfig`'s weld
/// pre-stage runs. Enclosed volume must be identical to the welded form: the

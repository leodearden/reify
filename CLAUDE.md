...
### Warm lanes
- The lane flock is a reseed mutex, **not** a one-consumer-per-lane liveness oracle: `<lane_dir>.lock` is held only across the ACQUIRE reseed and across `run_scoped_verification`, never across the implement phase, so a flock-only test is blind to an agent that has been building for tens of minutes (root-caused 2026-07-26, esc-5334-6). Status per script: `warm-lane-gc.sh` is REPAIRED by #5572 (a per-lane live process-reference gate in **both** passes); `thin-warm-lane.sh` is **still flock-only** — the gap is OPEN there, tracked as #5823. At ACQUIRE time, since #5223, `seed-warm-lane.sh --fresh-checkout --lane-lock` holds `flock -x <lane_dir>.lock` across the reseed, refusing/queuing on a live consumer (§9.5 inv.11) — serialising the destructive reseed, not proving the lane idle. `acquire_lane` always re-seeds from the base; only the `_merge-verify` lane's clean landed-commit `target/` may advance the base (`refresh-warm-base.sh --landed-commit <sha>`) — task-lane WIP must **never** advance it. Gate mechanism, ordering and cost: `docs/prds/warm-lane-pool-space-safety.md` §8.4. Full lifecycle, invariants, and pool sizing: `docs/prds/warm-lane-pool-cow-seeding.md` §9.3/§9.5 (inv.10/inv.11).

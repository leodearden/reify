...
### Warm lanes
- One consumer per lane at a time is **not** delivered by the lane flock alone: `<lane_dir>.lock` is held only across the ACQUIRE reseed and across `run_scoped_verification`, never across the implement phase, so a flock-only test is blind to an agent that has been building for tens of minutes (root-caused 2026-07-26, esc-5334-6). `warm-lane-gc.sh` is REPAIRED by #5572 — both passes add a per-lane live process-reference scan (cwd / open fd / mmap; `scripts/lib_live_refs.sh`), evaluated immediately before that entry's reset/remove under the flock the loop already holds, deliberately **not** an up-front batch snapshot (that is the TOCTOU the gate closes). `thin-warm-lane.sh` is **still flock-only** — #5572 did not touch it, so the gap remains open there. At ACQUIRE time, since #5223, `seed-warm-lane.sh --fresh-checkout --lane-lock` holds `flock -x <lane_dir>.lock` across the reseed, refusing/queuing on a live consumer (§9.5 inv.11) — a mutex serialising the destructive reseed, not a liveness oracle for the pool. `acquire_lane` always re-seeds from the base; only the `_merge-verify` lane's clean landed-commit `target/` may advance the base (`refresh-warm-base.sh --landed-commit <sha>`) — task-lane WIP must **never** advance it. Full lifecycle, invariants, and pool sizing: `docs/prds/warm-lane-pool-cow-seeding.md` §9.3/§9.5.
